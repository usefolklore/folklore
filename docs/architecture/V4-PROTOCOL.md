# Folklore v4 — Agent Brain Protocol
...
> reference only — run `folklore migrate v5` to upgrade live data.

**Version:** 0.4 (draft, supersedes V3-PROTOCOL.md)
**Status:** Reference implementation shipped in Folklore v4.x; spec stabilising ahead of v4.0 tag
...
Folklore's daemon listens on a Unix-domain socket at `${FOLKLORE_HOME}/daemon.sock` (POSIX permissions 0600 — only the owning user can connect).

Protocol is newline-delimited JSON, request/response. One request per line, one response per line.

```
client → daemon:  {"id": <number>, "cmd": "ask", "args": ["--room", "folklore-dev", "p2p memory"]}
...
`bin/folklore.js` checks for the socket file BEFORE importing `dist/cli/index.js`. If present and the command is in the delegatable set (currently `ask`, `stats`), the request is forwarded over IPC. Otherwise the CLI takes the normal spawn path.
...
**Default-on** in v4 — `defaultRuntime()` wraps the chosen backend automatically. Opt-out via `FOLKLORE_EMBEDDER_BATCH=off`.
...
Reference: `92f797d`, `bench/bench-embed-throughput.mjs`.
...
Episodic memory entries (raw session transcripts, ingested chat logs, observed events) accumulate linearly. A brain compresses them via overnight replay into semantic schemas. Folklore v4 ships this as a CLI primitive plus a graph-node schema.
...
`${FOLKLORE_HOME}/folklore.lock` — POSIX exclusive-create file.
...
```bash
FOLKLORE_VECTOR_QUANTIZATION=binary-512   # 64 bytes/vec, -1.79pt worst-case (measured)
FOLKLORE_VECTOR_QUANTIZATION=binary-768   # 96 bytes/vec, -1.10pt worst-case (safer)
...
bin/folklore.js                  Client-side IPC delegation (Phase 1)
...
bench/bench-consolidation.mjs      Storage + quality gate harness (Phase 4d)
...
bench/bench-embed-throughput.mjs   Throughput diagnostic (Phase 2)
```
...
- **HippoRAG-2 multi-hop PPR bench skeleton** — algorithmic harness (`bench/bench-ppr-multihop.mjs`) reuses `src/domain/pagerank.ts` over a localized doc-doc kNN graph. Synthetic mode validates pipeline; `--dataset hotpotqa|musique` is the path to the real gate.
...
- **SignedShareableNode envelope primitive** — `src/domain/share-envelope.ts` builds on the identity primitive to wrap each ShareableNode in a verifiable Ed25519 envelope chain. Sign/verify roundtrip + 9 tamper-detection tests pass; wiring into the share-sync inbound observer ships behind `FOLKLORE_REQUIRE_SIGNED_NODES` in a follow-up commit.

**Still deferred:**

- **v4.1 — Native binary client** (Rust `folklore-cli` that speaks the IPC protocol directly, bypassing Node boot). Target: warm-hit latency 100ms → 15ms.
...
- **v4.2 — Real-data PPR gate** — wire `bench/bench-ppr-multihop.mjs` to BEIR HotpotQA + MuSiQue corpora. If +3pt NDCG@10 or +5pt R@5, enable `multi_hop: true` flag on MCP queries.
...
- **v4.2 — Wire `FOLKLORE_REQUIRE_SIGNED_NODES` into share-sync inbound observer** — the receive-side complement to §5.6, using the now-shipped envelope primitive.
...
- Every measurement claim is reproducible via the bench scripts in `bench/bench-*.mjs`. Reviewers verify via `git log` + the reproduction commands. No "trust me" in this stack.
