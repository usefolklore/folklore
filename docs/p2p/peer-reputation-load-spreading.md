...
Two facts about the existing Folklore code make load-spreading
**incremental, not architectural**:

1. **Touch already replicates chunks.** When peer A `folklore touch`-es a
   chunk from peer B, that chunk lands in A's `graph.json` with
   `_folklore_source_peer: B` provenance. A is now a secondary source for
...
**What it does.** `folklore touch --peer B --label "lemlist pricing"` pulls
the chunk and writes it to A's local `graph.json` with `_folklore_source_peer: B`.
...
- Touch is *manual* — the user has to know to run `touch` for a specific node.
  ...
  flag to `folklore ask --peers` that, on a high-confidence federated
...
*touched* from peer B (carrying `_folklore_source_peer: B`), the
...
local update path to credit `_folklore_source_peer` chains).
...
**Files:** new wire protocol `/folklore/seed/1.0.0`. Substantial — Phase 3
or later.
...
Item (3) is the rep-update path crediting `_folklore_source_peer` chains.
