...
## Output
```
The repo is carrying three different stories at once: old `wellinformed` personal/team memory, mid-phase retrieval leaderboard work, and current `Akashik` OSS commons. The cleanup should optimize for one thing: make the federation-compounding thesis look like the product, and demote everything else to history.
...
| `package.json` | NEEDS-REWRITE | Still markets `wellinformed` as an “agent memory” package; code name can stay, description should not. |
...
| `wellinformed-rs/` | KEEP | Not obviously dead; still supports current performance path during the two-name period. |
...
codex
Auditing the repository structure and representative files first, then I’ll make hard keep/archive/delete calls tied to the Akashik pivot rather than the older wellinformed framing.
exec
/bin/zsh -lc "printf 'SRC\\n' && find src -maxdepth 3 -type f | sort" in /Users/saharbarak/personal/wellinformed
exec
/bin/zsh -lc "printf 'DOCS\\n' && find docs -maxdepth 3 -type f | sort" in /Users/saharbarak/personal/wellinformed
exec
/bin/zsh -lc "printf 'TESTS\\n' && find tests -maxdepth 3 -type f | sort" in /Users/saharbarak/personal/wellinformed
exec
/bin/zsh -lc "pwd && printf '\\nROOT\\n' && find . -maxdepth 1 -mindepth 1 | sort" in /Users/saharbarak/personal/wellinformed
 succeeded in 0ms:
DOCS
...
exec
/bin/zsh -lc "printf 'PLANNING\\n' && find .planning -maxdepth 4 -type f | sort" in /Users/saharbarak/personal/wellinformed
 succeeded in 0ms:
PLANNING
...
exec
/bin/zsh -lc "sed -n '1,220p' docs/product/ROADMAP.md" in /Users/saharbarak/personal/wellinformed
 succeeded in 0ms:
# Akashik — Roadmap
...
exec
/bin/zsh -lc "sed -n '1,220p' .planning/phases/phase-21/21-CONTEXT.md" in /Users/saharbarak/personal/wellinformed
 succeeded in 0ms:
...
exec
/bin/zsh -lc "sed -n '1,220p' .planning/phases/phase-23/23-CONTEXT.md" in /Users/saharbarak/personal/wellinformed
 succeeded in 0ms:
...
exec
/bin/zsh -lc "sed -n '1,200p' tests/bench-akashik-federation.test.ts" in /Users/saharbarak/personal/wellinformed
 succeeded in 0ms:
/**
...
exec
/bin/zsh -lc "sed -n '1,200p' tests/bench-real.test.ts" in /Users/saharbarak/personal/wellinformed
 succeeded in 0ms:
/**
...
exec
/bin/zsh -lc "rg -n \"\\.claude-octopus|WELLINFORMED_BENCH_PUBLIC_REAL|storybrand|wellinformed|Akashik\" -g '"'!node_modules'"' -g '"'!dist'"' ." in /Users/saharbarak/personal/wellinformed
 succeeded in 0ms:
...
./src/domain/metrics.ts:13: *   - `wellinformed metrics` IPC command returns a JSON snapshot.
./src/domain/bench-types.ts:42: *   - 0.10 tier-promotion F1 — wellinformed-specific
./src/domain/release.ts:4: * A release manifest published by the wellinformed project is a JSON
./src/domain/release.ts:161:  const message = new TextEncoder().encode(`wellinformed-release:v1:${canonicalManifestJSON(manifest)}`);
./src/domain/log-event.ts:4: * Every observable behavior on a wellinformed node is one of these
./src/domain/log-event.ts:143: * One-line human-readable rendering for `wellinformed logs tail`.
./src/domain/recency-rerank.ts:98: * (eventually) by `wellinformed ask --explain`.
./src/domain/peer-telemetry.ts:2: * Peer-pull telemetry — the record emitted every time wellinformed
./src/domain/job.ts:3: * and the `wellinformed jobs` CLI surface. The queue itself + the
./src/domain/job.ts:64: * Project ingest — what `wellinformed this` actually wants. Runs the
./src/domain/job.ts:68: * from the room+root inputs, mirroring `wellinformed index`.
./src/domain/llm-extractor.ts:12: *   1. retrieve top-k via wellinformed
./findings.md:3:This synthesis evaluates 2024-2026 retrieval techniques against the strict constraints of the `wellinformed` pipeline (CPU-only, ARM Hetzner CAX11 4GB, TypeScript + transformers.js + sqlite-vec), specifically targeting the 10-13pp loss in multi-session and temporal-reasoning questions on LongMemEval-S.
./src/domain/hot-cache.ts:6: * `~/.wellinformed/hot.md`. A new Claude session reads hot.md at
./src/domain/hot-cache.ts:127:  lines.push('generator: wellinformed');
./src/domain/hot-cache.ts:164:  lines.push(`_Generated by wellinformed. Reload with \`wellinformed hot --refresh\`._`);
./src/domain/system-rooms.ts:2: * System-managed rooms — the out-of-the-box rooms every wellinformed
./src/domain/system-rooms.ts:19: *    A git commit tagged `room: wellinformed-dev` is STILL in toolshed.
./src/domain/system-rooms.ts:38: * opt-in for P2P sharing via the interactive `wellinformed share` TUI.
./src/domain/graph.ts:2: * Pure domain model for the wellinformed knowledge graph.
./src/domain/graph.ts:43:/** wellinformed-added optional fields. Declared in graphify.validate.OPTIONAL_NODE_FIELDS. */
./src/domain/bloom.ts:2: * Pure Bloom filter — the primitive behind wellinformed v3's
./src/domain/entity.ts:60:  /** Optional human note shown in `wellinformed entity list`. */
./src/cli/ipc-client.ts:6: * Mirrors the wire format used in bin/wellinformed.js but lives in
./src/cli/ipc-client.ts:18:import { wellinformedHome } from './runtime.js';
./src/cli/ipc-client.ts:33:    const sockPath = join(wellinformedHome(), 'daemon.sock');
./src/domain/save-note.ts:2: * save-note — pure helpers for the `wellinformed save` command.
./src/domain/save-note.ts:72:    source_file: 'wellinformed:save',
./src/domain/identity.ts:4: * This module owns the pure domain layer for wellinformed's three-tier
./src/domain/identity.ts:508: *   `wellinformed-auth:v1:{device_id}:{hex(device_pub_key)}:{authorized_at}`
./src/domain/identity.ts:510: * The leading domain-separation tag (`wellinformed-auth:v1:`) prevents
./src/domain/identity.ts:520:  const s = `wellinformed-auth:v1:${deviceId}:${hex}:${authorizedAt}`;
./src/domain/identity.ts:526: *   `wellinformed-sig:v1:{device_id}:{signed_at}:{canonical_json(payload)}`
./src/domain/identity.ts:536:    const s = `wellinformed-sig:v1:${deviceId}:${signedAt}:${json}`;
./src/domain/internal-schemes.ts:3: * non-http/https URI prefixes wellinformed recognises as legitimate
./src/domain/internal-schemes.ts:44:  // User-saved typed notes via `wellinformed save --type ...`. These
./src/domain/errors.ts:2: * Domain errors for the wellinformed knowledge graph.
./src/domain/errors.ts:272: *   - SessionStateFileError  — ~/.wellinformed/sessions-state.json read/write/parse failure
./src/domain/errors.ts:568:        ? 'fix: run `wellinformed trigger` to populate the graph (this is normal on first run).'
./src/domain/errors.ts:569:        : 'fix: run `wellinformed doctor --fix` and check the file is readable.';
./src/domain/errors.ts:571:      return `fix: graph file is corrupted at ${e.path}. Restore from backup or move it aside and run \`wellinformed trigger\` to rebuild.`;
./src/domain/errors.ts:573:      return 'fix: check disk space and that no other wellinformed process is holding the write lock (`wellinformed doctor`).';
./src/domain/errors.ts:577:      return 'fix: run `wellinformed doctor --fix` to reset the sqlite-vec store, or check the file permissions on `~/.wellinformed/vectors.db`.';
...
./src/domain/errors.ts:596:      return 'fix: re-check the multiaddr; for diagnostics run `wellinformed peer list`.';
./src/domain/errors.ts:600:      return 'fix: run `wellinformed identity init` to (re)create the peer identity, or `wellinformed identity import <hex>` to restore.';
./src/domain/errors.ts:607:      return `fix: the node was BLOCKED before reaching the network — your secret is safe locally. Either remove the credential from the source content, or move it to a non-shared room. Inspect the node with \`wellinformed get-node ${e.nodeId}\`.`;
./src/domain/errors.ts:610:      return `fix: review flagged nodes with \`wellinformed lint --room ${e.room}\` and either remove the secrets or unshare the room.`;
...
./src/domain/errors.ts:617:      return 'fix: the identity chain failed to verify. If this is your own identity, `wellinformed identity rotate` regenerates the device key under your existing DID.';
./src/domain/errors.ts:630: *   ask: graph read error at ~/.wellinformed/graph.json: ENOENT
./src/domain/errors.ts:631: *     → fix: run `wellinformed trigger` to populate the graph (this is normal on first run).
./src/cli/index.ts:3: * wellinformed CLI — subcommand router.
./src/cli/index.ts:114:  // Plural-form alias: `wellinformed peers rep …` works as well as
./src/cli/index.ts:115:  // `wellinformed peer rep …`. The subcommand dispatcher handles both.
./src/cli/index.ts:120:    console.error('  usage: wellinformed peers rep [<peer-id>] [--subject <key>] [--json]');
./src/cli/index.ts:138:    console.error(`wellinformed: '${cmd}' is recognized but not yet implemented (Phase 0 scaffold).`);
./src/cli/index.ts:142:  console.error(`wellinformed: unknown command '${cmd}'. run 'wellinformed help'.`);
./src/cli/index.ts:149:    console.error('wellinformed: fatal error');
./src/domain/oracle.ts:96:    source_file: 'wellinformed:oracle',
./src/domain/oracle.ts:118:    source_file: 'wellinformed:oracle',
./src/cli/tui/share-picker-tty.ts:66:  const header = `${BOLD}wellinformed share${RESET} — toggle which physical rooms are open to peers`;
./src/cli/tui/share-picker-tty.ts:76:    : `${DIM}  (no physical rooms yet — run \`wellinformed trigger\` first)${RESET}`;
./src/cli/tui/share-picker-tty.ts:100: * `wellinformed share room <name>` command instead.
./src/cli/tui/share-picker-tty.ts:106:    throw new Error('share ui: requires a TTY. Use `wellinformed share room <name>` in scripts.');
./scripts/qrel-rejudge-v3.mjs:33:const CACHE_DIR = join(homedir(), '.wellinformed', 'bench');
./scripts/qrel-rejudge-v3.mjs:49:console.log(' wellinformed — SciFact qrel rejudge V2 (few-shot + CoT)');
./src/domain/touch.ts:32:export const TOUCH_PROTOCOL_ID = '/wellinformed/touch/1.0.0' as const;
./src/infrastructure/sessions-state.ts:3: * ~/.wellinformed/sessions-state.json.
./src/infrastructure/sessions-state.ts:209: * wellinformed processes (e.g., daemon + CLI) from racing and clobbering
./src/infrastructure/ydoc-store.ts:2: * Y.Doc binary persistence — `~/.wellinformed/ydocs/<room>.ydoc` files.
./bin/wellinformed.js:3: * wellinformed CLI entry (thin shim + optional IPC delegation).
./bin/wellinformed.js:70:const wellinformedHome = () =>
./bin/wellinformed.js:71:  process.env.WELLINFORMED_HOME ?? join(homedir(), '.wellinformed');
./bin/wellinformed.js:75:    const sockPath = join(wellinformedHome(), 'daemon.sock');
./bin/wellinformed.js:130:  console.error('wellinformed: no build output and no source found.');
./src/mcp/server.ts:2: * wellinformed MCP server — exposes the knowledge graph to Claude Code.
./src/mcp/server.ts:4: * Spawned by `wellinformed mcp start` or by Claude Code itself via
./src/mcp/server.ts:62:import { wellinformedHome } from '../cli/runtime.js';
./src/mcp/server.ts:72:    { name: 'wellinformed', version: '0.0.1' },
./src/mcp/server.ts:88:        'Semantic search over the wellinformed knowledge graph. Returns the top-k matches ordered by distance. Optionally filter by room.',
./src/mcp/server.ts:109:  // to ~/.wellinformed/prefetch-cache.jsonl, keyed by exact prompt
./src/mcp/server.ts:129:  const homeFromRuntime = (_rt: Runtime): string => wellinformedHome();
./src/mcp/server.ts:166:      // answer to ~/.wellinformed/prefetch-cache.jsonl. When the
./src/mcp/server.ts:205:            text: `# wellinformed context for: ${query}\n\n${blocks.join('\n\n---\n\n')}`,
./src/mcp/server.ts:321:      const identityPath = join(wellinformedHome(), 'peer-identity.json');
./src/mcp/server.ts:322:      const peersPath = join(wellinformedHome(), 'peers.json');
./src/mcp/server.ts:323:      const configPath = join(wellinformedHome(), 'config.yaml');
./src/mcp/server.ts:369:        // "wellinformed actually went to the network and here's what
./src/mcp/server.ts:444:        'in that case the agent should fall back to `ask` or suggest `wellinformed entity add <name>`.',
./src/mcp/server.ts:454:      const registry = fileEntityRegistry(join(wellinformedHome(), 'entities.json'));
./src/mcp/server.ts:467:            hint: `No entity registered for "${name}". Register one with \`wellinformed entity add "${name}"\`, or run an ingest — heuristic detection picks up CamelCase identifiers and URL hosts automatically.`,
./src/mcp/server.ts:492:  // Cross-peer entity recall via the /wellinformed/recall/1.0.0
./src/mcp/server.ts:514:      const cfgRes = await loadConfig(join(wellinformedHome(), 'config.yaml'));
./src/mcp/server.ts:516:      const idRes = await loadOrCreateIdentity(join(wellinformedHome(), 'peer-identity.json'));
./src/mcp/server.ts:523:        peersPath: join(wellinformedHome(), 'peers.json'),
./src/mcp/server.ts:528:        const peersRes = await loadPeers(join(wellinformedHome(), 'peers.json'));
./src/mcp/server.ts:731:  // Phase 19 — 15th MCP tool. Queries ~/.wellinformed/code-graph.db independently
...
./src/mcp/server.ts:856:        'broader federation of wellinformed peers to help answer a question ' +
./src/mcp/server.ts:868:        join(wellinformedHome(), 'peer-identity.json'),
./src/mcp/server.ts:915:        join(wellinformedHome(), 'peer-identity.json'),
./src/mcp/server.ts:1025:      const selfRes = await loadOrCreateIdentity(join(wellinformedHome(), 'peer-identity.json'));
./src/infrastructure/touch-protocol.ts:2: * Touch protocol — `/wellinformed/touch/1.0.0`.
./src/infrastructure/touch-protocol.ts:193:    // name (e.g. `wellinformed share room research`), include nodes
./src/cli/commands/save.ts:2: * `wellinformed save --room R [--type T] --label X [--text Y]`
./src/cli/commands/save.ts:11: *   wellinformed save --room project --type concept --label "Touch primitive" \
./src/cli/commands/save.ts:14: *   echo "long body..." | wellinformed save --room project --type synthesis --label "..."
./scripts/qrel-rejudge-v2.mjs:33:const CACHE_DIR = join(homedir(), '.wellinformed', 'bench');
./scripts/qrel-rejudge-v2.mjs:49:console.log(' wellinformed — SciFact qrel rejudge V2 (few-shot + CoT)');
./src/cli/commands/update.ts:2: * `wellinformed update <sub>` — auto-update CLI surface.
./src/cli/commands/update.ts:16: *      the install (npm update -g wellinformed, brew upgrade, etc.) — the
./src/cli/commands/update.ts:27:import { wellinformedHome } from '../runtime.js';
./src/cli/commands/update.ts:50:    console.error('  example: wellinformed update configure \\');
./src/cli/commands/update.ts:52:    console.error('             --url https://releases.wellinformed.dev/latest.json');
./src/cli/commands/update.ts:65:  const r = await configureUpdates(wellinformedHome(), {
./src/cli/commands/update.ts:80:  console.log(`  auto-check:          disabled (run 'wellinformed update enable-auto' to turn on)`);
./src/cli/commands/update.ts:88:  const r = await checkForUpdate(wellinformedHome(), v);
./src/cli/commands/update.ts:113:  console.log(`  npm update -g wellinformed   # if installed via npm`);
./src/cli/commands/update.ts:121:  const cfg = await loadUpdateConfig(wellinformedHome());
./src/cli/commands/update.ts:127:    console.log('not configured (run `wellinformed update configure --did ... --url ...`)');
./src/cli/commands/update.ts:130:  const state = await loadUpdateState(wellinformedHome());
./src/cli/commands/update.ts:147:  const cfg = await loadUpdateConfig(wellinformedHome());
./src/cli/commands/update.ts:150:    console.error('not configured. run: wellinformed update configure --did ... --url ...');
./src/cli/commands/update.ts:153:  const r = await configureUpdates(wellinformedHome(), { ...cfg.value, auto_check_enabled: enabled });
./src/cli/commands/update.ts:160:  console.log('usage: wellinformed update <sub>');
./src/infrastructure/x-client.ts:6: * ~/.wellinformed/x-token.json, refreshes automatically.
./src/infrastructure/x-client.ts:63:  console.log('\nOpen this URL in your browser to authorize wellinformed:\n');
./src/infrastructure/x-client.ts:103:        res.end('<h2>wellinformed authorized. You can close this tab.</h2>');
./Dockerfile:19:ENTRYPOINT ["node", "bin/wellinformed.js"]
./NEXT_STEPS.md:1:# wellinformed Next Steps
./NEXT_STEPS.md:3:This is the execution README for getting wellinformed closer to SOTA as a
./NEXT_STEPS.md:14:wellinformed should make this demo feel inevitable:
./NEXT_STEPS.md:18:3. wellinformed retrieves the trusted peer memory before web search.
./NEXT_STEPS.md:85:- `wellinformed-rs/src/bin/wellinformed_cli.rs`
./NEXT_STEPS.md:86:- `bin/wellinformed.js`
./NEXT_STEPS.md:160:wellinformed changes agent behavior.
./NEXT_STEPS.md:229:wellinformed is ready to claim category leadership when:
./scripts/bench-matryoshka.mjs:64:const CACHE_ROOT = join(homedir(), '.wellinformed', 'bench');
./scripts/bench-matryoshka.mjs:81:console.log(` wellinformed — Matryoshka truncation gate`);
./src/cli/commands/swarm.ts:2: * `wellinformed swarm` — Phase 3 of the P2P scale plan.
./src/cli/commands/swarm.ts:7: *     Generates ~/.wellinformed/swarm-corpus.jsonl with N synthetic
./src/cli/commands/swarm.ts:35:import { defaultRuntime, wellinformedHome } from '../runtime.js';
./src/cli/commands/swarm.ts:229:  let seed = 'wellinformed-swarm-default';
./src/cli/commands/swarm.ts:246:  const home = wellinformedHome();
./src/cli/commands/swarm.ts:311:  console.log(`Next: \`wellinformed swarm sim\` to start the responder.`);
./src/cli/commands/swarm.ts:328:  const home = wellinformedHome();
./src/cli/commands/swarm.ts:332:    console.error(`  run \`wellinformed swarm gen --count 100\` first.`);
./src/cli/commands/swarm.ts:351:  console.log(`          wellinformed daemon start`);
./src/cli/commands/swarm.ts:365:    console.log('usage: wellinformed swarm <gen|sim> [flags]');
./src/cli/commands/swarm.ts:369:    console.log('    ~/.wellinformed/swarm-corpus.jsonl. Adversarial peers');
./src/cli/commands/swarm.ts:374:    console.log('    synthetic gossip responses on /wellinformed/search-resp');
./src/infrastructure/rust-retrieval.ts:3: * wellinformed-rs `embed_server` binary for the non-embedder ops:
./src/infrastructure/rust-retrieval.ts:66: * Tunnel in wellinformed's domain shape — a semantic bridge between
./src/infrastructure/rust-retrieval.ts:119:   * `wellinformed-rs/target/release/embed_server`; override via
./src/infrastructure/rust-retrieval.ts:131:  return join(here, '..', '..', 'wellinformed-rs', 'target', 'release', 'embed_server');
./scripts/bench-ppr.mjs:38:const CACHE_ROOT = join(homedir(), '.wellinformed', 'bench');
./scripts/contextualize-corpus.mjs:10:// at ~/.wellinformed/bench/<output_dataset>/<output_dataset>/ along
./scripts/contextualize-corpus.mjs:40:const CACHE_ROOT = join(homedir(), '.wellinformed', 'bench');
./scripts/contextualize-corpus.mjs:220:console.log(`  WELLINFORMED_RUST_BIN=$(pwd)/wellinformed-rs/target/release/embed_server \\`);
./CLAUDE.md:248:<!-- wellinformed:start -->
./CLAUDE.md:250:# wellinformed
./CLAUDE.md:251:wellinformed is a knowledge-graph-first research layer with P2P
./CLAUDE.md:266:  wellinformed — zero overhead, zero noise.
./CLAUDE.md:282:Two canonical rooms every wellinformed peer advertises out of the box:
./CLAUDE.md:291:`wellinformed save`: a URL-sourced save lands in `research`
./CLAUDE.md:306:- If the hit is older, prefer a fresh pull — `mcp__wellinformed__trigger_room`
./CLAUDE.md:311:## When to invoke wellinformed
./CLAUDE.md:320:   already indexed?" question, call the wellinformed MCP tools
./CLAUDE.md:331:   the statusline panel and the `wellinformed metrics bypass` audit,
./CLAUDE.md:345:6. **Save synthesized insights with `wellinformed save --type
./CLAUDE.md:356:   a refresh via `mcp__wellinformed__trigger_room` then retry.
./CLAUDE.md:358:<!-- wellinformed:end -->
./scripts/sweep-rocchio.mjs:36:const CACHE_DIR = join(homedir(), '.wellinformed', 'bench');
./scripts/sweep-rocchio.mjs:50:console.log(' wellinformed — Rocchio dense PRF on SciFact');
./src/cli/commands/onboard.ts:2: * `wellinformed onboard` — first-run installer + onboarding wizard.
./src/cli/commands/onboard.ts:18: * indexing is the user's intent, exposed as `wellinformed this`.
./src/cli/commands/onboard.ts:147:  const def = flags.home ?? process.env.WELLINFORMED_HOME ?? join(homedir(), '.wellinformed');
./src/cli/commands/onboard.ts:160:  if (chosen !== join(homedir(), '.wellinformed')) {
./src/cli/commands/onboard.ts:178:    sp.stop("runtime check reported issues — run 'wellinformed doctor --fix'");
./src/cli/commands/onboard.ts:215:        '  3. wellinformed login',
./src/cli/commands/onboard.ts:231:    log.message('skipped — run `wellinformed login` when convenient');
./src/cli/commands/onboard.ts:236:  // logic is identical to `wellinformed login`. One canonical path
./src/cli/commands/onboard.ts:242:    log.warn('login failed — `wellinformed login` to retry');
./src/cli/commands/onboard.ts:299:    "Skip if unsure; you can always run 'wellinformed trigger --room sessions' later.",
./src/cli/commands/onboard.ts:312:    log.message('skipped — run `wellinformed trigger --room sessions` when convenient');
...
./src/cli/commands/onboard.ts:453:    lines.push('  wellinformed peer add /ip4/<host>/tcp/<port>/p2p/<id>');
./src/cli/commands/onboard.ts:460:const USAGE = `usage: wellinformed onboard [--yes] [--home DIR] [--no-sessions]
./src/cli/commands/onboard.ts:467:  daemon, and prints what wellinformed will do on every session.
./src/cli/commands/onboard.ts:470:  run 'wellinformed this me' (private) or 'wellinformed this everyone'
./src/cli/commands/onboard.ts:481:  intro('wellinformed onboard');
./src/cli/commands/onboard.ts:528:      '  wellinformed this              index the current folder, keep it private',
./src/cli/commands/onboard.ts:529:      '  wellinformed this everyone     index + share with the P2P network',
./src/cli/commands/onboard.ts:530:      '  wellinformed ask "..."         semantic search across your graph',
./src/cli/commands/onboard.ts:531:      '  wellinformed trigger           refresh all rooms',
./src/cli/commands/onboard.ts:532:      '  wellinformed peer list         see who you talk to',
./src/cli/commands/onboard.ts:533:      '  wellinformed doctor            health check',
./src/cli/commands/onboard.ts:539:      "  · Stop the network at any time: wellinformed daemon stop",
./src/infrastructure/config-loader.ts:2: * Config loader — reads ~/.wellinformed/config.yaml with typed defaults.
./config/config.example.yaml:1:# wellinformed — example configuration.
./config/config.example.yaml:2:# Copy to ~/.wellinformed/config.yaml and fill in secrets.
./config/config.example.yaml:14:  cache_dir: ~/.wellinformed/models
./config/config.example.yaml:18:  path: ~/.wellinformed/graph
./config/config.example.yaml:19:  python_venv: ~/.wellinformed/.venv
./config/config.example.yaml:24:  path: ~/.wellinformed/vectors.db
./src/cli/commands/sources.ts:2: * `wellinformed sources <sub>` — manage the ~/.wellinformed/sources.json
./src/cli/commands/sources.ts:14: *   wellinformed sources add hn-embeddings \
./src/cli/commands/sources.ts:37:  if (args.length === 0) return 'missing <id> — usage: wellinformed sources add <id> --kind K --room R --config {json}';
./src/cli/commands/sources.ts:85:    console.log('no sources configured. try `wellinformed sources add` to create one.');
./src/cli/commands/sessions.ts:2: * `wellinformed sessions <sub>` — Claude session ingestion lifecycle.
./src/cli/commands/sessions.ts:14: *   2. Delete ~/.wellinformed/sessions-state.json
./src/cli/commands/sessions.ts:27:import { wellinformedHome } from '../runtime.js';
./src/cli/commands/sessions.ts:29:const sessionsStatePath = (): string => join(wellinformedHome(), 'sessions-state.json');
./src/cli/commands/sessions.ts:37:    console.log('  next run of `wellinformed trigger --room sessions` will re-ingest from offset 0.');
./src/cli/commands/sessions.ts:45:    console.error(`  'wellinformed trigger --room sessions' (can re-create thousands of nodes).`);
./src/cli/commands/sessions.ts:51:  const lockRes = await acquireLock(wellinformedHome(), {
./src/cli/commands/sessions.ts:72:    console.log('  wellinformed trigger --room sessions');
./src/cli/commands/sessions.ts:115:  console.log('usage: wellinformed sessions <sub>');
./src/cli/commands/sessions.ts:123:  console.log('~/.claude/projects/**/*.jsonl are never touched by wellinformed — a fresh');
./src/infrastructure/peer-store.ts:2: * Peer store — persists known peers to ~/.wellinformed/peers.json.
./src/infrastructure/peer-store.ts:26: * concurrent wellinformed processes cannot clobber each other's peer list.
./src/infrastructure/peer-store.ts:111:   *   - 'manual' : `wellinformed peer add <multiaddr>`
./src/infrastructure/peer-store.ts:259: * and releases the lock. Prevents two wellinformed processes (e.g., daemon +
./scripts/qrel-rejudge.mjs:49:const CACHE_DIR = join(homedir(), '.wellinformed', 'bench');
./scripts/qrel-rejudge.mjs:65:console.log(' wellinformed — SciFact qrel completeness audit (Round 2)');
./src/cli/commands/logs.ts:2: * `wellinformed logs <sub>` — local-first network telemetry surface.
./src/cli/commands/logs.ts:11: * Logs live under ~/.wellinformed/logs/. Shipping is opt-in; default is
./src/cli/commands/logs.ts:29:import { wellinformedHome } from '../runtime.js';
./src/cli/commands/logs.ts:37:  const r = await tailToday(logPaths(wellinformedHome()), n);
./src/cli/commands/logs.ts:53:    console.error('logs export: missing <path>. usage: wellinformed logs export ./welly-debug.ndjson.gz');
./src/cli/commands/logs.ts:56:  const r = await exportBundle(logPaths(wellinformedHome()));
./src/cli/commands/logs.ts:71:    console.error('logs enable-shipping: missing <url>. usage: wellinformed logs enable-shipping https://logs.example.com/ingest');
./src/cli/commands/logs.ts:78:  const r = await enableShipping(logPaths(wellinformedHome()), url);
./src/cli/commands/logs.ts:85:  console.log('  disable any time:  wellinformed logs disable-shipping');
./src/cli/commands/logs.ts:90:  const r = await disableShipping(logPaths(wellinformedHome()));
./src/cli/commands/logs.ts:100:  const r = await getShippingStatus(logPaths(wellinformedHome()));
./src/cli/commands/logs.ts:117:  const r = await rotate(logPaths(wellinformedHome()));
./src/cli/commands/logs.ts:127:  console.log('usage: wellinformed logs <sub>');
./src/cli/commands/logs.ts:136:  console.log('Logs are local-first (~/.wellinformed/logs/). Shipping is opt-in; nothing');
./src/cli/commands/metrics.ts:2: * `wellinformed metrics` — emit the daemon's live metrics snapshot.
./src/cli/commands/metrics.ts:7: * shim in `bin/wellinformed.js` intercepts this command BEFORE we
./src/cli/commands/metrics.ts:11: *   - the user runs `wellinformed metrics` with no daemon running, OR
./src/cli/commands/metrics.ts:23:const wellinformedHome = (): string =>
./src/cli/commands/metrics.ts:24:  process.env.WELLINFORMED_HOME ?? join(homedir(), '.wellinformed');
./src/cli/commands/metrics.ts:60: * `wellinformed metrics bypass [--json] [--since <iso>]`
./src/cli/commands/metrics.ts:82:  const home = wellinformedHome();
./src/cli/commands/metrics.ts:114:  console.log(`wellinformed metrics bypass (window: ${out.window_since})`);
./src/cli/commands/metrics.ts:141:  const sock = join(wellinformedHome(), 'daemon.sock');
./src/cli/commands/metrics.ts:144:    : 'metrics live in the daemon process. start `wellinformed daemon` to populate.';
./wellinformed-rs/src/main.rs:1://! wellinformed-bench — BEIR benchmark runner (CLI entry).
./wellinformed-rs/src/main.rs:28:use wellinformed_bench::application::{run_benchmark, BenchmarkConfig, BenchmarkReport};
./wellinformed-rs/src/main.rs:29:use wellinformed_bench::domain::{beir::load_beir, BeirDataset, EncoderSpec};
./wellinformed-rs/src/main.rs:30:use wellinformed_bench::infrastructure::{
./wellinformed-rs/src/main.rs:64:        " wellinformed-bench (Rust/functional/DDD) — BEIR {}",
./wellinformed-rs/src/main.rs:145:    let bench_root = PathBuf::from(&home).join(".wellinformed/bench");
./src/cli/commands/mcp.ts:2: * `wellinformed mcp start` — start the MCP stdio server.
./src/cli/commands/discover-loop.ts:2: * `wellinformed discover-loop [--room R] [--max-iterations N]`
./src/cli/commands/share.ts:2: * `wellinformed share <sub>` — sharing boundary commands.
./src/cli/commands/share.ts:18:import { runtimePaths, wellinformedHome } from '../runtime.js';
./src/cli/commands/share.ts:25:const configPath = (): string => join(wellinformedHome(), 'config.yaml');
./src/cli/commands/share.ts:26:const sharedRoomsPath = (): string => join(wellinformedHome(), 'shared-rooms.json');
./src/cli/commands/share.ts:43:      'share audit: missing --room <name>. usage: wellinformed share audit --room <name> [--json]',
./src/cli/commands/share.ts:120:    console.error('share room: missing <name>. usage: wellinformed share room <name>');
./src/cli/commands/share.ts:175:    console.error(`\nrun 'wellinformed share audit --room ${roomId}' for full details.`);
./src/cli/commands/share.ts:202:  const ydocPath = join(wellinformedHome(), 'ydocs', `${roomId}.ydoc`);
./src/cli/commands/share.ts:203:  const logPath = join(wellinformedHome(), 'share-log.jsonl');
./src/cli/commands/share.ts:229:  console.log("  run 'wellinformed daemon start' (or restart it) so peers can sync this room");
./src/cli/commands/share.ts:251:    console.log('share ui: no physical rooms yet. Run `wellinformed trigger` to index some content first.');
./src/cli/commands/share.ts:292:      console.error(`  ${b.room}: ${b.count} flagged node(s). Run \`wellinformed share audit --room ${b.room}\` to inspect.`);
./src/cli/commands/share.ts:309:const USAGE = `usage: wellinformed share <audit|room|ui>
./scripts/sweep-rrf.mjs:9: * Data: ~/.wellinformed/bench/scifact__rust-via-ts__bge-base/vectors.db
./scripts/sweep-rrf.mjs:36:const CACHE_DIR = join(homedir(), '.wellinformed', 'bench');
./scripts/sweep-rrf.mjs:52:console.log(' wellinformed — RRF k + α sweep on cached SciFact');
./scripts/bootstrap.sh:3:# wellinformed bootstrap — sets up the per-user runtime dir, creates a
./scripts/bootstrap.sh:24:WELL_DIR="${WELLINFORMED_HOME:-$HOME/.wellinformed}"
./scripts/bootstrap.sh:28:log()  { printf '[wellinformed] %s\n' "$*"; }
./scripts/bootstrap.sh:29:warn() { printf '[wellinformed] WARN %s\n' "$*" >&2; }
./scripts/bootstrap.sh:30:die()  { printf '[wellinformed] FATAL %s\n' "$*" >&2; exit "${2:-1}"; }
./scripts/bootstrap.sh:68:# targets ~/.wellinformed/venv which is per-user cache.
./scripts/bootstrap.sh:118:print(f"[wellinformed] state written to {state_file}")
./scripts/bootstrap.sh:121:log "bootstrap OK — run 'wellinformed doctor' to verify the full stack"
./scripts/bench-arguana-dense.mjs:37:const CACHE_DIR = join(homedir(), '.wellinformed', 'bench');
./scripts/bench-arguana-dense.mjs:46:console.log(' wellinformed — ArguAna dense-only re-target gate');
./wellinformed-rs/src/lib.rs:1://! wellinformed-bench library — shared modules for the `bench_beir`
./wellinformed-rs/src/lib.rs:3://! embedding server consumed by the TypeScript wellinformed stack).
./src/infrastructure/sources/claude-sessions.ts:53:  /** ~/.wellinformed directory — state file is written here. */
./src/cli/commands/touch.ts:2: * `wellinformed touch <peer-id-or-multiaddr> --room <name> [--max N] [--dry-run]`
./src/cli/commands/touch.ts:17:import { wellinformedHome } from '../runtime.js';
./src/cli/commands/touch.ts:32:const identityPath = (): string => join(wellinformedHome(), 'peer-identity.json');
./src/cli/commands/touch.ts:33:const peersPath = (): string => join(wellinformedHome(), 'peers.json');
./src/cli/commands/touch.ts:34:const configPath = (): string => join(wellinformedHome(), 'config.yaml');
./src/cli/commands/touch.ts:35:const graphPath = (): string => join(wellinformedHome(), 'graph.json');
./src/cli/commands/touch.ts:46:    return 'touch: missing <peer-id-or-multiaddr>. usage: wellinformed touch <peer> --room <name> [--max N] [--dry-run]';
./src/cli/commands/touch.ts:82:  if (!rec || rec.addrs.length === 0) return `touch: peer '${target}' not found in peers.json (add via 'wellinformed peer add <multiaddr>' first)`;
./src/cli/commands/recent-sessions.ts:2: * `wellinformed recent-sessions` — Phase 20 CLI surface.
./src/cli/commands/recent-sessions.ts:4: * Queries ~/.wellinformed/graph.json for nodes in the `sessions` room,
./src/cli/commands/recent-sessions.ts:204:      'no recent sessions indexed. run `wellinformed daemon start` so the claude-sessions adapter can populate the graph.',
./src/cli/commands/index-project.ts:2: * `wellinformed index [--room R] [--root DIR]`
./src/cli/commands/consolidate.ts:2: * `wellinformed consolidate <sub>` — Phase 4c CLI surface for the
./src/cli/commands/consolidate.ts:21: * The CLI enforces `wellinformed daemon start` is NOT running with
./src/cli/commands/consolidate.ts:40:import { wellinformedHome } from '../runtime.js';
./src/cli/commands/consolidate.ts:68:  // entries go to an NDJSON file that `wellinformed sessions reingest`
./src/cli/commands/consolidate.ts:86:  if (!room) return 'missing <room>. usage: wellinformed consolidate run <room> [--dry-run] [--prune [--backup PATH | --no-backup]]';
./src/cli/commands/consolidate.ts:215:  const lockRes = await acquireLock(wellinformedHome(), {
./src/cli/commands/consolidate.ts:223:    console.error(`  retry, or run 'wellinformed daemon stop' to free the lock.`);
./src/cli/commands/consolidate.ts:283:      // graph nodes to an NDJSON file so `wellinformed sessions reingest`
./src/cli/commands/consolidate.ts:287:          ?? join(wellinformedHome(), `prune-backup-${parsed.room}-${Date.now()}.ndjson`);
./src/cli/commands/consolidate.ts:433:  console.log('usage: wellinformed consolidate <sub>');
./src/cli/commands/consolidate.ts:451:  console.log('Run consolidation while `wellinformed daemon` is stopped — v4.0 has no');
./src/cli/commands/consolidate.ts:481:    console.error('consolidate prune-marked: missing <room>. usage: wellinformed consolidate prune-marked <room> [--no-backup | --backup PATH] [--force]');
./src/cli/commands/consolidate.ts:485:  const lockRes = await acquireLock(wellinformedHome(), { owner: 'consolidate-prune', waitMs: 30_000, pollIntervalMs: 250 });
./src/cli/commands/consolidate.ts:516:      const path = backupPath ?? join(wellinformedHome(), `prune-marked-backup-${room}-${Date.now()}.ndjson`);
./scripts/bench-beir-sota.mjs:58:const CACHE_ROOT = join(homedir(), '.wellinformed', 'bench');
./scripts/bench-beir-sota.mjs:71:console.log(` wellinformed — BEIR ${DATASET.toUpperCase()} SOTA Benchmark`);
./src/cli/commands/entity.ts:2: * `wellinformed entity <sub>` — manage the entity registry.
./src/cli/commands/entity.ts:22:const USAGE = `usage: wellinformed entity <sub>
./src/cli/commands/entity.ts:83:  console.log(`run \`wellinformed recall ${label}\` after ingest to see hits.`);
./src/cli/commands/entity.ts:106:    console.log('add one with: wellinformed entity add <label> [--alias A] [--type T]');
./src/cli/commands/entity.ts:111:    console.log('add one with: wellinformed entity add <label> [--alias A] [--type T]');
./src/cli/commands/gc.ts:2: * `wellinformed gc` — long-term-memory garbage collection.
./src/cli/commands/gc.ts:111:    console.error(`  see: wellinformed gc help`);
./src/cli/commands/gc.ts:142:  console.log(`wellinformed gc — long-term memory garbage collection
./src/cli/commands/gc.ts:184:      console.error(`  see: wellinformed gc help`);
./src/cli/commands/eval.ts:2: * `wellinformed eval <queries.jsonl> [--room R] [--k 10] [--json]`
./src/cli/commands/eval.ts:66:    return 'missing queries file — usage: wellinformed eval <queries.jsonl> [--room R] [--k 10] [--limit N] [--json]';
./scripts/jacobi-preconditioning.mjs:36:const CACHE_DIR = join(homedir(), '.wellinformed', 'bench');
./scripts/jacobi-preconditioning.mjs:49:console.log(' wellinformed — Diagonal Jacobi preconditioning (CFD #1)');
./src/cli/commands/oracle.ts:2: * `wellinformed oracle <sub>` — peer-to-peer Q&A via the oracle system room.
./src/cli/commands/oracle.ts:35:import { wellinformedHome } from '../runtime.js';
./src/cli/commands/oracle.ts:61:  const homePath = wellinformedHome();
...
./src/cli/commands/oracle.ts:441:const USAGE = `usage: wellinformed oracle <ask|answer|list|show|answerable>
./src/cli/commands/oracle.ts:456:surface. Run \`wellinformed daemon start\` (or ensure peers are connected)
./src/cli/commands/cache-stats.ts:2: * `wellinformed cache-stats` — print L1 query cache observability.
./src/cli/commands/cache-stats.ts:15:const wellinformedHome = (): string =>
./src/cli/commands/cache-stats.ts:16:  process.env.WELLINFORMED_HOME ?? join(homedir(), '.wellinformed');
./src/cli/commands/cache-stats.ts:19:  // The actual IPC call is intercepted in bin/wellinformed.js when the
./src/cli/commands/cache-stats.ts:22:  const sock = join(wellinformedHome(), 'daemon.sock');
./src/cli/commands/cache-stats.ts:31:      note: 'L1 cache lives in the daemon process. Start `wellinformed daemon` to populate.',
./src/cli/commands/login.ts:2: * `wellinformed login` — link a verified GitHub identity to your
./src/cli/commands/login.ts:6: * the social DID for wellinformed is anchored to existing OAuth
./src/cli/commands/login.ts:8: * GitHub user id + profile URL go into `~/.wellinformed/linked-
./src/cli/commands/login.ts:34:import { wellinformedHome } from '../runtime.js';
./src/cli/commands/login.ts:49:      return 'verification code expired before you completed the flow. re-run `wellinformed login github`.';
./src/cli/commands/login.ts:135:  const persisted = saveLinkedAccount(wellinformedHome(), 'github', {
./src/cli/commands/login.ts:150:  console.log(`  recorded:   ~/.wellinformed/linked-accounts.json`);
./src/cli/commands/login.ts:158:const USAGE = `usage: wellinformed login
./src/cli/commands/login.ts:171:    3. Re-run: wellinformed login
./src/cli/commands/login.ts:181:  // Permit `wellinformed login github` as an explicit alias, but
./src/cli/commands/login.ts:182:  // bare `wellinformed login` is the canonical form.
./src/cli/commands/login.ts:184:    console.error(`login: unknown argument '${args[0]}'. usage: wellinformed login`);
./src/cli/commands/recall.ts:2: * `wellinformed recall <name> [--room R] [--k N] [--json]`
./src/cli/commands/recall.ts:19:import { defaultRuntime, wellinformedHome } from '../runtime.js';
./src/cli/commands/recall.ts:50:  if (!query) return 'missing name — usage: wellinformed recall <name> [--room R] [--k N] [--peers] [--json]';
./src/cli/commands/recall.ts:56:    console.log(`usage: wellinformed recall <name> [--room R] [--k N] [--peers] [--json]
./src/cli/commands/recall.ts:59:(\`wellinformed entity add ...\`) plus heuristic auto-detected
./src/cli/commands/recall.ts:67:              /wellinformed/recall/1.0.0 protocol — returns
./src/cli/commands/recall.ts:107:  const idPath = join(wellinformedHome(), 'peer-identity.json');
./src/cli/commands/recall.ts:108:  const peersPath = join(wellinformedHome(), 'peers.json');
./src/cli/commands/recall.ts:109:  const cfgPath = join(wellinformedHome(), 'config.yaml');
./src/cli/commands/recall.ts:166:    console.log(`# wellinformed recall --peers: ${parsed.query}`);
./src/cli/commands/recall.ts:182:        console.log('  (no connected peers — dial one with `wellinformed peer add <multiaddr>`)');
./src/cli/commands/recall.ts:222:      console.log(`  register one with: wellinformed entity add "${parsed.query}"`);
./src/cli/commands/recall.ts:251:  console.log(`# wellinformed recall: ${entity.label}`);
./src/cli/commands/recall.ts:261:    console.log('or no ingest has run since registration. try `wellinformed trigger`.');
./src/cli/commands/init.ts:2: * `wellinformed init` — interactive room seeding wizard.
./src/cli/commands/init.ts:17: * Non-interactive mode: `wellinformed init --name X --desc Y --keywords a,b`
./src/cli/commands/init.ts:117:  console.log('\nwellinformed init — set up a new research room\n');
./src/cli/commands/init.ts:230:    console.log(`run 'wellinformed trigger --room ${result.room.id}' to fetch initial content.`);
./src/cli/commands/telegram.ts:2: * `wellinformed telegram <sub>`
./src/cli/commands/telegram.ts:18:wellinformed telegram setup
./src/cli/commands/telegram.ts:58:  console.log('Run `wellinformed telegram test` to verify.');
./src/cli/commands/telegram.ts:66:    console.error('No config.yaml found. Run `wellinformed telegram setup` first.');
./src/cli/commands/telegram.ts:74:    console.error('Telegram not configured. Run `wellinformed telegram setup`.');
./src/cli/commands/telegram.ts:91:  const result = await bot.value.sendMessage('wellinformed bot is working. Send a URL to ingest or type a command.');
./src/cli/commands/telegram.ts:108:    console.error('No config.yaml. Run `wellinformed telegram setup`.');
./scripts/bench-v2.sh:2:# wellinformed v2.0 comprehensive benchmark
./scripts/bench-v2.sh:7:WI=wellinformed
./scripts/bench-v2.sh:48:# wellinformed v2.0 — Benchmark Report
./scripts/bench-v2.sh:56:**wellinformed:** $($WI version 2>/dev/null || echo "unknown")
./scripts/bench-v2.sh:114:bench_capture "reindex wellinformed (116 files)"       $WI codebase reindex 19a0c7525684eded
./scripts/bench-v2.sh:141:graph_size=$(wc -c < ~/.wellinformed/graph.json 2>/dev/null || echo 0)
./scripts/bench-v2.sh:142:vectors_size=$(wc -c < ~/.wellinformed/vectors.db 2>/dev/null || echo 0)
./scripts/bench-v2.sh:143:code_size=$(wc -c < ~/.wellinformed/code-graph.db 2>/dev/null || echo 0)
...
./scripts/bench-v2.sh:152:research_nodes=$(sqlite3 ~/.wellinformed/vectors.db "SELECT COUNT(*) FROM vec_meta" 2>/dev/null || echo 0)
./scripts/bench-v2.sh:153:code_nodes=$(sqlite3 ~/.wellinformed/code-graph.db "SELECT COUNT(*) FROM code_nodes")
./scripts/bench-v2.sh:154:code_edges=$(sqlite3 ~/.wellinformed/code-graph.db "SELECT COUNT(*) FROM code_edges")
./scripts/bench-v2.sh:155:codebases=$(sqlite3 ~/.wellinformed/code-graph.db "SELECT COUNT(*) FROM codebases")
./scripts/bench-v2.sh:212:| ~/.wellinformed/graph.json (research graph) | $(echo "scale=2; $graph_size / 1024 / 1024" | bc) MB |
./scripts/bench-v2.sh:213:| ~/.wellinformed/vectors.db (ONNX vectors + meta) | $(echo "scale=2; $vectors_size / 1024 / 1024" | bc) MB |
./scripts/bench-v2.sh:214:| ~/.wellinformed/code-graph.db (Phase 19 code graph) | $(echo "scale=2; $code_size / 1024 / 1024" | bc) MB |
./src/cli/commands/viz.ts:2: * `wellinformed viz [--room R] [--output FILE]`
./src/cli/commands/viz.ts:19:<html><head><meta charset="UTF-8"><title>wellinformed — ${title}</title>
./src/cli/commands/viz.ts:24:<div id="info">wellinformed — ${title}</div>
./scripts/bench-beir.mjs:3:// the canonical retrieval metrics on wellinformed's ONNX + sqlite-vec stack.
./scripts/bench-beir.mjs:37:const CACHE_ROOT = join(homedir(), '.wellinformed', 'bench');
./scripts/bench-beir.mjs:49:console.log(` wellinformed — BEIR ${DATASET.toUpperCase()} Benchmark`);
./demo/scene-federated.tape:1:# wellinformed — natural Claude Code session, federated retrieval live.
./demo/scene-federated.tape:20:Type "cd /Users/saharbarak/personal/wellinformed"
./demo/scene-federated.tape:22:Type "export WELLINFORMED_HOME=$HOME/.wellinformed.demo"
./src/cli/commands/export-obsidian.ts:2: * `wellinformed export obsidian [--room R] [--output DIR]`
./src/cli/commands/export-obsidian.ts:102:    const indexLines = [`# wellinformed vault — ${room ?? 'all rooms'}`, '', `${nodes.length} nodes, ${edges.length} edges`, ''];
./scripts/bench-beir-rust.mjs:6:// wellinformed-rs embed_server binary) → openSqliteVectorIndex.upsert
./scripts/bench-beir-rust.mjs:47:const CACHE_ROOT = join(homedir(), '.wellinformed', 'bench');
./src/cli/commands/hot.ts:2: * `wellinformed hot [--refresh] [--path]`
./wellinformed-rs/src/bin/wellinformed_cli.rs:1://! wellinformed-cli — native Rust client for the v4.1 IPC fast path.
./wellinformed-rs/src/bin/wellinformed_cli.rs:3://! v4.0 ships `bin/wellinformed.js` which checks for the daemon socket
./wellinformed-rs/src/bin/wellinformed_cli.rs:18://! command isn't IPC-delegatable), exec `node bin/wellinformed.js`
./wellinformed-rs/src/bin/wellinformed_cli.rs:53:fn wellinformed_home() -> PathBuf {
./wellinformed-rs/src/bin/wellinformed_cli.rs:58:            home.join(".wellinformed")
./wellinformed-rs/src/bin/wellinformed_cli.rs:63:    wellinformed_home().join("daemon.sock")
./wellinformed-rs/src/bin/wellinformed_cli.rs:109:    // Locate bin/wellinformed.js relative to this binary. The standard
./wellinformed-rs/src/bin/wellinformed_cli.rs:111:    //   <repo>/wellinformed-rs/target/release/wellinformed-cli
./wellinformed-rs/src/bin/wellinformed_cli.rs:112:    //   <repo>/bin/wellinformed.js
./wellinformed-rs/src/bin/wellinformed_cli.rs:113:    // ↑ four ancestors: target → release → wellinformed-rs → repo root
./wellinformed-rs/src/bin/wellinformed_cli.rs:114:    let exe = env::current_exe().unwrap_or_else(|_| PathBuf::from("./wellinformed-cli"));
./wellinformed-rs/src/bin/wellinformed_cli.rs:122:    node_entry.push("wellinformed.js");
./wellinformed-rs/src/bin/wellinformed_cli.rs:126:            "wellinformed-cli: fallback path {} does not exist",
./wellinformed-rs/src/bin/wellinformed_cli.rs:139:            eprintln!("wellinformed-cli: failed to exec node: {e}");
./src/cli/commands/ask.ts:2: * `wellinformed ask "<query>" [--room R] [--k N] [--peers]`
./src/cli/commands/ask.ts:8: * With --peers: fans out to all connected peers via /wellinformed/search/1.0.0,
./src/cli/commands/ask.ts:22:import { defaultRuntime, wellinformedHome } from '../runtime.js';
./src/cli/commands/ask.ts:53:  if (!query) return 'missing query — usage: wellinformed ask "your question" [--room R] [--k N] [--peers] [--json]';
./src/cli/commands/ask.ts:155: * "wellinformed_hook_version" — defends against zombie hooks after a
./src/cli/commands/ask.ts:173:  console.log(`# wellinformed agent contract (hook_version: ${HOOK_SCHEMA_VERSION})`);
./src/cli/commands/ask.ts:205:    console.log(`# wellinformed: "${r.query}" matches entity ${e.id}`);
./src/cli/commands/ask.ts:228:      console.log('no results found. try a broader query or run `wellinformed trigger` to index content first.');
./src/cli/commands/ask.ts:288:  const identityPath = join(wellinformedHome(), 'peer-identity.json');
./src/cli/commands/ask.ts:289:  const peersPath = join(wellinformedHome(), 'peers.json');
./src/cli/commands/ask.ts:290:  const configPath = join(wellinformedHome(), 'config.yaml');
./src/cli/commands/ask.ts:347:      home: wellinformedHome(),
./src/cli/commands/ask.ts:395:          const id = await ensureIdentity(wellinformedHome());
./src/cli/commands/ask.ts:403:            home: wellinformedHome(),
./src/cli/commands/ask.ts:452:    console.log(`# wellinformed federated results for: ${parsed.query}`);
./src/cli/commands/ask.ts:488:    // 7. Peer-pull telemetry block — visible signal of "wellinformed
./scripts/bench-ppr-multihop.mjs:63:const RESULTS_DIR = join(homedir(), '.wellinformed', 'bench', `ppr-multihop-${DATASET}`);
./scripts/bench-ppr-multihop.mjs:67:console.log(` wellinformed — PPR multi-hop rerank gate (${DATASET})`);
./src/cli/commands/help.ts:6:wellinformed — knowledge graph + research daemon Claude Code plugin
./src/cli/commands/help.ts:9:  wellinformed <command> [options]
./demo/scene-touch.tape:1:# wellinformed — scene 6 — P2P touch across a 5-daemon mesh
./demo/scene-touch.tape:26:Type "export WELLINFORMED_HOME=$HOME/.wellinformed.demo"
./demo/scene-touch.tape:38:Type 'wellinformed peer list'
./demo/scene-touch.tape:51:Type 'wellinformed ask "open-hardware portable Raman LH2 spectrometer munich" --k 3 --json | jq ".aggregate // .satisfaction.score"'
./demo/scene-touch.tape:62:Type 'wellinformed touch --peer "$WELLINFORMED_DEMO_PEER_B_ID" --room research'
./demo/scene-touch.tape:71:Type 'wellinformed touch --peer "$WELLINFORMED_DEMO_PEER_D_ID" --room research'
./demo/scene-touch.tape:82:Type 'wellinformed ask "open-hardware portable Raman LH2 spectrometer munich" --k 3'
./src/cli/commands/doctor.ts:2: * doctor — checks runtime prerequisites for wellinformed.
./src/cli/commands/doctor.ts:9: *   - ~/.wellinformed/venv exists and can `import graphify`
./src/cli/commands/doctor.ts:50:function wellinformedHome(): string {
./src/cli/commands/doctor.ts:51:  return process.env.WELLINFORMED_HOME || join(homedir(), '.wellinformed');
./src/cli/commands/doctor.ts:55:  return join(wellinformedHome(), 'venv', 'bin', 'python');
./src/cli/commands/doctor.ts:126:    name: 'wellinformed venv',
./src/cli/commands/doctor.ts:130:    fix: 'run `wellinformed doctor --fix` (or `scripts/bootstrap.sh`)',
./src/cli/commands/doctor.ts:142:      fix: 'run `wellinformed doctor --fix`',
./src/cli/commands/doctor.ts:156:      fix: 'run `wellinformed doctor --fix` to reinstall graphify into the venv',
./src/cli/commands/doctor.ts:171:      name: 'wellinformed schema patch',
./src/cli/commands/doctor.ts:180:    name: 'wellinformed schema patch',
./src/cli/commands/doctor.ts:227:  console.log('wellinformed doctor\n');
./src/cli/commands/doctor.ts:238:    console.log("run 'wellinformed doctor --fix' to bootstrap the venv + graphify install.");
./src/cli/commands/claude-install.ts:2: * `wellinformed claude install` / `wellinformed claude uninstall`
./src/cli/commands/claude-install.ts:5: * knowledge questions through the wellinformed graph automatically:
./src/cli/commands/claude-install.ts:8: *    Extracts the query from the tool input, runs `wellinformed ask
./src/cli/commands/claude-install.ts:11: *    ~/.wellinformed/miss-log.jsonl so the user can decide whether to
./src/cli/commands/claude-install.ts:38:const LEGACY_HOOK_NAME = 'wellinformed-hook.sh';
./src/cli/commands/claude-install.ts:39:const SMART_HOOK_SH = 'wellinformed-smart-hook.sh';
./src/cli/commands/claude-install.ts:40:const SMART_HOOK_CJS = 'wellinformed-smart-hook.cjs';
./src/cli/commands/claude-install.ts:41:const POST_FETCH_SH = 'wellinformed-post-fetch.sh';
./src/cli/commands/claude-install.ts:42:const POST_FETCH_CJS = 'wellinformed-post-fetch.cjs';
./src/cli/commands/claude-install.ts:61: * wellinformed package. When running from source, resolves to the repo's
./src/cli/commands/claude-install.ts:72:# wellinformed PreToolUse + SessionStart hook.
./src/cli/commands/claude-install.ts:74:GRAPH="\${WELLINFORMED_HOME:-$HOME/.wellinformed}/graph.json"
./src/cli/commands/claude-install.ts:78:  if command -v wellinformed >/dev/null 2>&1; then
./src/cli/commands/claude-install.ts:79:    RECENT=$(wellinformed recent-sessions --hours 24 --limit 1 --json 2>/dev/null || echo '{"count":0,"sessions":[]}')
./src/cli/commands/claude-install.ts:86:      MSG="wellinformed: Previous session $SID (started $STARTED, branch $BRANCH). Last assistant: \${FINAL:-<none>}. Call the recent_sessions MCP tool for the full rollup."
...
./src/cli/commands/claude-install.ts:101:// WebFetch. Extracts the query and runs `wellinformed ask --json` against
./src/cli/commands/claude-install.ts:144:# wellinformed
./src/cli/commands/claude-install.ts:145:wellinformed is a knowledge-graph-first research layer with P2P
./src/cli/commands/claude-install.ts:154:Two canonical rooms every wellinformed peer advertises out of the box:
./src/cli/commands/claude-install.ts:163:\`wellinformed save\`: a URL-sourced save lands in \`research\`
./src/cli/commands/claude-install.ts:178:- If the hit is older, prefer a fresh pull — \`mcp__wellinformed__trigger_room\`
./src/cli/commands/claude-install.ts:183:## When to invoke wellinformed
./src/cli/commands/claude-install.ts:185:1. Use the wellinformed MCP tools (\`search\`, \`ask\`, \`get_node\`,
./src/cli/commands/claude-install.ts:193:   \`wellinformed metrics bypass\` audit, not by routing through Bash.
./src/cli/commands/claude-install.ts:197:   \`wellinformed save --type synthesis --room <room>\` to file the
./src/cli/commands/claude-install.ts:202:const CLAUDE_MD_MARKER_START = '<!-- wellinformed:start -->';
./src/cli/commands/claude-install.ts:203:const CLAUDE_MD_MARKER_END = '<!-- wellinformed:end -->';
./src/cli/commands/claude-install.ts:247:  // dedupe filter — any owned script name is a wellinformed entry
./src/cli/commands/claude-install.ts:283:  console.log(`  updated ${claudeMdPath} (wellinformed section added)`);
...
./src/cli/commands/claude-install.ts:359:      console.log('wellinformed claude install\n');
./src/cli/commands/claude-install.ts:362:      console.log('wellinformed claude uninstall\n');
./scripts/bench-room-routing.mjs:4:// Tests whether wellinformed's room architecture gives a measurable retrieval lift
./scripts/bench-room-routing.mjs:11://     --datasets-dir ~/.wellinformed/bench/cqadupstack/cqadupstack \
./scripts/bench-room-routing.mjs:38:const DATASETS_DIR = getArg('--datasets-dir', join(homedir(), '.wellinformed/bench/cqadupstack/cqadupstack'));
./scripts/bench-room-routing.mjs:51:const CACHE_ROOT = join(homedir(), '.wellinformed', 'bench');
./scripts/bench-room-routing.mjs:59:console.log(' wellinformed Wave 4 — Room Routing Gate Test');
./scripts/bench-room-routing.mjs:237:// target room. This is the same pattern wellinformed's production
./src/cli/commands/identity.ts:2: * `wellinformed identity <sub>` — inspect and manage the user+device
./src/cli/commands/identity.ts:24:import { wellinformedHome } from '../runtime.js';
./src/cli/commands/identity.ts:43:  const res = await ensureIdentity(wellinformedHome());
./src/cli/commands/identity.ts:56:  const res = await ensureIdentity(wellinformedHome());
./src/cli/commands/identity.ts:66:  const res = await rotateDeviceKey(wellinformedHome());
./src/cli/commands/identity.ts:78:    ? await exportRecoveryHex(wellinformedHome())
./src/cli/commands/identity.ts:79:    : await exportRecoveryMnemonic(wellinformedHome());
./src/cli/commands/identity.ts:90:    console.error('   Use `wellinformed identity export --hex` for the legacy v1 hex format.\n');
./src/cli/commands/identity.ts:104:    console.error('  usage: wellinformed identity import <24-word-mnemonic>');
./src/cli/commands/identity.ts:105:    console.error('         wellinformed identity import <64-char-hex>');
./src/cli/commands/identity.ts:109:  const res = await importRecoveryAuto(wellinformedHome(), input);
./src/cli/commands/identity.ts:119:  console.log('usage: wellinformed identity <sub>');
./src/cli/commands/identity.ts:128:  console.log('Every memory entry wellinformed signs is wrapped in an envelope');
./src/cli/commands/this.ts:2: * `wellinformed this [me|everyone] [--root DIR] [--name NAME]`
./src/cli/commands/this.ts:9: *   wellinformed this              → index cwd, room private
./src/cli/commands/this.ts:10: *   wellinformed this me           → same (explicit private)
./src/cli/commands/this.ts:11: *   wellinformed this everyone     → index cwd + share room with peers
./src/cli/commands/this.ts:15: * boundary identical to `wellinformed share room <name>`.
./src/cli/commands/this.ts:23:import { wellinformedHome, runtimePaths } from '../runtime.js';
./src/cli/commands/this.ts:53:const USAGE = `usage: wellinformed this [me|everyone] [--root DIR] [--name NAME]
./src/cli/commands/this.ts:71:  console.log(`wellinformed this ${visibility} — room '${slug}' (${root})\n`);
./src/cli/commands/this.ts:78:  registerWatchTarget(join(wellinformedHome(), 'watch-targets.json'), {
./src/cli/commands/this.ts:99:    console.log(`    wellinformed daemon stop && wellinformed daemon start`);
./src/cli/commands/this.ts:100:    console.log(`\n  track ingest progress with:  wellinformed jobs watch`);
./src/cli/commands/this.ts:107:      console.log(`  to share later: wellinformed share room ${slug}`);
./src/cli/commands/this.ts:115:  console.log(`  wellinformed daemon start`);
./src/cli/commands/this.ts:125:    console.log(`  to share later: wellinformed share room ${slug}`);
./src/cli/commands/report.ts:2: * `wellinformed report [--room R] [--since DATE] [--no-save]`
./src/cli/commands/report.ts:5: * optionally persists it to ~/.wellinformed/reports/<room>/<date>.md.
./scripts/bench-bridge.mjs:7:// against the same corpus. If true, wellinformed peers can federate
./scripts/bench-bridge.mjs:47:const CACHE_ROOT = join(homedir(), '.wellinformed', 'bench');
./scripts/bench-bridge.mjs:61:console.log(' wellinformed — cross-model embedding bridge gate');
./src/cli/commands/publish.ts:2: * `wellinformed publish <sub>`
./scripts/bench-consolidation.mjs:56:const wiHome = () => process.env.WELLINFORMED_HOME ?? join(homedir(), '.wellinformed');
./src/cli/commands/discover.ts:2: * `wellinformed discover [--room R] [--auto]`
./src/cli/commands/discover.ts:48:    console.error('discover: no room specified and no default room set. use --room or run `wellinformed init`.');
./src/cli/commands/discover.ts:82:    console.log(`\n${suggestions.length} source(s) added. run 'wellinformed trigger --room ${room}' to fetch.`);
./src/cli/commands/discover.ts:84:    console.log(`add them with: wellinformed discover --room ${room} --auto`);
./wellinformed-rs/src/bin/embed_server.rs:4://! wellinformed stack spawns on first use to embed via production-
./wellinformed-rs/src/bin/embed_server.rs:41:use wellinformed_bench::{
./wellinformed-rs/src/bin/embed_server.rs:193:    // wellinformed's production indexNode path gets the correct
./wellinformed-rs/src/bin/embed_server.rs:290:        "wellinformed-bench embed_server v{} — stdio JSON-RPC",
./scripts/bench-lab.mjs:11:// Inputs: existing bench sota.db files under ~/.wellinformed/bench/, which
./scripts/bench-lab.mjs:43:const CACHE_ROOT = join(homedir(), '.wellinformed', 'bench');
./scripts/bench-lab.mjs:48:console.log(' wellinformed — retrieval lab');
./demo/scene-claude.tape:1:# wellinformed — scene 4 — side-by-side timing comparison
./demo/scene-claude.tape:5:# Run B: this repo, wellinformed PreToolUse hook wired.
./demo/scene-claude.tape:30:Type "export WELLINFORMED_HOME=$HOME/.wellinformed.demo"
./demo/scene-claude.tape:38:Type '# Same model. Same prompt. The only difference: wellinformed.'
./demo/scene-claude.tape:42:Type 'cd /tmp/demo-without-wellinformed'
./demo/scene-claude.tape:55:# ─── Run B: Claude + wellinformed — fast + cited ───────────
./demo/scene-claude.tape:60:Type 'cd /Users/saharbarak/personal/wellinformed'
./demo/scene-claude.tape:64:Type '# (2)  claude WITH the wellinformed PreToolUse hook'
./src/cli/commands/room.ts:2: * `wellinformed room <sub>` — manage the room registry.
./src/cli/commands/room.ts:12: *   wellinformed room create homelab --desc "Home lab infra" --keywords "proxmox,mikrotik,10gbe"
./src/cli/commands/room.ts:13: *   wellinformed room switch homelab
./src/cli/commands/room.ts:36:    console.log('no rooms configured. try `wellinformed init` or `wellinformed room create <name>`.');
./src/cli/commands/room.ts:50:    console.error('room create: missing <name>. usage: wellinformed room create <name> [--desc "..."] [--keywords "a,b,c"] [--wing "default"]');
./src/cli/commands/room.ts:117:    console.log('no default room set. run `wellinformed init` to create one.');
./src/cli/commands/trigger.ts:2: * `wellinformed trigger [--room <room>] [--sync]` — run an ingest pass.
./src/cli/commands/trigger.ts:75:    console.log('trigger: no sources configured — use `wellinformed sources add` to seed one.');
./src/cli/commands/trigger.ts:89:  console.log(`\n${rooms.length - failed} job(s) queued — track with: wellinformed jobs watch`);
./src/cli/commands/trigger.ts:130:      console.log('trigger: no sources configured — use `wellinformed sources add` to seed one.');
./src/cli/commands/unshare.ts:2: * `wellinformed unshare <name>` — make a room private again.
./src/cli/commands/unshare.ts:8: * KEEPS the .ydoc binary file at ~/.wellinformed/ydocs/<name>.ydoc so a
./src/cli/commands/unshare.ts:15:import { wellinformedHome } from '../runtime.js';
./src/cli/commands/unshare.ts:17:const sharedRoomsPath = (): string => join(wellinformedHome(), 'shared-rooms.json');
./src/cli/commands/unshare.ts:19:const USAGE = `usage: wellinformed unshare <name>
./src/cli/commands/unshare.ts:59:  console.log('  restart the daemon (wellinformed daemon stop && start) to close active sync streams for this room');
./scripts/bench-compare.mjs:10://     ~/.wellinformed/bench/scifact__nomic-ai-nomic-embed-text-v1-5/results.json \
./scripts/bench-compare.mjs:11://     ~/.wellinformed/bench/scifact__nomic-ai-nomic-embed-text-v1-5__hybrid/results.json
./src/cli/commands/daemon.ts:2: * `wellinformed daemon <sub>` — background research daemon.
./src/cli/commands/daemon.ts:132:    console.error(`  another wellinformed process is already mutating. retry in a moment.`);
./src/cli/commands/daemon.ts:170:  // Phase 41 — file-watcher (registered roots from `wellinformed this`)
./src/cli/commands/daemon.ts:195:  void rt.value.embedder.embed('wellinformed daemon warm').then(
./src/cli/runtime.ts:36:export const wellinformedHome = (): string =>
./src/cli/runtime.ts:37:  process.env.WELLINFORMED_HOME ?? join(homedir(), '.wellinformed');
./src/cli/runtime.ts:69: *   'rust'    — spawns the `wellinformed-rs` embed_server binary and
./src/cli/runtime.ts:87:  // Measured 3.1× throughput on the live wellinformed stack via
./src/cli/runtime.ts:129:  const home = wellinformedHome();
./src/cli/commands/lint.ts:2: * `wellinformed lint [--room R] [--json]`
./src/cli/commands/bench.ts:2: * `wellinformed bench memory` — unified memory benchmark runner.
./src/cli/commands/bench.ts:56: * the Hetzner box (with `WELLINFORMED_BENCH_PUBLIC_REAL=1` and the
./src/cli/commands/bench.ts:161:  lines.push(`wellinformed memory bench — ${r.runAt}`);
./src/cli/commands/bench.ts:190:  console.log(`wellinformed bench — unified memory benchmark
./demo/scene-prompt-hook.tape:1:# wellinformed — scene 9 — UserPromptSubmit hook in flight
./demo/scene-prompt-hook.tape:27:Type "export WELLINFORMED_HOME=$HOME/.wellinformed.demo"
./demo/scene-prompt-hook.tape:34:Type "# wellinformed UserPromptSubmit hook — fires BEFORE Claude reads the user message."
./demo/scene-prompt-hook.tape:48:Type "cat /tmp/wi-prompt-demo.json | node .claude/hooks/wellinformed-prompt-submit.cjs | jq -r .hookSpecificOutput.additionalContext"
./src/cli/commands/peers-rep.ts:2: * `wellinformed peers rep [<peer-id>] [--subject <key>] [--json]`
./src/cli/commands/peers-rep.ts:18: * consumers (CI tooling, the future `wellinformed audit export`).
./src/cli/commands/peers-rep.ts:33:const wellinformedHomeDir = (): string =>
./src/cli/commands/peers-rep.ts:34:  process.env.WELLINFORMED_HOME ?? join(homedir(), '.wellinformed');
./src/cli/commands/peers-rep.ts:132:    console.log('no peer reputation data yet — needs at least one federated ask with `wellinformed ask --peers`.');
./src/cli/commands/peers-rep.ts:201:const USAGE = `usage: wellinformed peers rep [<peer-id>] [--subject <key>] [--json]
./src/cli/commands/peers-rep.ts:214:  const home = wellinformedHomeDir();
./src/cli/commands/codebase.ts:2: * `wellinformed codebase <sub>` — Phase 19 structured code graph management.
./src/cli/commands/codebase.ts:5: *   index <path> [--name N]   parse a codebase into ~/.wellinformed/code-graph.db
./src/cli/commands/codebase.ts:14: * Separate from `wellinformed index` (shallow research-room indexing — NOT modified
./src/cli/commands/codebase.ts:64:    console.error('codebase index: missing <path>. usage: wellinformed codebase index <path> [--name <name>]');
./src/cli/commands/codebase.ts:139:      console.log('no indexed codebases. try `wellinformed codebase index <path>`.');
./src/cli/commands/codebase.ts:165:    console.error('codebase show: missing <id>. usage: wellinformed codebase show <id>');
./src/cli/commands/codebase.ts:214:    console.error('codebase reindex: missing <id>. usage: wellinformed codebase reindex <id>');
./src/cli/commands/codebase.ts:259:    console.error('codebase attach: usage: wellinformed codebase attach <id> --room <room-id>');
./src/cli/commands/codebase.ts:296:    console.error('codebase detach: usage: wellinformed codebase detach <id> --room <room-id>');
./src/cli/commands/codebase.ts:326:    console.error('codebase search: missing <query>. usage: wellinformed codebase search <query> [--codebase <id>] [--kind <kind>]');
./src/cli/commands/codebase.ts:380:    console.error('codebase remove: missing <id>. usage: wellinformed codebase remove <id>');
./src/cli/commands/codebase.ts:406:const USAGE = `usage: wellinformed codebase <index|list|show|reindex|attach|detach|search|remove>
./src/cli/commands/codebase.ts:409:  index <path> [--name N] [--json]     parse a codebase into ~/.wellinformed/code-graph.db
./src/cli/commands/peer.ts:2: * `wellinformed peer <sub>` — manage P2P peer connections.
./src/cli/commands/peer.ts:26:import { wellinformedHome } from '../runtime.js';
./src/cli/commands/peer.ts:28:const identityPath = (): string => join(wellinformedHome(), 'peer-identity.json');
./src/cli/commands/peer.ts:29:const peersPath = (): string => join(wellinformedHome(), 'peers.json');
./src/cli/commands/peer.ts:30:const configPath = (): string => join(wellinformedHome(), 'config.yaml');
./src/cli/commands/peer.ts:36:    console.error('peer add: missing <multiaddr>. usage: wellinformed peer add /ip4/1.2.3.4/tcp/9001');
./src/cli/commands/peer.ts:107:    console.error('peer remove: missing <id>. usage: wellinformed peer remove <peerId>');
./src/cli/commands/peer.ts:173:    console.log('no known peers. try `wellinformed peer add <multiaddr>`.');
./src/cli/commands/peer.ts:216:const USAGE = `usage: wellinformed peer <add|remove|list|status>
./src/infrastructure/embedders.ts:149: * Spawns the `wellinformed-rs/target/release/embed_server` binary on
./src/infrastructure/embedders.ts:166:   * `wellinformed-rs/target/release/embed_server`; override via
./src/infrastructure/embedders.ts:203:    // Default path assumes the repo layout: wellinformed-rs is a sibling
./src/infrastructure/embedders.ts:208:      return join(here, '..', '..', 'wellinformed-rs', 'target', 'release', 'embed_server');
./src/infrastructure/embedders.ts:333: * encoder. Measured gain on the live wellinformed stack (bench-embed-
./scripts/bench-warm.mjs:28:const home = process.env.WELLINFORMED_HOME ?? join(homedir(), '.wellinformed');
./package.json:2:  "name": "wellinformed",
./package.json:7:    "wellinformed": "./bin/wellinformed.js"
./package.json:13:    "doctor": "node bin/wellinformed.js doctor",
./package.json:14:    "start": "node bin/wellinformed.js",
./src/cli/commands/dashboard.ts:2: * `wellinformed dashboard [--port N]`
./src/cli/commands/dashboard.ts:24:<title>wellinformed dashboard</title>
./src/cli/commands/dashboard.ts:60:  <div class="logo">wellinformed</div>
./src/cli/commands/dashboard.ts:256:    console.log(`wellinformed dashboard running at http://localhost:${port}`);
./src/infrastructure/process-lock.ts:12: *   - POSIX exclusive-create on `<home>/wellinformed.lock` (the same
./src/infrastructure/process-lock.ts:70:const lockPath = (homeDir: string): string => join(homeDir, 'wellinformed.lock');
./src/infrastructure/process-lock.ts:107: * Try to acquire the wellinformed write lock. Returns a LockHandle on
./src/infrastructure/process-lock.ts:161:        `wellinformed.lock held by ${result.owner} (pid=${result.pid}); wait or stop the holder`,
./src/infrastructure/process-lock.ts:193: * `wellinformed status` style commands that want to surface "is anyone
./wellinformed-rs/Cargo.lock:2937:name = "wellinformed-bench"
./src/infrastructure/log-store.ts:122: * `wellinformed logs tail` flow. For a streaming watcher the caller
./src/cli/commands/jobs.ts:2: * `wellinformed jobs <sub>` — inspect the daemon's background queue.
./src/cli/commands/jobs.ts:23:const USAGE = `usage: wellinformed jobs <sub>
./src/cli/commands/jobs.ts:32:    console.error('jobs: daemon is not running. start it with `wellinformed daemon start`.');
./src/cli/commands/jobs.ts:60:    process.stdout.write(`wellinformed jobs — ${new Date().toISOString()}\n\n`);
./src/infrastructure/telemetry-formatter.ts:5: *   - CLI tail output after `wellinformed ask --peers`
./src/infrastructure/telemetry-formatter.ts:12:const HR_TOP    = '─── wellinformed peer pull ─────────────────────────────────';
./wellinformed-rs/Cargo.toml:2:name = "wellinformed-bench"
./wellinformed-rs/Cargo.toml:7:# Minimum-viable BEIR benchmark for wellinformed in Rust.
./wellinformed-rs/Cargo.toml:11:# NDCG@10. Reads BEIR corpora directly from ~/.wellinformed/bench/
./src/infrastructure/watch-targets.ts:5: * Each entry: { room, root, registered_at }. `wellinformed this`
./src/infrastructure/watch-targets.ts:50:  // watch-targets — losing them means every `wellinformed this` has
./src/infrastructure/watch-targets.ts:60: * (room, root) pair so re-running `wellinformed this` refreshes the
./src/infrastructure/http/fetcher.ts:49:  const userAgent = opts.userAgent ?? 'wellinformed/0.1 (+https://github.com/saharbarak/wellinformed)';
./src/infrastructure/share-store.ts:2: * Share store — persists the shared-rooms registry to ~/.wellinformed/shared-rooms.json.
./src/infrastructure/share-store.ts:29: * concurrent wellinformed processes (e.g., daemon + CLI `share room foo`)
./src/infrastructure/share-store.ts:284: * and releases the lock. Prevents two wellinformed processes (e.g., daemon +
./src/infrastructure/sources-config.ts:4: * The registry lives at `~/.wellinformed/sources.json` (overridable
./README.md.bak.830lines:2:  <img src="docs/logo.png" alt="wellinformed" width="400" />
./README.md.bak.830lines:6:  <a href="https://github.com/SaharBarak/wellinformed/stargazers"><img src="https://img.shields.io/github/stars/SaharBarak/wellinformed?style=social" alt="Stars" /></a>&nbsp;
./README.md.bak.830lines:7:  <a href="https://github.com/SaharBarak/wellinformed/network/members"><img src="https://img.shields.io/github/forks/SaharBarak/wellinformed?style=social" alt="Forks" /></a>&nbsp;
./README.md.bak.830lines:8:  <a href="https://github.com/SaharBarak/wellinformed/watchers"><img src="https://img.shields.io/github/watchers/SaharBarak/wellinformed?style=social" alt="Watchers" /></a>
./README.md.bak.830lines:14:  <img src="demo/scene-claude.gif" alt="Side-by-side: Claude alone (~14 s, hedged) vs Claude + wellinformed (~1.5 s, cited)" width="880" />
./README.md.bak.830lines:18:  <em>Same model. Same prompt. The only difference is wellinformed.</em>
./README.md.bak.830lines:24:$ wellinformed ask "vector search sqlite" --k 3
./README.md.bak.830lines:27:   distance: 0.961 | room: wellinformed-dev
./README.md.bak.830lines:47:| <img src="demo/scene-prompt-hook.gif" alt="UserPromptSubmit hook — answer arrives before Claude reads the message" width="420" /> | **The hook fires BEFORE the LLM reads your message.** UserPromptSubmit + wellinformed = retrieval at prompt time. Claude has the answer with citations *before* it considers a tool call. |
./README.md.bak.830lines:49:| <img src="demo/scene-codebase.gif" alt="codebase Q&A" width="420" /> | **Codebase Q&A.** `claude -p` cites `src/daemon/job-queue.ts` directly via the wellinformed PreToolUse hook. |
./README.md.bak.830lines:55:## Why wellinformed exists
./README.md.bak.830lines:61:**wellinformed is how that ecosystem shares its knowledge.**
...
./README.md.bak.830lines:71:**1. Each peer carries a shard of what's current — together, the live index.** Every wellinformed instance is a libp2p peer. Rooms sync across peers via Y.js CRDT. A federated `ask --peers` fans a query across the network in parallel, 2-second per-peer timeout, results merged by cosine distance with per-peer attribution. The stranger who read that paper last Thursday, the peer who benchmarked that library two weeks ago, the dev who debugged that exact bug last night — their embeddings flow into your session. Nobody knows the whole graph; together the community does — the live state of the field, something no frozen-weight model can touch.
./README.md.bak.830lines:80:  <img src="docs/memory-stack.png" alt="The same Claude session, two different Fridays — without vs with wellinformed" width="920" />
./README.md.bak.830lines:85:Without wellinformed, your Claude session starts empty every time. Claude browses ten URLs, burns forty-five thousand tokens, takes thirty seconds, returns an answer half a year stale, and dies with the tab. Ten thousand other people run the exact same loop the same day. None of it compounds.
./README.md.bak.830lines:87:With wellinformed, the `PreToolUse` hook fires before Claude reaches for the web. Your graph — already holding every arXiv paper you've pulled, every repo you've starred, every past session you've had, plus every shard shared by every other peer running wellinformed — answers in **11 ms**. Three hits across three rooms: a GitHub repo someone starred yesterday, a piece of community code you hadn't seen, an arXiv paper from two hours ago. Claude replies instantly from the community's latest state. When the session ends, your transcript is vector-indexed back into the graph so tomorrow's session starts richer than today's. And every peer on the network is doing the same — the ten-thousand-stranger loop runs **once**, not ten thousand times.
./README.md.bak.830lines:96:git clone https://github.com/SaharBarak/wellinformed.git && cd wellinformed
./README.md.bak.830lines:103:wellinformed init                      # create a room, pick your sources
./README.md.bak.830lines:104:wellinformed trigger --room homelab    # fetch from ArXiv, HN, RSS, blogs, any URL
./README.md.bak.830lines:105:wellinformed index                     # index your codebase + deps + git
./README.md.bak.830lines:110:Register wellinformed as a **user-scoped MCP server** so every project gets it automatically — no `.mcp.json` per repo, no restart for each new project:
./README.md.bak.830lines:113:claude mcp add --scope user wellinformed -- wellinformed mcp
./README.md.bak.830lines:114:wellinformed claude install            # PreToolUse hook — Claude checks the graph first
./README.md.bak.830lines:136:**Research channels** — wellinformed also connects to the GitHub analytics ecosystem for tracking trends, competitors, and emerging tools:
./README.md.bak.830lines:151:wellinformed discover --room homelab --auto
./README.md.bak.830lines:157:After `wellinformed claude install`, a PreToolUse hook fires before every Glob/Grep/Read. Claude sees:
./README.md.bak.830lines:159:> *"wellinformed: Knowledge graph exists (425 nodes). Consider using search, ask, get_node before searching raw files."*
...
./README.md.bak.830lines:177:**Tunnels** are the exception — when nodes in different rooms are semantically close, wellinformed flags them. A paper about embedding quantization in `ml-papers` connects to a memory issue in `homelab`. That connection is what rooms exist to produce.
./README.md.bak.830lines:181:Every wellinformed instance is a libp2p peer with a cryptographic identity. Rooms can be shared across peers via Y.js CRDT. Search runs federated across the network. mDNS auto-discovers peers on your LAN; NAT traversal via circuit-relay-v2 + dcutr + UPnP handles the public internet. All traffic is encrypted by libp2p Noise; peers authenticate via ed25519 during the handshake.
./README.md.bak.830lines:185:wellinformed peer status                          # show your PeerId + public key
./README.md.bak.830lines:186:wellinformed peer add /ip4/1.2.3.4/tcp/9001      # connect to a remote peer
./README.md.bak.830lines:187:wellinformed peer list                            # known peers (--json for agents)
./README.md.bak.830lines:190:wellinformed share audit --room homelab           # see exactly what would be shared
./README.md.bak.830lines:191:wellinformed share room homelab                   # mark room as shared (audit-gated)
./README.md.bak.830lines:192:wellinformed unshare homelab                      # stop sharing (keeps local .ydoc)
./README.md.bak.830lines:195:wellinformed ask "proxmox GPU passthrough" --peers  # query across connected peers
./README.md.bak.830lines:212:Every wellinformed peer advertises **three always-on system rooms** out of the box. No opt-in, no manual sharing — every peer can touch them immediately:
./README.md.bak.830lines:220:Membership is **virtual** — derived from each node's `source_uri` scheme, not from its physical `room` field. A git commit tagged `room: wellinformed-dev` still shows up in `toolshed` for peers. User-chosen rooms stay intact; system rooms are an additional query-time lens.
./README.md.bak.830lines:227:wellinformed share ui                             # interactive toggle list
./README.md.bak.830lines:236:wellinformed oracle ask "how do I wire prefetch hooks without adding a dep?"
./README.md.bak.830lines:240:wellinformed oracle answerable                    # what can your graph plausibly answer?
./README.md.bak.830lines:241:wellinformed oracle answer <qid> "use raw ANSI + setRawMode" --confidence 0.85
./README.md.bak.830lines:242:wellinformed oracle show <qid>                    # confidence-ranked answers
./README.md.bak.830lines:248:wellinformed oracle ask "..." --live              # publishes over pubsub for real-time fan-out
./README.md.bak.830lines:249:wellinformed oracle answer <qid> "..." --live     # live response path
./README.md.bak.830lines:250:wellinformed daemon start                         # daemon subscribes at boot and upserts
./README.md.bak.830lines:256:`@libp2p/floodsub` ships Layer B today; gossipsub's latest still targets libp2p/interface v2 while wellinformed runs v3 — the service API is identical so a future swap is a one-line change.
./README.md.bak.830lines:260:Beyond raw ingest, `wellinformed save` files typed notes (synthesis / concept / decision / source) that outlive chat transcripts. Ported from [claude-obsidian](https://github.com/AgriciDaniel/claude-obsidian) — reused for the Q&A distillation flow.
./README.md.bak.830lines:263:wellinformed save --room project --type synthesis --label "Touch primitive" \
./README.md.bak.830lines:265:echo "body..." | wellinformed save --room project --type concept --label "RNG tunnels"
./README.md.bak.830lines:272:Every wellinformed install provisions a W3C `did:key` on first boot. Ed25519 keypair, 32-byte pubkey, base58btc-encoded with the `0xed01` multicodec prefix per the [did:key spec](https://w3c-ccg.github.io/did-method-key/). The user DID is long-lived and survives device changes; a device key is authorized by the user DID via a signed tuple `(device_id, device_pub, authorized_at)` so individual devices can be rotated without losing identity.
./README.md.bak.830lines:275:wellinformed identity show           # prints your user DID + authorized devices
./README.md.bak.830lines:276:wellinformed identity rotate         # new device key, same user DID, old device revoked
./README.md.bak.830lines:277:wellinformed identity export         # BIP39 mnemonic recovery phrase
./README.md.bak.830lines:278:wellinformed identity import         # restore from recovery phrase on a new machine
...
./README.md.bak.830lines:292:**Why this matters:** the VC-funded AI memory category holds your identity in their user table. When they change pricing, when they get acquired, when they revoke your account — they take your identity with them. wellinformed's identity is math you already own; no intermediary to revoke it.
./README.md.bak.830lines:296:Separate from the research graph, wellinformed parses codebases into a rich structured code graph via tree-sitter. Codebases are first-class aggregates attachable to rooms via a join table. Nothing mixes the research nodes and the code nodes — two distinct graphs, two distinct query surfaces.
./README.md.bak.830lines:299:wellinformed codebase index ~/work/my-app         # parse with tree-sitter (TS+JS+Python)
./README.md.bak.830lines:300:wellinformed codebase attach <id> --room homelab  # attach to a research room (M:N)
./README.md.bak.830lines:301:wellinformed codebase search "loadConfig"          # lexical search across attached codebases
./README.md.bak.830lines:302:wellinformed codebase list --json                  # machine-readable view with node counts
...
./README.md.bak.830lines:312:wellinformed discover-loop --room homelab --max-iterations 3
./README.md.bak.830lines:325:wellinformed publish auth              # OAuth 2.0 — opens browser
./README.md.bak.830lines:326:wellinformed publish preview           # see what would be posted
./README.md.bak.830lines:327:wellinformed publish launch            # post the launch thread
./README.md.bak.830lines:328:wellinformed publish tweet "text"      # post a single tweet
./README.md.bak.830lines:334:wellinformed daemon start              # runs trigger on a schedule
./README.md.bak.830lines:335:wellinformed daemon status             # check if running
./README.md.bak.830lines:336:wellinformed report --room homelab     # see what's new
./README.md.bak.830lines:341:Real retrieval quality measured against canonical BEIR datasets using wellinformed's runtime pipeline. All numbers directly comparable to the [MTEB BEIR leaderboard](https://huggingface.co/spaces/mteb/leaderboard).
./README.md.bak.830lines:367:| **wellinformed Phase 25 (hybrid + Rust)** | **137M** | **75.22%** | **CPU, 11ms p50** |
./README.md.bak.830lines:397:cd wellinformed-rs && cargo build --release && cd ..
./README.md.bak.830lines:398:WELLINFORMED_RUST_BIN=$(pwd)/wellinformed-rs/target/release/embed_server \
./README.md.bak.830lines:409:  --datasets-dir ~/.wellinformed/bench/cqadupstack/cqadupstack \
./README.md.bak.830lines:468:wellinformed is not a vector store with peer sync bolted on. It is an attempt
./README.md.bak.830lines:478:wellinformed becomes a serious agent-memory protocol. Full thinking surface
./README.md.bak.830lines:491:the missing fact that would change the action. wellinformed treats this as a
./README.md.bak.830lines:750:The execution priorities for getting wellinformed to category leadership as a
./README.md.bak.830lines:760:3. wellinformed retrieves the trusted peer memory before web search.
./README.md.bak.830lines:793:wellinformed claims category leadership when:
./README.md.bak.830lines:812:<a href="https://www.star-history.com/#SaharBarak/wellinformed&Date">
./README.md.bak.830lines:814:    <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=SaharBarak/wellinformed&type=Date&theme=dark" />
./README.md.bak.830lines:815:    <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=SaharBarak/wellinformed&type=Date" />
./README.md.bak.830lines:816:    <img alt="Star History" src="https://api.star-history.com/svg?repos=SaharBarak/wellinformed&type=Date" />
./src/infrastructure/identity-resolver.ts:46: *   list     — snapshot of every DID seen; used by `wellinformed
./src/infrastructure/identity-resolver.ts:65: * commit lifts state to `~/.wellinformed/identity-audit.json` with
./docs/logo.svg:1:<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 960 240" fill="none" role="img" aria-label="wellinformed — the network before the web">
./docs/logo.svg:44:        fill="#f7f4ec" letter-spacing="-2.4">wellinformed</text>
./src/infrastructure/sources/twitter-search.ts:5: * reads from a local cache file (~/.wellinformed/twitter-cache.json)
./src/infrastructure/sources/twitter-search.ts:64:const CACHE_PATH = join(homedir(), '.wellinformed', 'twitter-cache.json');
./demo/MANUSCRIPT.md:1:# wellinformed — Demo Manuscript
./demo/MANUSCRIPT.md:10:1. **wellinformed turns local research into agent context faster than the agent can search the web.**
./demo/MANUSCRIPT.md:25:# 1. Clean wellinformed home so the demo starts from a known state.
./demo/MANUSCRIPT.md:26:mv ~/.wellinformed ~/.wellinformed.backup-$(date +%s) 2>/dev/null || true
./demo/MANUSCRIPT.md:33:wellinformed onboard --yes
./demo/MANUSCRIPT.md:36:wellinformed daemon status
./demo/MANUSCRIPT.md:37:wellinformed metrics | jq .gauges     # expect queue.queued=0, queue.running=0
./demo/MANUSCRIPT.md:43:wellinformed ask "boil-off detection in cryogenic hydrogen tanks" --k 3
./demo/MANUSCRIPT.md:60:wellinformed
./demo/MANUSCRIPT.md:75:"point wellinformed at a folder of notes" — that's what we demonstrate.
./demo/MANUSCRIPT.md:90:$ wellinformed this me
./demo/MANUSCRIPT.md:102:> and the ML used in that field. `wellinformed this me` chunks them,
./demo/MANUSCRIPT.md:116:$ time wellinformed ask "what ML methods are used for liquid hydrogen leak detection"
./demo/MANUSCRIPT.md:122:# wellinformed agent contract (hook_version: 2)         ◄── decision FIRST
./demo/MANUSCRIPT.md:163:graph — no `wellinformed` invocation visible.
./demo/MANUSCRIPT.md:166:already has the wellinformed PreToolUse hook wired (from `wellinformed onboard` earlier).
./demo/MANUSCRIPT.md:180:- RIGHT (terminal tail of `~/.wellinformed/daemon.log`): the hook fires, `ask` runs,
./demo/MANUSCRIPT.md:186:> wellinformed hook fires before Claude's tool call, runs `ask`, and
./demo/MANUSCRIPT.md:189:> `wellinformed`.** The agent is just smarter.
./demo/MANUSCRIPT.md:193:## Scene 4 — Side-by-side: Claude alone vs Claude + wellinformed (90 sec)
./demo/MANUSCRIPT.md:197:**Setup:** two Claude Code panels side-by-side. **LEFT** has the wellinformed hook
./demo/MANUSCRIPT.md:212:- **RIGHT** (Claude + wellinformed): Claude responds in ~1.5 sec with specifics from the
./demo/MANUSCRIPT.md:219:> Same question. Same model. The only difference is whether wellinformed
./demo/MANUSCRIPT.md:228:**Goal:** show that wellinformed has TWO retrieval channels — semantic search AND
./demo/MANUSCRIPT.md:234:$ wellinformed recall stanford-cryo-lab
./demo/MANUSCRIPT.md:276:WELLINFORMED_HOME=~/.wellinformed-peerB wellinformed onboard --yes
./demo/MANUSCRIPT.md:277:WELLINFORMED_HOME=~/.wellinformed-peerB wellinformed save \
./demo/MANUSCRIPT.md:280:WELLINFORMED_HOME=~/.wellinformed-peerB wellinformed share room research
./demo/MANUSCRIPT.md:283:wellinformed peer add /ip4/127.0.0.1/tcp/<peerB-port>/p2p/<peerB-id>
./demo/MANUSCRIPT.md:284:wellinformed share room research
./demo/MANUSCRIPT.md:290:$ wellinformed peer list
./demo/MANUSCRIPT.md:294:$ wellinformed touch --peer peer-B --room research --label "open-source LH2 spectrometers"
./demo/MANUSCRIPT.md:298:$ wellinformed ask "open-source spectrometer projects for hydrogen sensing"
./demo/MANUSCRIPT.md:308:> `wellinformed touch` pulls just that note, attributes it to the peer
./demo/MANUSCRIPT.md:318:question answered with citations from `wellinformed`'s own source tree.
./demo/MANUSCRIPT.md:320:**Setup:** in the wellinformed repo:
./demo/MANUSCRIPT.md:323:$ wellinformed this me
./demo/MANUSCRIPT.md:324:indexing /Users/.../wellinformed … 312 chunks / 5.3 s
./demo/MANUSCRIPT.md:330:How does the wellinformed daemon do bounded backpressure on the job queue?
./demo/MANUSCRIPT.md:356:wellinformed
./demo/MANUSCRIPT.md:364:  github.com/SaharBarak/wellinformed
./demo/MANUSCRIPT.md:369:> Three things make wellinformed different. It's local-first — your
./demo/MANUSCRIPT.md:373:> without a middleman. Try it: github.com/SaharBarak/wellinformed.
./package-lock.json:2:  "name": "wellinformed",
./package-lock.json:8:      "name": "wellinformed",
./package-lock.json:51:        "wellinformed": "bin/wellinformed.js"
./tests/phase17.mcp-tool.test.ts:31:      home: '/tmp/wellinformed-test',
./tests/phase17.mcp-tool.test.ts:32:      graph: '/tmp/wellinformed-test/graph.json',
./tests/phase17.mcp-tool.test.ts:33:      vectors: '/tmp/wellinformed-test/vectors.db',
./tests/phase17.mcp-tool.test.ts:34:      sources: '/tmp/wellinformed-test/sources.json',
./tests/phase17.mcp-tool.test.ts:35:      rooms: '/tmp/wellinformed-test/rooms.json',
./tests/phase17.mcp-tool.test.ts:36:      modelCache: '/tmp/wellinformed-test/models',
./src/infrastructure/cross-encoder.ts:46:   * to keep wellinformed self-contained.
./README.md:1:# Akashik
...
./README.md:74:> The npm package and CLI binary are still named `wellinformed` during the two-name period. The brand-marketing name is **Akashik**; a coordinated rename of package + repo + DNS is queued behind the public launch. Examples below show both forms.
./README.md:79:npm install -g wellinformed
./README.md:86:wellinformed init
./README.md:87:wellinformed daemon start
./README.md:93:wellinformed save https://arxiv.org/abs/2406.16678 --room research
./README.md:94:wellinformed save ./notes/cuda-oom-debug.md --room toolshed
./README.md:100:wellinformed ask "how does mxbai-rerank compare to cross-encoder on long contexts?"
...
./README.md:125:- **Codebase rename** — coordinated `wellinformed → akashik` migration across npm, GitHub, DNS.
...
./README.md:144:Pre-launch. Simulator-validated, retrieval-benchmarked, pilot pending. Two-name period in effect (`wellinformed` in code, `Akashik` in marketing). Public protocol spec lands with the rename.
./demo/scene-touch.sh:3:# wellinformed demo — scene 6 (P2P touch) end-to-end orchestrator.
./demo/scene-touch.sh:42:export WELLINFORMED_DEMO_PEER_B_ID=$(cat "$HOME/.wellinformed.demo/peer-B-id")
./demo/scene-touch.sh:43:export WELLINFORMED_DEMO_PEER_C_ID=$(cat "$HOME/.wellinformed.demo/peer-C-id")
./demo/scene-touch.sh:44:export WELLINFORMED_DEMO_PEER_D_ID=$(cat "$HOME/.wellinformed.demo/peer-D-id")
./demo/scene-touch.sh:45:export WELLINFORMED_DEMO_PEER_E_ID=$(cat "$HOME/.wellinformed.demo/peer-E-id")
./tests/phase5.ask-report.test.ts:58:  const tmp = mkdtempSync(join(tmpdir(), 'wellinformed-phase5-'));
./tests/phase5.ask-report.test.ts:101:    assert.ok(md.includes('# wellinformed report'));
./demo/teardown-p2p.sh:3:# wellinformed demo — P2P touch teardown.
./demo/teardown-p2p.sh:14:# `~/.wellinformed.demo*.archived-<ts>` so the next setup starts
./demo/teardown-p2p.sh:25:  "${WELLINFORMED_DEMO_HOME:-$HOME/.wellinformed.demo}"
./demo/teardown-p2p.sh:26:  "${WELLINFORMED_DEMO_PEER_B_HOME:-$HOME/.wellinformed.demo-peerB}"
./demo/teardown-p2p.sh:27:  "${WELLINFORMED_DEMO_PEER_C_HOME:-$HOME/.wellinformed.demo-peerC}"
./demo/teardown-p2p.sh:28:  "${WELLINFORMED_DEMO_PEER_D_HOME:-$HOME/.wellinformed.demo-peerD}"
./demo/teardown-p2p.sh:29:  "${WELLINFORMED_DEMO_PEER_E_HOME:-$HOME/.wellinformed.demo-peerE}"
./demo/teardown-p2p.sh:32:echo "── wellinformed P2P teardown ────────────────────────────"
./demo/teardown-p2p.sh:37:    if WELLINFORMED_HOME="$h" wellinformed daemon stop 2>/dev/null; then
./demo/teardown-p2p.sh:48:# Belt-and-braces — kill any wellinformed daemon process still alive
./demo/teardown-p2p.sh:51:# main wellinformed daemon if it shares the parent terminal session.
./demo/teardown-p2p.sh:52:if pgrep -fl "wellinformed.*daemon.*_run" >/dev/null 2>&1; then
./demo/teardown-p2p.sh:54:  pkill -TERM -f "wellinformed.*daemon.*_run" 2>/dev/null || true
./src/infrastructure/search-gossip.ts:6: * `wellinformed ask --peers` from O(peers × dial_handshake_ms) to
./src/infrastructure/search-gossip.ts:14: *   /wellinformed/search/1.0.0           — request topic
./src/infrastructure/search-gossip.ts:15: *   /wellinformed/search-resp/1.0.0      — response topic
./src/infrastructure/search-gossip.ts:72:export const SEARCH_REQ_TOPIC = '/wellinformed/search/1.0.0' as const;
./src/infrastructure/search-gossip.ts:73:export const SEARCH_RESP_TOPIC = '/wellinformed/search-resp/1.0.0' as const;
./src/infrastructure/search-gossip.ts:226:// loaded (~/.wellinformed/swarm-corpus.jsonl), it ALSO publishes
./src/infrastructure/tree-sitter-parser.ts:5: * CRITICAL: tree-sitter and its grammar packages are CommonJS. wellinformed
./demo/validate-reputation.sh:3:# wellinformed demo — peer reputation validation harness.
./demo/validate-reputation.sh:7:# via `wellinformed peers rep` to verify the wire-up actually
./demo/validate-reputation.sh:22:A_HOME="${WELLINFORMED_DEMO_HOME:-$HOME/.wellinformed.demo}"
./demo/validate-reputation.sh:43:# `wellinformed ask --peers` boots its OWN ephemeral libp2p node and
./demo/validate-reputation.sh:51:WELLINFORMED_HOME="$A_HOME" wellinformed daemon stop || true
./demo/validate-reputation.sh:52:# Remove the stale daemon.sock — the bin/wellinformed.js shim's
./demo/validate-reputation.sh:72:  WELLINFORMED_HOME="$A_HOME" wellinformed ask "$q" --peers --k 5 \
./demo/validate-reputation.sh:84:echo "── \`wellinformed peers rep\` (default — top-3 per peer) ──"
./demo/validate-reputation.sh:85:WELLINFORMED_HOME="$A_HOME" wellinformed peers rep || true
./demo/validate-reputation.sh:88:echo "── \`wellinformed peers rep --subject room:research\` ──"
./demo/validate-reputation.sh:89:WELLINFORMED_HOME="$A_HOME" wellinformed peers rep --subject room:research || true
./demo/validate-reputation.sh:92:echo "── raw rep file (~/.wellinformed.demo/peer-reputation.json) ──"
./src/infrastructure/linked-accounts.ts:5: * Persisted at `~/.wellinformed/linked-accounts.json` with an atomic
./src/infrastructure/linked-accounts.ts:25: * verified-at timestamp says "wellinformed proved the user controls
./demo/timing.tape:1:# wellinformed — single-frame timing teaser
./demo/timing.tape:3:# Shows the headline number — `wellinformed ask` returning a full
./demo/timing.tape:27:Type "export WELLINFORMED_HOME=$HOME/.wellinformed.demo"
./demo/timing.tape:38:Type 'time wellinformed ask "who runs quench-detection LSTM research" --k 3 | head -20'
./src/infrastructure/identity-store.ts:3: * `~/.wellinformed/identity/` (or the caller-supplied home directory).
./src/infrastructure/identity-store.ts:61:/** Build the identity-layer file paths from a wellinformed home directory. */
./src/infrastructure/peer-reputation-store.ts:3: * `~/.wellinformed/peer-reputation.json`.
./src/infrastructure/peer-reputation-store.ts:57: * Load `peer-reputation.json` from the wellinformed home directory.
./src/infrastructure/recall-sync.ts:4: * Sibling protocol to /wellinformed/search/1.0.0. The architectural
./src/infrastructure/recall-sync.ts:12: *   - Mirrors how /wellinformed/touch/1.0.0 sits next to /search.
./src/infrastructure/recall-sync.ts:41:export const RECALL_PROTOCOL_ID = '/wellinformed/recall/1.0.0' as const;
./tests/peer-telemetry.test.ts:235:  assert.match(lines[0], /^─+ wellinformed peer pull/);
./demo/setup.sh:3:# wellinformed demo — one-shot setup.
./demo/setup.sh:6:# wellinformed home, then verifies the install with a sample query.
./demo/setup.sh:8:# ~/.wellinformed.demo so the demo always starts from the same place.
./demo/setup.sh:16:#   WELLINFORMED_DEMO_HOME   alternate data home (default ~/.wellinformed.demo)
./demo/setup.sh:21:DEMO_HOME="${WELLINFORMED_DEMO_HOME:-$HOME/.wellinformed.demo}"
./demo/setup.sh:24:echo "── wellinformed demo setup ─────────────────────────────"
./demo/setup.sh:39:WELLINFORMED_HOME="$DEMO_HOME" wellinformed daemon stop 2>/dev/null || true
./demo/setup.sh:41:# 3. Load each markdown note via `wellinformed save`. Each file is
./demo/setup.sh:53:  if WELLINFORMED_HOME="$DEMO_HOME" wellinformed save \
./demo/setup.sh:70:WELLINFORMED_HOME="$DEMO_HOME" wellinformed ask \
./demo/setup.sh:78:echo "    WELLINFORMED_HOME=$DEMO_HOME wellinformed ask \"who runs the cryo lab at stanford\""
./demo/setup.sh:79:echo "    WELLINFORMED_HOME=$DEMO_HOME wellinformed recall stanford-cryo-lab"
./demo/setup.sh:80:echo "    WELLINFORMED_HOME=$DEMO_HOME wellinformed metrics | jq ."
./tests/bench-tier-promotion.test.ts:8: * This is a wellinformed-specific axis — no public benchmark exists
./src/infrastructure/github-oauth.ts:6: *   1. wellinformed runs in a terminal on the user's laptop. There's
./src/infrastructure/github-oauth.ts:30: *   - User-Agent identifies wellinformed so GitHub's audit log can
./src/infrastructure/github-oauth.ts:119:const USER_AGENT = 'wellinformed-oauth/1.0';
./tests/bench-onnx.test.ts:136:    console.log(`    wellinformed:       ${(avgR5 * 100).toFixed(1)}% R@5 (measured, real ONNX)`);
./demo/README.md:1:# wellinformed — demo
./demo/README.md:10:bash demo/setup.sh         # loads the corpus into ~/.wellinformed.demo
./demo/README.md:18:export WELLINFORMED_HOME=$HOME/.wellinformed.demo
./demo/README.md:20:wellinformed ask "ML methods for liquid hydrogen leak detection" --k 3
./demo/README.md:21:wellinformed recall stanford-cryo-lab
./demo/README.md:22:wellinformed recall nasa-glenn
./demo/README.md:23:wellinformed metrics | jq .
./demo/README.md:44:| `scene-claude.gif`        | Side-by-side: Claude alone vs Claude + wellinformed (1200×800, ~350 KB). |
./demo/README.md:52:The VHS-rendered GIFs are real recordings of `wellinformed` running against the
./demo/README.md:74:`~/.wellinformed.demo*` home with a unique research note. Records the
./src/infrastructure/hw-detect.ts:3: * compute resources. Drives the rerank-tier picker so wellinformed
./src/infrastructure/code-graph.ts:4: * Opens ~/.wellinformed/code-graph.db (separate from vectors.db per
./demo/setup-p2p.sh:3:# wellinformed demo — P2P touch setup (5-peer mesh).
./demo/setup-p2p.sh:5:# Brings up 5 wellinformed daemons on 127.0.0.1 (peer A + 4 peers
./demo/setup-p2p.sh:7:# is connected to all four — `wellinformed touch` from A pulls just
./demo/setup-p2p.sh:29:A_HOME="${WELLINFORMED_DEMO_HOME:-$HOME/.wellinformed.demo}"
./demo/setup-p2p.sh:36:  "${WELLINFORMED_DEMO_PEER_B_HOME:-$HOME/.wellinformed.demo-peerB}"
./demo/setup-p2p.sh:37:  "${WELLINFORMED_DEMO_PEER_C_HOME:-$HOME/.wellinformed.demo-peerC}"
./demo/setup-p2p.sh:38:  "${WELLINFORMED_DEMO_PEER_D_HOME:-$HOME/.wellinformed.demo-peerD}"
./demo/setup-p2p.sh:39:  "${WELLINFORMED_DEMO_PEER_E_HOME:-$HOME/.wellinformed.demo-peerE}"
./demo/setup-p2p.sh:86:echo "── wellinformed P2P touch setup (5-daemon mesh) ─────────"
./demo/setup-p2p.sh:114:  WELLINFORMED_HOME="$home" wellinformed daemon stop 2>/dev/null || true
./demo/setup-p2p.sh:123:WELLINFORMED_HOME="$A_HOME" wellinformed identity init >/dev/null 2>&1 || true
./demo/setup-p2p.sh:124:WELLINFORMED_HOME="$A_HOME" wellinformed share room research >/dev/null
./demo/setup-p2p.sh:174:  WELLINFORMED_HOME="$h" wellinformed identity init >/dev/null
./demo/setup-p2p.sh:175:  WELLINFORMED_HOME="$h" wellinformed save \
./demo/setup-p2p.sh:181:  WELLINFORMED_HOME="$h" wellinformed save \
./demo/setup-p2p.sh:187:  WELLINFORMED_HOME="$h" wellinformed share room research >/dev/null
./demo/setup-p2p.sh:195:WELLINFORMED_HOME="$A_HOME" wellinformed daemon start
./demo/setup-p2p.sh:200:  WELLINFORMED_HOME="${HOMES[$i]}" wellinformed daemon start
./demo/setup-p2p.sh:208:# Note: `wellinformed peer add` boots an ephemeral libp2p node to
./demo/setup-p2p.sh:214:A_PEERID=$(WELLINFORMED_HOME="$A_HOME" wellinformed peer status 2>/dev/null | awk '/peerId/ {print $2}')
./demo/setup-p2p.sh:229:    pid=$(WELLINFORMED_HOME="$h" wellinformed peer status 2>/dev/null | awk '/peerId/ {print $2}')
./demo/setup-p2p.sh:277:# rendering. Mirrors what real `wellinformed login` would produce
./demo/setup-p2p.sh:289:    pid=$(WELLINFORMED_HOME="$h" wellinformed peer status 2>/dev/null | awk '/peerId/ {print $2}')
./demo/setup-p2p.sh:290:    did=$(WELLINFORMED_HOME="$h" wellinformed identity show 2>/dev/null | awk '/user DID/ {print $3}')
./demo/setup-p2p.sh:318:WELLINFORMED_HOME="$A_HOME" wellinformed daemon start
./demo/setup-p2p.sh:321:  WELLINFORMED_HOME="${HOMES[$i]}" wellinformed daemon start
./demo/screencast.tape:1:# wellinformed — terminal demo (screencast)
./demo/screencast.tape:28:Type "export WELLINFORMED_HOME=$HOME/.wellinformed.demo"
./demo/screencast.tape:36:Type '# wellinformed — local-first knowledge graph for AI agents'
./demo/screencast.tape:43:Type 'time wellinformed ask "ML methods for liquid hydrogen leak detection" --k 3'
./demo/screencast.tape:54:Type 'wellinformed recall stanford-cryo-lab'
./demo/screencast.tape:65:Type 'wellinformed recall nasa-glenn'
./demo/screencast.tape:76:Type 'wellinformed peers rep'
./demo/screencast.tape:83:Type 'echo "wellinformed — local-first knowledge for AI agents"'
./demo/screencast.tape:95:Type 'echo "  github.com/SaharBarak/wellinformed"'
./src/infrastructure/search-sync.ts:4: * Phase 17 core. Registers /wellinformed/search/1.0.0 on a libp2p node.
./src/infrastructure/search-sync.ts:9: *   1. SEARCH_PROTOCOL_ID is '/wellinformed/search/1.0.0' — separate from
./src/infrastructure/search-sync.ts:10: *      '/wellinformed/share/1.0.0' so sync and search have independent stream
./src/infrastructure/search-sync.ts:53:export const SEARCH_PROTOCOL_ID = '/wellinformed/search/1.0.0' as const;
./src/infrastructure/search-sync.ts:413: * Register the /wellinformed/search/1.0.0 protocol on the libp2p node.
./src/infrastructure/search-sync.ts:437: * Unregister the /wellinformed/search/1.0.0 protocol.
./src/infrastructure/search-sync.ts:486:            `wellinformed: peer ${peerIdStr} search error: ${resp.error}\n`,
./tests/contextual-enrich.test.ts:10: * Xenova model when WELLINFORMED_BENCH_PUBLIC_REAL is unset, so the
./src/application/consolidator.ts:81:   * does NOT call persist/mark. Useful for `wellinformed consolidate
./tests/identity.test.ts:172:    const msg = new TextEncoder().encode('hello wellinformed');
./tests/identity.test.ts:258:      room: 'wellinformed-dev',
./src/infrastructure/share-sync.ts:4: * Phase 16 core. Registers /wellinformed/share/1.0.0 on a libp2p node,
./src/infrastructure/share-sync.ts:76:export const SHARE_PROTOCOL_ID = '/wellinformed/share/1.0.0' as const;
./src/infrastructure/share-sync.ts:85:export const REMOTE_ORIGIN: unique symbol = Symbol('wellinformed-share-remote');
./src/infrastructure/share-sync.ts:367: *   2. Carry _wellinformed_source_peer: <peerId> as a provenance tag
./src/infrastructure/share-sync.ts:448:        _wellinformed_source_peer: remotePeerId,
./src/infrastructure/share-sync.ts:449:        ...(screened.signedBy ? { _wellinformed_signed_by: screened.signedBy } : {}),
./src/infrastructure/share-sync.ts:556:  readonly ydocsDir: string;                           // ~/.wellinformed/ydocs
./src/infrastructure/share-sync.ts:579:   * `wellinformed identity peers` (future) and audit logs can answer
./src/infrastructure/share-sync.ts:633: * Register the /wellinformed/share/1.0.0 protocol on the libp2p node.
./demo/scene-codebase.tape:1:# wellinformed — scene 7 — codebase Q&A inside Claude Code
./demo/scene-codebase.tape:3:# The wellinformed repo is indexed into ~/.wellinformed.demo (room
./demo/scene-codebase.tape:4:# `wellinformed`, 342 nodes including 249 code chunks + 50 git log
./demo/scene-codebase.tape:10:# wellinformed hook, the relevant code chunks (and the very commit
./demo/scene-codebase.tape:13:# Run: bash demo/setup.sh && WELLINFORMED_HOME=~/.wellinformed.demo \
./demo/scene-codebase.tape:14:#        wellinformed this me  &&  vhs demo/scene-codebase.tape
./demo/scene-codebase.tape:33:Type "export WELLINFORMED_HOME=$HOME/.wellinformed.demo"
./demo/scene-codebase.tape:35:Type "cd /Users/saharbarak/personal/wellinformed"
./demo/scene-codebase.tape:42:Type '# Claude Code session in the wellinformed repo'
./demo/scene-codebase.tape:44:Type '# PreToolUse hook fires before any Read/Glob/Grep — wellinformed prefetches'
./demo/scene-codebase.tape:48:Type 'time claude -p "How does the wellinformed daemon do bounded backpressure on the job queue? Cite the file path and key constants."'
./demo/scene-codebase.tape:54:Type 'echo "↑ wellinformed-prefetched: Claude cites src/daemon/job-queue.ts directly"'
./src/infrastructure/rooms-config.ts:4: * Stores the room registry at `~/.wellinformed/rooms.json`. Same
./demo/scene-federated.sh:3:# wellinformed demo — scene "federated" end-to-end orchestrator.
./tests/phase1.graph-rooms.test.ts:76:  const tmp = mkdtempSync(join(tmpdir(), 'wellinformed-phase1-'));
./tests/phase34.save-note.test.ts:4: * Covers the pure helpers that back the `wellinformed save` CLI:
./tests/phase34.save-note.test.ts:56:    assert.strictEqual(n.source_file, 'wellinformed:save');
./src/infrastructure/async-mutex.ts:7: * keeps OTHER wellinformed processes off the files; this mutex
./src/application/ingest.ts:20: *      (a new wellinformed extra field — not in graphify's patch but
./tests/bench-standard.test.ts:191:    console.log(`  ║  BEIR/HotPotQA-style Benchmark (wellinformed v1.1)       ║`);
./tests/bench-standard.test.ts:216:    console.log(`  ║    wellinformed (this run):   ${(overall.r5 * 100).toFixed(1)}% R@5, ${(overall.ndcg10 * 100).toFixed(1)}% NDCG@10  ║`);
./src/infrastructure/oracle-gossip.ts:54:export const ORACLE_TOPIC = '/wellinformed/oracle/1.0.0';
./src/infrastructure/peer-transport.ts:26:// the small networks wellinformed targets at this stage. Gossipsub 14.x
./src/infrastructure/peer-transport.ts:27:// still targets @libp2p/interface v2 while wellinformed uses v3, so
./src/infrastructure/peer-transport.ts:230:            `wellinformed: mDNS unavailable (${(e as Error).message}). ` +
./src/infrastructure/peer-transport.ts:251:        ...(dhtOn ? { dht: kadDHT({ clientMode: true, protocol: '/wellinformed/kad/1.0.0' }) } : {}),
./src/infrastructure/peer-transport.ts:331:            (e) => process.stderr.write(`wellinformed: peer:discovery persist failed: ${e.type}\n`),
./src/infrastructure/peer-transport.ts:376:        tags: { 'keep-alive-wellinformed': { value: 50 } },
./tests/phase31.remote-node-validator.test.ts:5: * known-bad node shape must be rejected; every legitimate wellinformed
./tests/phase31.remote-node-validator.test.ts:6: * node must pass. If either side breaks, wellinformed becomes either
./tests/phase31.remote-node-validator.test.ts:32:    room: 'wellinformed-dev',
./tests/phase35.p2p-touch-e2e.test.ts:89:      room: 'wellinformed-dev',
./tests/phase35.p2p-touch-e2e.test.ts:198:    // physical room is `wellinformed-dev`, NOT 'toolshed' — virtual
./tests/bench-locomo-real.test.ts:18: * strings. We collapse to the set of source SESSIONS — wellinformed
...
./tests/phase29.rust-retrieval-regression.test.ts:87:  return join(here, '..', 'wellinformed-rs', 'target', 'release', 'embed_server');
...
./tests/phase3.mcp.test.ts:137:  const tmp = mkdtempSync(join(tmpdir(), 'wellinformed-phase3-'));
./tests/bench-real.test.ts:230:    // The 30-item labeled corpus with NDCG@5 is wellinformed's BEIR
...
./tests/bench-longmemeval-real.test.ts:397:  // their full pipeline + reranker. wellinformed retrieves with
./src/application/use-cases.ts:237:// downstream callers can `import { Result } from 'wellinformed/application'`
./tests/phase20.sessions.test.ts:542:  it('SESS-07 K2: hook script shells out to wellinformed recent-sessions --hours 24 --json', () => {
./tests/phase20.sessions.test.ts:565:  it('K5: hook script guards wellinformed binary with command -v before shelling out', () => {
./tests/phase20.sessions.test.ts:567:    assert.match(src, /command -v wellinformed/, 'PATH guard required');
./tests/phase20.sessions.test.ts:574:    const hookScriptName = 'wellinformed-hook.sh';
./src/application/bip39-recovery.ts:10: *     `wellinformed identity import` autodetects the format.
./tests/release.test.ts:80:  const message = new TextEncoder().encode(`wellinformed-release:v1:${json}`);
./tests/error-hints.test.ts:38:test('GraphReadError ENOENT → hint says run `wellinformed trigger`', () => {
./tests/error-hints.test.ts:42:  assert.match(h!, /wellinformed trigger/);
./docs/assets/logo-wordmark.svg:1:<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 360 64" width="360" height="64" role="img" aria-label="wellinformed">
./docs/assets/logo-wordmark.svg:2:  <title>wellinformed — wordmark</title>
./docs/assets/logo-wordmark.svg:32:  <text x="84" y="42" class="wm-text">wellinformed</text>
...
./tests/identity-lifecycle.test.ts:124:      label: 'wellinformed v3 is a P2P memory protocol for the free LLM world',
./tests/identity-lifecycle.test.ts:125:      room: 'wellinformed-dev',
./tests/phase2.ingest.test.ts:66:  const tmp = mkdtempSync(join(tmpdir(), 'wellinformed-phase2-'));
./tests/phase2.ingest.test.ts:115:    // graph should contain 3 nodes with wellinformed fields set
./src/application/codebase-indexer.ts:372:          `wellinformed codebase: ${parseErrors} file(s) skipped due to parse errors (e.g. ${parseErrorSamples.join(', ')})\n`,
./tests/phase16.share-crdt.test.ts:69:const makeHome = (): string => mkdtempSync(join(tmpdir(), 'wellinformed-phase16-'));
./tests/phase16.share-crdt.test.ts:951:  test('SHARE_PROTOCOL_ID is /wellinformed/share/1.0.0', () => {
./tests/phase16.share-crdt.test.ts:952:    assert.equal(SHARE_PROTOCOL_ID, '/wellinformed/share/1.0.0');
./tests/phase16.share-crdt.test.ts:955:  test('REMOTE_ORIGIN is a Symbol with description wellinformed-share-remote', () => {
./tests/phase16.share-crdt.test.ts:959:      'wellinformed-share-remote',
./tests/phase16.share-crdt.test.ts:967:    assert.notEqual(REMOTE_ORIGIN as unknown, Symbol('wellinformed-share-remote'), 'two Symbol() calls with same desc are not equal');
./tests/log-store.test.ts:67:      room: 'wellinformed-dev',
./tests/identity-bridge.test.ts:65:      room: 'wellinformed-dev',
...
./src/application/peer-pull-telemetry.ts:7: * smart-hook, and `wellinformed ask --peers` CLI tail.
./docs/assets/manifesto-illustration.svg:1:<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1100 220" width="1100" height="220" role="img" aria-label="lineage timeline: Napster, eMule, BitTorrent, IPFS, wellinformed">
./docs/assets/manifesto-illustration.svg:2:  <title>wellinformed — cooperative-network lineage timeline</title>
./docs/assets/manifesto-illustration.svg:52:  <!-- wellinformed — 2026, knowledge -->
./docs/assets/manifesto-illustration.svg:58:    <text class="ln-mark" text-anchor="middle" y="36" font-weight="700">wellinformed</text>
./src/application/update-checker.ts:11: *   - npm-based installs (the canonical wellinformed distribution)
./src/application/update-checker.ts:12: *     already have an idiomatic upgrade path (`npm update -g wellinformed`)
./src/application/update-checker.ts:20: * v3.1 may add a `wellinformed update install` flow for the npm path
./src/application/update-checker.ts:135: * eligibility (for `wellinformed update status` reporting).
./src/application/update-checker.ts:145:        PeerError.identityReadError(updatePaths(homeDir).configPath, 'update not configured — run `wellinformed update configure`'),
./docs/banner.svg:1:<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1280 640" fill="none" role="img" aria-label="wellinformed — the network before the web. peer-to-peer knowledge graph for AI agents.">
./docs/banner.svg:83:        fill="#f7f4ec" letter-spacing="-3">wellinformed</text>
./docs/banner.svg:116:    <text x="44" y="48">github.com/SaharBarak/wellinformed</text>
./src/application/identity-bridge.ts:34:import { wellinformedHome } from '../cli/runtime.js';
./src/application/identity-bridge.ts:52:    const home = homeOverride ?? wellinformedHome();
./docs/assets/_graph-fallback.svg:1:<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 500" width="800" height="500" role="img" aria-label="wellinformed network — sample 50-node knowledge graph">
./docs/assets/_graph-fallback.svg:2:  <title>wellinformed knowledge graph — static fallback for §11 live demo on mobile / unreachable backend</title>
./docs/assets/og-square.svg:1:<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 1200" width="1200" height="1200" role="img" aria-label="wellinformed — The globally accumulating knowledge network. For AI agents and humans.">
./docs/assets/og-square.svg:2:  <title>wellinformed — Open Graph square / LinkedIn carousel cover</title>
./docs/assets/og-square.svg:24:    <text x="56" y="22" class="og-mark" font-size="32">wellinformed</text>
./docs/assets/og-square.svg:63:  <text class="og-meta" font-size="20" x="96" y="1140">github.com/twocirclestudios/wellinformed</text>
./src/application/report.ts:120:  lines.push(`# wellinformed report — ${data.room}`);
./docs/assets/logo-mark.svg:1:<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64" width="64" height="64" role="img" aria-label="wellinformed mark">
./docs/assets/logo-mark.svg:2:  <title>wellinformed — 6-peer mesh mark</title>
./docs/assets/og-portrait.svg:1:<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1080 1920" width="1080" height="1920" role="img" aria-label="wellinformed — The globally accumulating knowledge network. For AI agents and humans.">
./docs/assets/og-portrait.svg:2:  <title>wellinformed — vertical share card (Stories / TikTok / IG portrait)</title>
./docs/assets/og-portrait.svg:24:    <text x="64" y="28" class="og-mark" font-size="36">wellinformed</text>
./docs/assets/og-portrait.svg:66:  <text class="og-meta" font-size="24" x="96" y="1820">github.com/twocirclestudios/wellinformed</text>
./docs/assets/logo-monochrome.svg:1:<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 360 64" width="360" height="64" role="img" aria-label="wellinformed">
./docs/assets/logo-monochrome.svg:2:  <title>wellinformed — monochrome wordmark for non-brand placements</title>
./docs/assets/logo-monochrome.svg:32:  <text x="84" y="42" class="wm-text">wellinformed</text>
...
./docs/PROJECT-PLAN-AKASHIK.md:9:the brand is now Akashik — the codebase is still `wellinformed` and
...
./docs/PROJECT-PLAN-AKASHIK.md:27:Codebase is still named `wellinformed`; brand is Akashik; the
...
./docs/PROJECT-PLAN-AKASHIK.md:100:    `src/daemon/ipc.ts` written to `~/.wellinformed/ipc.token`
./docs/PROJECT-PLAN-AKASHIK.md:126:pigeonholes Akashik as 'niche debugging tool' — but guarantees
./docs/PROJECT-PLAN-AKASHIK.md:134:  Each artifact saved via `wellinformed save --type research` from
./docs/PROJECT-PLAN-AKASHIK.md:158:  90-second video walkthrough + `npm install -g wellinformed` +
./docs/PROJECT-PLAN-AKASHIK.md:159:  `wellinformed share` to join the pilot rooms. Success criterion:
./docs/PROJECT-PLAN-AKASHIK.md:187:  and expose `wellinformed quarantine list / replay` so operators
./docs/PROJECT-PLAN-AKASHIK.md:249:- **No premature rebrand-PR (`wellinformed → akashik` package
./docs/PROJECT-PLAN-AKASHIK.md:251:  participants install `wellinformed` and read about Akashik;
./docs/PROJECT-PLAN-AKASHIK.md:332:  `wellinformed metrics fallback --room <room>` returns a per-day
...
./docs/assets/og-card.svg:1:<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1200 630" width="1200" height="630" role="img" aria-label="wellinformed — The globally accumulating knowledge network. For AI agents and humans.">
./docs/assets/og-card.svg:2:  <title>wellinformed — Open Graph social card</title>
./docs/assets/og-card.svg:27:    <text x="56" y="22" class="og-mark" font-size="32">wellinformed</text>
./docs/assets/og-card.svg:39:  <text class="og-meta" font-size="18" x="72" y="572">github.com/twocirclestudios/wellinformed</text>
./docs/protocol/PROTOCOL-QUALITY-QUESTIONS.md:4:parts of wellinformed that decide whether peer knowledge is good enough for an
./docs/protocol/PROTOCOL-QUALITY-QUESTIONS.md:14:wellinformed becomes a serious agent-memory protocol.
./docs/protocol/PROTOCOL-QUALITY-QUESTIONS.md:201:2. wellinformed retrieves local + peer candidates.
./docs/protocol/PROTOCOL-QUALITY-QUESTIONS.md:241:  "room": "wellinformed-dev",
./docs/protocol/PROTOCOL-QUALITY-QUESTIONS.md:521:wellinformed should treat this as a protocol problem:
./docs/protocol/PROTOCOL-QUALITY-QUESTIONS.md:594:- Can the agent ask wellinformed for "coverage" instead of "search"?
./docs/protocol/PROTOCOL-QUALITY-QUESTIONS.md:814:If wellinformed works, peers are not just caches. They become sources of
./docs/protocol/PROTOCOL-QUALITY-QUESTIONS.md:1185:- Which metrics should be visible in `wellinformed stats`?
./docs/architecture/claude-obsidian-parity.md:15:| **Skill trigger vocabulary** (rich frontmatter trigger phrases: "ingest this url", "what do you know about", "/save", "find orphans", …) | `.claude/skills/wellinformed/SKILL.md` has 7 triggers | **Minor.** Our skill is wired but the vocabulary is narrow; natural-sounding phrases like "save this to wellinformed" don't activate. |
./docs/architecture/claude-obsidian-parity.md:35:**Add:** `wellinformed/hot-cache` — a new domain concept. After each tick, generate a ~500-word summary of: (a) newest N nodes, (b) most-queried rooms this session, (c) pending ingests, (d) 3-5 most surprising cross-references. Store at `~/.wellinformed/hot.md` and include in SessionStart hook output.
./docs/architecture/claude-obsidian-parity.md:37:**Why:** Session continuity is the single biggest daily UX improvement. Today Claude walks into a wellinformed session with no context. With a hot cache, the first thing Claude reads is an actionable recency digest.
./docs/architecture/claude-obsidian-parity.md:43:**Add:** `wellinformed lint [--room R] [--fix]` — graph-health checker with the 8 categories from claude-obsidian's wiki-lint, plus P2P-specific ones (orphaned remote nodes, stale shared-room manifest, secret-pattern drift since last audit).
./docs/architecture/claude-obsidian-parity.md:51:**Add:** `wellinformed save --room R` — called from a Claude session, takes the last N assistant messages + the user question, produces a typed node (synthesis/concept/decision), writes into the chosen room. Complements auto-ingest by capturing *distillations*, not transcripts.
./docs/architecture/claude-obsidian-parity.md:59:**Add:** `~/.wellinformed/research-program.md` — a YAML+markdown config read by `discover-loop` to parameterise source preferences, min-confidence gate, round depth, stop conditions. Mirrors claude-obsidian's `program.md`.
./docs/architecture/claude-obsidian-parity.md:79:**Add:** Expand `.claude/skills/wellinformed/SKILL.md` frontmatter to include trigger phrases matching claude-obsidian's voice: "save this to X", "ingest this url", "what do you know about", "find orphans", etc.
...
./src/telegram/capture.ts:60:    return 'No rooms configured. Run `wellinformed init` first.';
...
./docs/research/energy-based-contradiction-detection.md:127:2. **User feedback on `wellinformed contradictions resolve --prefer
...
./docs/architecture/ADR-002-v4-agent-brain.md:1:# ADR-002 — wellinformed v4 Agent Brain design decisions
./docs/architecture/ADR-002-v4-agent-brain.md:36:- No structured types on the wire — JSON only. Acceptable because the handler registry is small and version-bumped via the protocol ID (`/wellinformed/search/2.0.0`).
./docs/architecture/ADR-002-v4-agent-brain.md:47:- Server-side coalescing in `wellinformed-rs/src/bin/embed_server.rs` (tokio + async channel + thread pool). High effort, requires a Rust async refactor.
./docs/architecture/ADR-002-v4-agent-brain.md:92:**Chosen:** `wellinformed consolidate run <room>` is an explicit CLI command, run by an operator (or a cron job, or the daemon's tick loop in v4.1). Not auto-run on a timer in v4.0.
./docs/architecture/ADR-002-v4-agent-brain.md:138:**Chosen:** `${WELLINFORMED_HOME}/wellinformed.lock` — POSIX exclusive-create file with `{pid, owner, timestamp}` content. Daemon refreshes every 20s; consolidate waits up to 30s.
./docs/architecture/ADR-002-v4-agent-brain.md:151:- **Operator-debuggable** — `cat ~/.wellinformed/wellinformed.lock` shows who holds it
./src/telegram/commands.ts:104:    '*wellinformed status*',
./src/telegram/commands.ts:128:    '*wellinformed commands:*',
./docs/demo.tape:1:# wellinformed CLI demo — accumulate · share · stream
./docs/demo.tape:4:# Thesis: wellinformed is a COMMUNITY network. Everyone who runs it
./docs/demo.tape:11:#   1. Thesis banner       — what wellinformed does in one sentence
./docs/demo.tape:44:Type `echo "▸ wellinformed — accumulate · share · stream knowledge into every LLM session"`
./docs/demo.tape:57:Type "wellinformed peer status"
./docs/demo.tape:70:Type@28ms `wellinformed ask "vector search sqlite" --k 3`
./docs/demo.tape:80:Type "# federate — every wellinformed peer, anywhere, contributes a shard"
./docs/demo.tape:83:Type@28ms `wellinformed ask "P2P knowledge graph" --k 3 --peers`
./docs/demo.tape:96:Type "claude mcp get wellinformed"
./src/telegram/bot.ts:2: * Telegram bot — long-polling client for wellinformed.
./src/telegram/bot.ts:47:    return errAsync(GE.readError('telegram', 'bot not configured — run wellinformed telegram setup'));
./docs/memory-stack.svg:1:<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1400 1020" fill="none" role="img" aria-label="Why wellinformed compounds — same Claude session, two different Fridays">
./docs/memory-stack.svg:94:  <!-- ─── RIGHT COLUMN — with wellinformed ────────────────── -->
./docs/memory-stack.svg:105:      <text x="840" y="227" font-size="13" font-family="'JetBrains Mono','SF Mono','Menlo',monospace" fill="#b4b4c0">PreToolUse hook fires · `wellinformed ask` runs before Claude reaches for a tool</text>
...
./docs/research/beat-the-competitors-retrieval-plan.md:187:- **Bus factor on `wellinformed-rs`** — Rust ARM cross-compile is specialized knowledge; consider TypeScript-only fallback path.
...
./docs/architecture/V4-PROTOCOL.md:1:# wellinformed v4 — Agent Brain Protocol
./docs/architecture/V4-PROTOCOL.md:4:**Status:** Reference implementation shipped in wellinformed v4.x; spec stabilising ahead of v4.0 tag
./docs/architecture/V4-PROTOCOL.md:28:client → daemon:  {"id": <number>, "cmd": "ask", "args": ["--room", "wellinformed-dev", "p2p memory"]}
./docs/architecture/V4-PROTOCOL.md:45:`bin/wellinformed.js` checks for the socket file BEFORE importing `dist/cli/index.js`. If present and the command is in the delegatable set (currently `ask`, `stats`), the request is forwarded over IPC. Otherwise the CLI takes the normal spawn path.
./docs/architecture/V4-PROTOCOL.md:98:Episodic memory entries (raw session transcripts, ingested chat logs, observed events) accumulate linearly. A brain compresses them via overnight replay into semantic schemas. wellinformed v4 ships this as a CLI primitive plus a graph-node schema.
./docs/architecture/V4-PROTOCOL.md:175:`${WELLINFORMED_HOME}/wellinformed.lock` — POSIX exclusive-create file.
./docs/architecture/V4-PROTOCOL.md:248:bin/wellinformed.js                  Client-side IPC delegation (Phase 1)
./docs/architecture/V4-PROTOCOL.md:285:- **v4.1 — Native binary client** (Rust `wellinformed-cli` that speaks the IPC protocol directly, bypassing Node boot). Target: warm-hit latency 100ms → 15ms.
...
./docs/README.md:1:# wellinformed — documentation index
./docs/README.md:18:What wellinformed is, why it exists, what's shipped, what's planned.
./docs/README.md:21:- [`VISION.md`](product/VISION.md) — the agent-memory protocol problem and where wellinformed sits in it.
...
./src/daemon/job-queue.ts:4: * Owned by the daemon process (one instance per `wellinformed daemon
./src/daemon/job-queue.ts:11: *   - Sequential output is easier to read in `wellinformed jobs watch`.
./src/daemon/job-queue.ts:13: * Persistence: queue + completed log written to ~/.wellinformed/jobs.json
...
./src/daemon/ipc.ts:5: * Why: a cold `wellinformed ask` pays ~700 ms of Node+tsx boot + ~40 ms
./docs/index.html:6:  <title>wellinformed — the network before the web</title>
./docs/index.html:12:  <meta property="og:title"          content="wellinformed — the globally accumulating knowledge network" />
./docs/index.html:14:  <meta property="og:url"            content="https://saharbarak.github.io/wellinformed/" />
./docs/index.html:15:  <meta property="og:image"          content="https://saharbarak.github.io/wellinformed/assets/og-card.png" />
./docs/index.html:20:  <meta name="twitter:title"         content="wellinformed — the globally accumulating knowledge network" />
./docs/index.html:22:  <meta name="twitter:image"         content="https://saharbarak.github.io/wellinformed/assets/og-card.png" />
./docs/index.html:596:    /* ───── hero-compare — side-by-side: vanilla Claude vs Claude+wellinformed ───── */
./docs/index.html:693:       window, so wellinformed falls back to research and writes the
./docs/index.html:1971:        <a href="#" class="brand" aria-label="wellinformed home">
./docs/index.html:1973:          wellinformed
./docs/index.html:1983:        <a class="nav-cta" href="https://github.com/SaharBarak/wellinformed" rel="noopener">
./docs/index.html:1984:          SaharBarak/wellinformed
./docs/index.html:2026:            <pre class="install-code"><code id="install-code">$ npm install -g wellinformed</code><button id="install-copy" type="button" aria-label="Copy install command">
./docs/index.html:2041:            <a class="btn btn-primary" href="https://github.com/SaharBarak/wellinformed">
./docs/index.html:2057:        <figure class="hero-compare" aria-label="Side-by-side: vanilla Claude vs Claude with wellinformed answering the same question">
./docs/index.html:2080:              <span class="cmp-agent js-agent" aria-live="polite">claude code</span>&nbsp;+ wellinformed
./docs/index.html:2085:              <div class="cmp-step cmp-hit" data-slot="w-hit">&#9670; wellinformed: 11&nbsp;ms &middot; 3 cited chunks &middot; age 2d &middot; fresh</div>
./docs/index.html:2105:    <div class="try-graph-canvas" id="try-graph-canvas" aria-label="Live wellinformed knowledge-graph view"></div>
./docs/index.html:2147:        <h2>How wellinformed hooks into your workflow.</h2>
./docs/index.html:2156:            A <code>UserPromptSubmit</code> hook runs <code>wellinformed ask</code> on every message &mdash; top graph matches inject as <code>additionalContext</code> before the LLM reads a token.
./docs/index.html:2177:claude mcp add --scope user wellinformed -- wellinformed mcp
./docs/index.html:2178:wellinformed claude install
./docs/index.html:2197:          <span><b>wellinformed</b></span>
./docs/index.html:2222:      <pre data-reveal style="margin-top: 2rem; background: var(--ink-900); color: #e8e8ed; padding: 1rem 1.2rem; border-radius: 8px; font-family: var(--mono); font-size: 0.85rem; max-width: 720px; overflow-x: auto;"><code>$ wellinformed bench beir-scifact   # one command, full reproduce</code></pre>
./docs/index.html:2292:        <img src="assets/manifesto-illustration.svg" alt="Cooperative-network lineage: Napster, eMule, BitTorrent, IPFS, wellinformed" style="width: 100%; height: auto; display: block;" />
./docs/index.html:2298:        With service bills spiraling out of control and the stack becoming more unsustainable by the quarter, <b>wellinformed enters to help us work together again</b> &mdash; like we did back when the boundaries were technological. Today the boundaries are economic.
./docs/index.html:2317:        <pre class="code finale-code"><code>$ npm install -g wellinformed
./docs/index.html:2318:$ wellinformed init</code></pre>
./docs/index.html:2320:          <a class="btn btn-primary" href="https://github.com/SaharBarak/wellinformed">
./docs/index.html:2326:          <a class="btn btn-ghost" href="https://github.com/SaharBarak/wellinformed/discussions">
./docs/index.html:2334:          <a href="https://github.com/SaharBarak/wellinformed/stargazers">
./docs/index.html:2335:            <img src="https://img.shields.io/github/stars/SaharBarak/wellinformed?style=social" alt="GitHub stars" />
./docs/index.html:2337:          <a href="https://github.com/SaharBarak/wellinformed/releases">
./docs/index.html:2338:            <img src="https://img.shields.io/github/v/release/SaharBarak/wellinformed?include_prereleases&label=release&color=34d399" alt="release" />
./docs/index.html:2352:          <a href="#" class="footer-mark" aria-label="wellinformed home">
./docs/index.html:2354:            <span>wellinformed</span>
./docs/index.html:2369:            <li><a href="https://github.com/SaharBarak/wellinformed">GitHub</a></li>
./docs/index.html:2370:            <li><a href="https://github.com/SaharBarak/wellinformed/blob/main/README.md">README</a></li>
./docs/index.html:2371:            <li><a href="https://github.com/SaharBarak/wellinformed/issues">Issues</a></li>
./docs/index.html:2372:            <li><a href="https://github.com/SaharBarak/wellinformed/blob/main/LICENSE">MIT License</a></li>
./docs/index.html:2379:            <li><code>npm install -g wellinformed</code></li>
./docs/index.html:2380:            <li><code>brew install saharbarak/wellinformed/wellinformed</code></li>
./docs/index.html:2381:            <li><code>npx wellinformed init</code></li>
./docs/index.html:2402:          <a href="https://github.com/SaharBarak/wellinformed">github.com/SaharBarak/wellinformed</a>
./docs/index.html:2449:        npm:      '$ npm install -g wellinformed',
./docs/index.html:2450:        homebrew: '$ brew install saharbarak/wellinformed/wellinformed',
./docs/index.html:2451:        npx:      '$ npx wellinformed init',
./docs/index.html:2551:            hit:    '◆ wellinformed: 11 ms · 3 cited chunks · age 2d · fresh',
./docs/index.html:2581:            hit:    '◆ wellinformed: 14 ms · 1 cached chunk · age 31d · STALE (> 7d)',
./docs/index.html:3049:        // Each scenario sources its answer from a specific wellinformed room.
./docs/index.html:3430:          ['code:wi-recall', 'code · wellinformed/src/recall.rs'],
./docs/index.html:3431:          ['code:wi-federated', 'code · wellinformed/src/net/federated.rs'],
./docs/index.html:3432:          ['code:wi-hooks', 'code · wellinformed/.claude/hooks/wellinformed-hook.sh'],
./docs/architecture/V3-PROTOCOL.md:1:# wellinformed v3 — P2P Memory Protocol
./docs/architecture/V3-PROTOCOL.md:4:**Status:** Reference implementation shipped in wellinformed v3.x; spec stabilising ahead of v3.0 tag
./docs/architecture/V3-PROTOCOL.md:15:**wellinformed v3** defines a protocol so that:
./docs/architecture/V3-PROTOCOL.md:22:This spec defines the wire primitives. The reference implementation is the `wellinformed` TypeScript codebase; the spec is portable to any language (Rust, Go, Python, Swift).
./docs/architecture/V3-PROTOCOL.md:38:**Why did:key over did:web, did:ion, did:plc:** wellinformed needs zero-registry offline verifiability. Any peer can decode a did:key in microseconds without a network round-trip. Composable DID methods (did:web for human-readable names, did:plc for AT Protocol interop) are a v3.1 extension; the core protocol only requires did:key.
./docs/architecture/V3-PROTOCOL.md:49:wellinformed-auth:v1:<device_id>:<hex(device_public_key)>:<authorized_at_ISO8601>
./docs/architecture/V3-PROTOCOL.md:91:wellinformed-sig:v1:<device_id>:<signed_at>:<canonical_json(payload)>
./docs/architecture/V3-PROTOCOL.md:96:Domain separation: the `wellinformed-auth:v1:` and `wellinformed-sig:v1:` prefixes prevent cross-protocol replay. A valid authorization signature cannot be re-presented as a payload signature.
./docs/architecture/V3-PROTOCOL.md:104:   recompute `wellinformed-auth:v1:<device_id>:<hex(device_pub)>:<authorized_at>`,
./docs/architecture/V3-PROTOCOL.md:107:   recompute `wellinformed-sig:v1:<device_id>:<signed_at>:<canonical_json(payload)>`,
./docs/architecture/V3-PROTOCOL.md:134:wellinformed v3 ships **linear bridges** — for each supported encoder pair (A, B), a matrix `W_{A→B} ∈ R^{d_B × d_A}` such that `bridge(v_A) = L2_normalize(W · v_A)` is approximately equivalent to embedding the original text in encoder B.
./docs/architecture/V3-PROTOCOL.md:197:/wellinformed/capabilities/1.0.0 → {
./docs/architecture/V3-PROTOCOL.md:220:- `/wellinformed/search/2.0.0` — one-shot request/response semantic search
./docs/architecture/V3-PROTOCOL.md:221:- `/wellinformed/touch/1.0.0` — asymmetric public-room pull
./docs/architecture/V3-PROTOCOL.md:222:- `/wellinformed/share/2.0.0` — bidirectional CRDT room sync (Y.js)
./docs/architecture/V3-PROTOCOL.md:223:- `/wellinformed/save/1.0.0` — signed note append
./docs/architecture/V3-PROTOCOL.md:224:- `/wellinformed/capabilities/1.0.0` — capability exchange
./docs/architecture/V3-PROTOCOL.md:305:- Ed25519 sign/verify on the two domain-separation tags (`wellinformed-auth:v1:`, `wellinformed-sig:v1:`)
./docs/architecture/V3-PROTOCOL.md:310:The reference suite is `tests/identity.test.ts` + `tests/identity-lifecycle.test.ts` + `tests/identity-bridge.test.ts` (38 tests total) in the wellinformed repo.
./docs/architecture/V3-PROTOCOL.md:320:- **CLI**: `wellinformed identity {init|show|rotate|export|import}`
./docs/architecture/V3-PROTOCOL.md:343:This protocol specification is CC-BY-4.0. The reference implementation is MIT. Cross-model bridge matrices distributed by the wellinformed project are CC0.
./docs/architecture/V3-PROTOCOL.md:349:All numbers are measured on commodity CPU hardware (Apple Silicon, M-series). Reproduction scripts are in the wellinformed repo at `scripts/bench-*.mjs`.
./docs/architecture/ADR-001-v3-memory-protocol.md:1:# ADR-001 — wellinformed v3 memory protocol design decisions
./docs/architecture/ADR-001-v3-memory-protocol.md:12:wellinformed v2 established measured retrieval quality (Phase 25: 75.22% NDCG@10 on SciFact via Rust-fastembed bge-base + hybrid — within ~1.5pt of GPU reranker ceiling). The v3 milestone moves from "a retrieval engine" to "a protocol" — with the specific goal of unlocking cross-model, cross-device, cross-agent portable memory for the free-LLM world.
./docs/architecture/ADR-001-v3-memory-protocol.md:160:- Incremental adoption: `/wellinformed/search/2.0.0` (signed) runs alongside `/wellinformed/search/1.0.0` (unsigned) during migration
./docs/architecture/ADR-001-v3-memory-protocol.md:181:- Every memory entry wellinformed emits can be cryptographically attributed to a user DID
...
./docs/p2p/P2P-VISION.md:1:# P2P Knowledge Graph — wellinformed v2.0 Vision
./docs/p2p/P2P-VISION.md:5:Every developer running wellinformed has a local knowledge graph. Right now these graphs are isolated — your homelab research doesn't connect to mine.
./docs/p2p/P2P-VISION.md:7:**v2.0 makes them connected.** A peer-to-peer network where wellinformed nodes discover each other, share graph fragments, and build a collective knowledge layer that's bigger than any single user's research.
./docs/p2p/P2P-VISION.md:30:- Or manually add peers: `wellinformed peer add <address>`
./docs/p2p/P2P-VISION.md:41:- Federated search: `wellinformed ask "vector search" --peers` searches across all connected graphs
./docs/p2p/P2P-VISION.md:90:A team of 5 researchers each tracks different domains. P2P wellinformed connects their graphs. When researcher A indexes a paper about "efficient attention", researcher B (tracking "GPU optimization") gets a tunnel notification: "your GPU optimization connects to A's attention paper."
./docs/p2p/P2P-VISION.md:96:At a conference, attendees run wellinformed in P2P mode. Their graphs auto-discover via local network. The collective graph of 100 attendees, each with 500 nodes, creates a 50K-node searchable knowledge base spanning every talk, paper, and conversation.
./docs/p2p/P2P-VISION.md:123:wellinformed goes from "your personal research memory" to "a collective intelligence network for developers." Every peer makes the network smarter. The graph grows faster than any individual could build it.
...
./docs/marketing/positioning-v2.1.md:204:- **Auto-prefetch** — the Claude Code hook runs `wellinformed ask`
./docs/marketing/positioning-v2.1.md:221:federate. They're each their own fragmentation. Akashik is the
./docs/marketing/positioning-v2.1.md:234:  Domain-separation tags (`wellinformed-auth:v1:` vs
./docs/marketing/positioning-v2.1.md:235:  `wellinformed-sig:v1:`) prevent replay of authorization signatures
...
./docs/marketing/positioning-v2.1.md:288:> `git clone https://github.com/SaharBarak/wellinformed && npm i && bash scripts/bootstrap.sh`
...
./docs/marketing/positioning-v2.1.md:353:- Proof: screenshot of `wellinformed peer list` with real peer IDs.
./docs/marketing/positioning-v2.1.md:366:- Proof: `wellinformed identity show` output — real DID,
...
./docs/research/octopus-discover/round-5-2026-05-26/probes/claude-sonnet-5.md:172:- Index the **MemGPT/Letta paper** (arxiv:2310.08560) with a comparison note: "Letta requires their agent runtime; wellinformed works with Claude Code, Cursor, and Cline today."
./docs/research/octopus-discover/round-5-2026-05-26/probes/claude-sonnet-5.md:177:- **Days 8–14**: Post "Show HN: wellinformed — P2P semantic cache for AI agent memory" with the FedComp-LoCoMo chart (even a synthetic one from the simulation). Link the terminal video. HN is the fastest path to 50 GitHub stars from technical users.
...
./docs/research/octopus-discover/round-5-2026-05-26/probes/claude-sonnet-5.md:232:The cold-start problem is real and the argument is correct that federation provides zero benefit to a single isolated peer. Our answer is that *local value is already sufficient on day zero* — `wellinformed save` + `wellinformed ask` already beats a bare LLM context window for a single user on the LME-S benchmark (R@5=0.9202), and that value is why someone installs it. Federation is a multiplier on existing value, not a prerequisite for it. The coordination burden is also lower than Slack or GitHub Discussions because it's *implicit*: when two developers in the same OSS project both install wellinformed, they automatically share knowledge through the `toolshed` system room (which is always-on and P2P-shared) without any explicit "join this workspace" action. The social coordination is replaced by ambient proximity in the codebase — the same mechanism that makes pull request visibility work without users needing to subscribe to each other. The honest concession: this argument becomes false if the `toolshed` room and oracle gossip are not reliably working on first install. The current state (federation silently downgrades to dense-only, oracle gossip end-to-end test is in `phase39.oracle-gossip-e2e.test.ts` but not shipped) means the argument *is* currently true. Fixing that is the same work as Q1's answer.
./src/daemon/ipc-handlers.ts:96:  if (!query) return 'missing query — usage: wellinformed ask "your question" [--room R] [--k N] [--json]';
./src/daemon/ipc-handlers.ts:209:    const stdout = 'no results found. try a broader query or run `wellinformed trigger` to index content first.\n';
./src/daemon/ipc-handlers.ts:223:    lines.push(`# wellinformed: "${r.query}" matches entity ${e.id}`);
./src/daemon/ipc-handlers.ts:280: * (wellinformed cache-stats) prints this for operators monitoring
./src/daemon/ipc-handlers.ts:341: * `wellinformed jobs submit <kind> <...args>` over IPC.
./src/daemon/ipc-handlers.ts:350: * spam — keeps `wellinformed this | xargs` style scripting easy).
./src/daemon/ipc-handlers.ts:441: *   $ wellinformed metrics
./src/daemon/ipc-handlers.ts:468: * buildIpcHandlers(). Used by bin/wellinformed.js to know whether to
./docs/marketing/x-launch-posts.md:1:# X/Twitter Launch Posts — Akashik
./docs/marketing/x-launch-posts.md:12:Open source: github.com/SaharBarak/wellinformed
./docs/marketing/x-launch-posts.md:26:1. `wellinformed init` — pick your research topics
./docs/marketing/x-launch-posts.md:27:2. `wellinformed trigger` — fetch from ArXiv, HN, RSS
./docs/marketing/x-launch-posts.md:28:3. `wellinformed index` — index your codebase
./docs/marketing/x-launch-posts.md:29:4. `wellinformed claude install` — hook into Claude Code
./docs/marketing/x-launch-posts.md:34:The key command is `wellinformed claude install`.
...
./docs/marketing/x-launch-posts.md:88:wellinformed init
./docs/marketing/x-launch-posts.md:89:wellinformed trigger --room homelab
./docs/marketing/x-launch-posts.md:90:wellinformed index
./docs/marketing/x-launch-posts.md:91:wellinformed claude install
./docs/marketing/x-launch-posts.md:99:I typed `wellinformed discover --room wellinformed-dev --auto` and it found:
./docs/marketing/x-launch-posts.md:113:Akashik changes that.
./docs/marketing/x-launch-posts.md:115:github.com/SaharBarak/wellinformed
...
./docs/marketing/x-launch-posts.md:131:github.com/SaharBarak/wellinformed
./docs/marketing/growth-sources-plan.md:89:wellinformed discover-loop --room homelab --max-iterations 3
./src/daemon/loop.ts:11: * PID file at `~/.wellinformed/daemon.pid` for lifecycle management.
./src/daemon/loop.ts:14: * by `wellinformed daemon start`. It logs to
./src/daemon/loop.ts:15: * `~/.wellinformed/daemon.log` and exits cleanly on SIGTERM.
./src/daemon/loop.ts:366:  // (i.e. they have run `wellinformed peer status` or `peer add` at least once).
./src/daemon/loop.ts:550:              daemonLog(deps.homePath, `share sync registered: /wellinformed/share/1.0.0`);
./src/daemon/loop.ts:617:              daemonLog(deps.homePath, `search protocol registered: /wellinformed/search/1.0.0`);
./src/daemon/loop.ts:632:              daemonLog(deps.homePath, `recall protocol registered: /wellinformed/recall/1.0.0`);
./src/daemon/loop.ts:654:              daemonLog(deps.homePath, `touch protocol registered: /wellinformed/touch/1.0.0`);
./src/daemon/loop.ts:658:            // /wellinformed/oracle/1.0.0 topic so inbound questions and
./src/daemon/loop.ts:678:              daemonLog(deps.homePath, `oracle pubsub subscribed: /wellinformed/oracle/1.0.0`);
./src/daemon/loop.ts:720:              daemonLog(deps.homePath, `search-gossip subscribed: /wellinformed/search/1.0.0`);
./src/daemon/job-runner.ts:9: * `wellinformed jobs list`. Errors throw — the queue catches them and
./src/daemon/job-runner.ts:200: * Project ingest — the four ephemeral descriptors that `wellinformed
./src/daemon/job-runner.ts:203: * NOT persisted to sources.json (mirrors `wellinformed index`).
...
./docs/marketing/storybrand-messaging-draft.md:389:called `wellinformed` as of this writing. The brand-marketing
...
./docs/marketing/storybrand-messaging-draft.md:395:  PROJECT.md, source code): **wellinformed** (for now)
./docs/marketing/storybrand-messaging-draft.md:396:- Migration plan to rename `wellinformed → akashik` in the
./docs/marketing/storybrand-messaging-draft.md:402:| | Pass 1 — Personal | Pass 2 — Small teams | Pass 3 — OSS community (Akashik) |
./docs/p2p/peer-reputation-load-spreading.md:28:Two facts about the existing wellinformed code make load-spreading
./docs/p2p/peer-reputation-load-spreading.md:31:1. **Touch already replicates chunks.** When peer A `wellinformed touch`-es a
./docs/p2p/peer-reputation-load-spreading.md:33:   `_wellinformed_source_peer: B` provenance. A is now a secondary source for
./docs/p2p/peer-reputation-load-spreading.md:62:**What it does.** `wellinformed touch --peer B --label "lemlist pricing"` pulls
./docs/p2p/peer-reputation-load-spreading.md:63:the chunk and writes it to A's local `graph.json` with `_wellinformed_source_peer: B`.
./docs/p2p/peer-reputation-load-spreading.md:73:  flag to `wellinformed ask --peers` that, on a high-confidence federated
./docs/p2p/peer-reputation-load-spreading.md:175:*touched* from peer B (carrying `_wellinformed_source_peer: B`), the
./docs/p2p/peer-reputation-load-spreading.md:194:local update path to credit `_wellinformed_source_peer` chains).
./docs/p2p/peer-reputation-load-spreading.md:214:**Files:** new wire protocol `/wellinformed/seed/1.0.0`. Substantial — Phase 3
./docs/p2p/peer-reputation-load-spreading.md:308:Item (3) is the rep-update path crediting `_wellinformed_source_peer` chains.
./docs/product/ROADMAP.md:1:# Akashik — Roadmap
./docs/product/ROADMAP.md:19:Pluggable Source port with four adapters: generic_rss, arxiv, hn_algolia, generic_url. RSS 2.0 + Atom normaliser. Recursive paragraph chunker. Content-hash (sha256) dedup on re-runs. `wellinformed trigger [--room R]` and `wellinformed sources list|add|remove|enable|disable`.
./docs/product/ROADMAP.md:25:9 tools exposed over stdio via @modelcontextprotocol/sdk: search, ask, get_node, get_neighbors, list_rooms, find_tunnels, sources_list, trigger_room, graph_stats. `wellinformed mcp start` — auto-spawned by Claude Code via the plugin manifest.
./docs/product/ROADMAP.md:29:**Commands:** `wellinformed init`, `wellinformed room list|create|switch|current`
./docs/product/ROADMAP.md:35:- `src/infrastructure/rooms-config.ts` — Room registry at `~/.wellinformed/rooms.json`
./docs/product/ROADMAP.md:44:**Commands:** `wellinformed ask "<query>"`, `wellinformed report [date] [--room R]`
./docs/product/ROADMAP.md:52:- Report persistence at `~/.wellinformed/reports/<room>/<date>.md`
./docs/product/ROADMAP.md:57:**Commands:** `wellinformed daemon start|stop|status`, `wellinformed discover [--room R]`
./docs/product/ROADMAP.md:66:- Config integration: read daemon settings from `~/.wellinformed/config.yaml` (add `yaml` dep)
./docs/product/ROADMAP.md:71:**Commands:** `wellinformed telegram setup|test|capture-start|digest-test`
./src/daemon/file-watcher.ts:8: * (.git, node_modules, dist, .wellinformed, *.swp, *.tmp).
./src/daemon/file-watcher.ts:45:  /\/\.wellinformed\//,
./src/daemon/file-watcher.ts:202:  // sessions are universal, not opted-in via `wellinformed this`.
./docs/marketing/SITE-REDESIGN-SPEC.md:1:# Akashik — landing page redesign spec
./docs/marketing/SITE-REDESIGN-SPEC.md:17:- **GitHub repo (target):** `github.com/twocirclestudios/wellinformed`
./docs/marketing/SITE-REDESIGN-SPEC.md:18:  (currently `github.com/SaharBarak/wellinformed`; migration pending)
./docs/marketing/SITE-REDESIGN-SPEC.md:22:  git clone https://github.com/twocirclestudios/wellinformed.git
./docs/marketing/SITE-REDESIGN-SPEC.md:23:  cd wellinformed
./docs/marketing/SITE-REDESIGN-SPEC.md:35:  npx -y wellinformed init
./docs/marketing/SITE-REDESIGN-SPEC.md:43:  $ git clone https://github.com/twocirclestudios/wellinformed.git
./docs/marketing/SITE-REDESIGN-SPEC.md:44:  $ cd wellinformed && npm install && npm run bootstrap
./docs/marketing/SITE-REDESIGN-SPEC.md:47:**Migration touchpoints — every reference to `SaharBarak/wellinformed`
./docs/marketing/SITE-REDESIGN-SPEC.md:48:must flip to `twocirclestudios/wellinformed` when execution starts.**
...
./docs/marketing/SITE-REDESIGN-SPEC.md:134:│   ║      twocirclestudios/wellinformed.git   ║                    │
./docs/marketing/SITE-REDESIGN-SPEC.md:135:│   ║  $ cd wellinformed && npm install &&     ║                    │
./docs/marketing/SITE-REDESIGN-SPEC.md:181:$ git clone https://github.com/twocirclestudios/wellinformed.git
./docs/marketing/SITE-REDESIGN-SPEC.md:182:$ cd wellinformed && npm install && npm run bootstrap
./docs/marketing/SITE-REDESIGN-SPEC.md:185:$ npx -y wellinformed init
./docs/marketing/SITE-REDESIGN-SPEC.md:188:$ brew install twocirclestudios/wellinformed/wellinformed
./docs/marketing/SITE-REDESIGN-SPEC.md:195:  page; will be `wellinformed peer list` JSON output until built)
./docs/marketing/SITE-REDESIGN-SPEC.md:199:  links to `https://github.com/twocirclestudios/wellinformed`)
...
./docs/marketing/SITE-REDESIGN-SPEC.md:323:  git:      '$ git clone https://github.com/twocirclestudios/wellinformed.git\n$ cd wellinformed && npm install && npm run bootstrap',
./docs/marketing/SITE-REDESIGN-SPEC.md:324:  npm:      '$ npx -y wellinformed init',
./docs/marketing/SITE-REDESIGN-SPEC.md:325:  homebrew: '$ brew install twocirclestudios/wellinformed/wellinformed',
./docs/marketing/SITE-REDESIGN-SPEC.md:422:     href="https://github.com/twocirclestudios/wellinformed">
./docs/marketing/SITE-REDESIGN-SPEC.md:429:  <a class="btn btn-primary" href="https://github.com/twocirclestudios/wellinformed">
...
./docs/marketing/SITE-REDESIGN-SPEC.md:684:> a message, the hook runs `wellinformed ask` against your local
...
./docs/marketing/SITE-REDESIGN-SPEC.md:713:claude mcp add --scope user wellinformed -- wellinformed mcp
./docs/marketing/SITE-REDESIGN-SPEC.md:714:wellinformed claude install
./docs/marketing/SITE-REDESIGN-SPEC.md:742:│   $ git clone https://github.com/twocirclestudios/wellinformed.git│
./docs/marketing/SITE-REDESIGN-SPEC.md:743:│   $ cd wellinformed && npm install && npm run bootstrap           │
./docs/marketing/SITE-REDESIGN-SPEC.md:765:$ git clone https://github.com/twocirclestudios/wellinformed.git
./docs/marketing/SITE-REDESIGN-SPEC.md:766:$ cd wellinformed && npm install && npm run bootstrap
./docs/marketing/SITE-REDESIGN-SPEC.md:770:- Primary: `☆ Star on GitHub` → `https://github.com/twocirclestudios/wellinformed`
./docs/marketing/SITE-REDESIGN-SPEC.md:771:- Ghost: `Join Discussions` → `https://github.com/twocirclestudios/wellinformed/discussions`
./docs/marketing/SITE-REDESIGN-SPEC.md:779:- `shields.io/github/stars/twocirclestudios/wellinformed?style=social`
./docs/marketing/SITE-REDESIGN-SPEC.md:780:- `shields.io/github/v/release/twocirclestudios/wellinformed`
./docs/marketing/SITE-REDESIGN-SPEC.md:781:- (optional) `shields.io/npm/v/wellinformed` once published
./docs/marketing/SITE-REDESIGN-SPEC.md:794:<img src="https://raw.githubusercontent.com/SaharBarak/wellinformed/main/demo/scene-claude.gif" …>
...
./docs/marketing/SITE-REDESIGN-SPEC.md:868:- Update repo references to `twocirclestudios/wellinformed`
./docs/marketing/SITE-REDESIGN-SPEC.md:914:1. **Repo migration timing.** Is `twocirclestudios/wellinformed` live
./docs/marketing/SITE-REDESIGN-SPEC.md:916:   404 until upload) or keep `SaharBarak/wellinformed` until the move
./docs/marketing/SITE-REDESIGN-SPEC.md:939:5. **npm package name.** Will we publish as `wellinformed` or
./docs/marketing/SITE-REDESIGN-SPEC.md:940:   `@twocircle/wellinformed` (scoped)?
./docs/marketing/SITE-REDESIGN-SPEC.md:941:   - Recommended: try `wellinformed` first — short, memorable. If
./docs/marketing/SITE-REDESIGN-SPEC.md:942:     taken on npm, fall back to `@twocircle/wellinformed`.
./docs/marketing/SITE-REDESIGN-SPEC.md:960:     cheap follow-up — JSON dump of `wellinformed peer list --json`
./docs/marketing/SITE-REDESIGN-SPEC.md:965:   twocirclestudios/wellinformed/wellinformed` as one of the three
./docs/marketing/SITE-REDESIGN-SPEC.md:975:   endpoint that runs `wellinformed ask --peers --json` against the
./docs/marketing/SITE-REDESIGN-SPEC.md:979:   - Recommended: Fly.io node running `wellinformed daemon` + a thin
./docs/marketing/SITE-REDESIGN-SPEC.md:988:      ($1.94/mo) running `wellinformed daemon`. Generates a fresh
./docs/marketing/SITE-REDESIGN-SPEC.md:1085:product demo. Visitors don't read about Akashik — they query the
./docs/marketing/SITE-REDESIGN-SPEC.md:1097:  The page fires `POST /api/ask`, the server runs `wellinformed ask
./docs/marketing/SITE-REDESIGN-SPEC.md:1242:| Error      | inline message: `couldn't reach the network — try again? (or run `wellinformed ask` locally)` |
./docs/marketing/SITE-REDESIGN-SPEC.md:1281:- Runs `wellinformed ask --peers --json --k 5` server-side against the
...
./docs/marketing/BRAND-KIT.md:576:│   github.com/twocirclestudios/wellinformed   MIT · 2026 │
./docs/marketing/BRAND-KIT.md:581:- Brand mark: `Akashik` 32 px Outfit 700, `--accent`
./docs/marketing/BRAND-KIT.md:594:<meta property="og:image" content="https://wellinformed.dev/og-card.png">
./docs/marketing/BRAND-KIT.md:598:<meta name="twitter:image" content="https://wellinformed.dev/og-card.png">
...
./docs/marketing/IMAGEGEN-FRONTEND-WEB.md:368:│   │ claude mcp add --scope user wellinformed -- wellinformed mcp     │   │
./docs/marketing/IMAGEGEN-FRONTEND-WEB.md:369:│   │ wellinformed claude install                                      │   │
./docs/marketing/IMAGEGEN-FRONTEND-WEB.md:391:> showing "claude mcp add --scope user wellinformed -- wellinformed
./docs/marketing/IMAGEGEN-FRONTEND-WEB.md:392:> mcp" followed by "wellinformed claude install"; below that, a
./docs/marketing/IMAGEGEN-FRONTEND-WEB.md:427:│         │ │ $ cd wellinformed && npm install &&            │ │           │
...
./docs/marketing/IMAGEGEN-FRONTEND-WEB.md:757:│   $ git clone https://github.com/twocirclestudios/wellinformed.git        │
./docs/marketing/IMAGEGEN-FRONTEND-WEB.md:758:│   $ cd wellinformed && npm install && npm run bootstrap                   │
...
./docs/marketing/influencer-outreach.md:32:Would love your take: github.com/SaharBarak/wellinformed
...
./docs/p2p/satisfaction-scoring.md:3:How wellinformed turns a set of retrieval hits into a single number
./docs/p2p/satisfaction-scoring.md:16:- Hook-level boost: [`.claude/hooks/wellinformed-prompt-submit.cjs`](../../.claude/hooks/wellinformed-prompt-submit.cjs) — applied AFTER the base scorer, capped at 1.0, never demotes.
./docs/p2p/satisfaction-scoring.md:47:Applied in `wellinformed-prompt-submit.cjs` because the base scorer
./docs/p2p/satisfaction-scoring.md:105:  100 queries where Claude was told to use vs. ignore wellinformed
...
./docs/marketing/SOCIAL-LAUNCH.md:26:> github.com/SaharBarak/wellinformed
./docs/marketing/SOCIAL-LAUNCH.md:30:> Just shipped Akashik — the globally accumulating knowledge
./docs/marketing/SOCIAL-LAUNCH.md:37:> github.com/SaharBarak/wellinformed
...
./docs/marketing/SOCIAL-LAUNCH.md:84:> Run a node: github.com/SaharBarak/wellinformed
...
./docs/marketing/SOCIAL-LAUNCH.md:121:> - P2P: libp2p protocols `/wellinformed/{search,recall,touch,share}/1.0.0`
./docs/marketing/SOCIAL-LAUNCH.md:132:> github.com/SaharBarak/wellinformed
...
./docs/marketing/SOCIAL-LAUNCH.md:187:> *you* run `wellinformed ask` in your terminal, you read the same
./docs/marketing/SOCIAL-LAUNCH.md:192:> into the agent's tool-call path, Akashik wires it into the
./docs/marketing/SOCIAL-LAUNCH.md:202:> github.com/SaharBarak/wellinformed
...
./docs/marketing/SOCIAL-LAUNCH.md:227:> `wellinformed ask` against a local knowledge graph + connected
./docs/marketing/SOCIAL-LAUNCH.md:243:> github.com/SaharBarak/wellinformed
./docs/marketing/SOCIAL-LAUNCH.md:247:> [P] Akashik — peer-federated knowledge retrieval with
./docs/marketing/SOCIAL-LAUNCH.md:266:> github.com/SaharBarak/wellinformed
...
./docs/marketing/SOCIAL-LAUNCH.md:312:> github.com/SaharBarak/wellinformed
./docs/marketing/SOCIAL-LAUNCH.md:316:> Akashik — knowledge before the prompt. Cooperative, P2P, MIT.
./docs/marketing/SOCIAL-LAUNCH.md:317:> github.com/SaharBarak/wellinformed
...
./docs/research/octopus-discover/round-5-2026-05-26/probes/codex-0.md:130:workdir: /Users/saharbarak/personal/wellinformed
...
./docs/p2p/peer-reputation-design.md:21:| Work                 | What it got right                                                    | What's wrong-fit for wellinformed                              |
./docs/p2p/peer-reputation-design.md:28:| **SybilGuard** (Yu 2008) | Graph-based identity defense                                   | Assumes social trust graph wellinformed doesn't have today    |
./docs/p2p/peer-reputation-design.md:107:`~/.wellinformed/peer-reputation.json` — local, atomic temp-write + rename
./docs/p2p/peer-reputation-design.md:230:| 7 | `wellinformed peers rep [<peer-id>]` inspect command | **ADD-NEXT** | `src/cli/commands/peers-rep.ts` (new) |
./docs/p2p/peer-reputation-design.md:231:| 8 | Pull-on-demand wire protocol (signed review summaries) | **ADD-LATER** | New `/wellinformed/reputation/1.0.0` libp2p protocol |
...
./docs/product/RELEASE-v4.md:26:**Claim**: A running `wellinformed daemon` with its L1 query cache warm serves repeat `ask` queries in **~100 ms**. A cold CLI (no daemon) takes **~900 ms**.
./docs/product/RELEASE-v4.md:28:**Measured** (live wellinformed home, 10,607 nodes, warm ONNX cache on disk):
./docs/product/RELEASE-v4.md:36:The native Rust client (`wellinformed-rs/src/bin/wellinformed_cli.rs`) collapses the Node-boot floor to ~5 ms — total round trip ~27 ms for cached repeat queries. Compose with daemon IPC + L1 cache and you hit the original 60× plan target *for cached repeats*. Cache misses run ~160–240 ms because the actual work (embed + search) dominates; native client overhead is irrelevant on miss.
./docs/product/RELEASE-v4.md:40:npx wellinformed@4 daemon start
./docs/product/RELEASE-v4.md:42:time wellinformed ask "your query"                 # cold miss, ~280 ms
./docs/product/RELEASE-v4.md:43:time wellinformed ask "your query"                 # warm hit, ~100 ms
./docs/product/RELEASE-v4.md:68:WELLINFORMED_RUST_BIN=./wellinformed-rs/target/release/embed_server \
...
./docs/product/RELEASE-v4.md:177:- Domain-separated signatures (`wellinformed-auth:v1:` vs `wellinformed-sig:v1:`)
./docs/product/RELEASE-v4.md:236:npm install -g wellinformed@4
./docs/product/RELEASE-v4.md:237:wellinformed identity init         # no-op if you already have a DID
./docs/product/RELEASE-v4.md:238:wellinformed daemon start          # enables the IPC hot path
./docs/product/RELEASE-v4.md:259:wellinformed consolidate run sessions --threshold 0.8 --min-size 5
./docs/product/RELEASE-v4.md:261:wellinformed consolidate run sessions --threshold 0.8 --min-size 5 --prune
./docs/product/RELEASE-v4.md:271:- ✅ **v4.0 — Native Rust client** (`wellinformed-rs/src/bin/wellinformed_cli.rs`, commit `a462f86`). Cold-starts ~5 ms, IPC round trip ~22 ms, total **27 ms** end-to-end for cached repeats. **33× faster than v3 cold CLI**. Falls back to Node shim when daemon socket absent or command not delegatable.
...
./docs/product/RELEASE-v4.md:273:- ✅ **v4.0 — Cross-process write lock** (`src/infrastructure/process-lock.ts`, commit `5591757`). Daemon and mutating CLI commands coordinate via `<home>/wellinformed.lock`. Stale-PID recovery + 20s heartbeat refresh. No "stop daemon" caveat anymore.
./docs/product/RELEASE-v4.md:276:- ✅ **v4.0 — Backup-before-prune** (commits `3c16238`, `91840da`). `--prune` writes NDJSON backup of source nodes by default; `--no-backup` opt-out. `wellinformed sessions reingest` recovers from full sessions-state.json wipe + re-trigger.
./docs/product/RELEASE-v4.md:277:- ✅ **v4.0 — Cache observability** (`wellinformed cache-stats`, commit `b2ab5ed`). Daemon-side L1 hit/miss/eviction counters exposed via IPC.
...
./docs/product/BENCHMARKS.md:59:cd wellinformed-rs && cargo build --release && cd ..
./docs/product/BENCHMARKS.md:60:WELLINFORMED_RUST_BIN=$(pwd)/wellinformed-rs/target/release/embed_server \
./docs/product/BENCHMARKS.md:71:  --datasets-dir ~/.wellinformed/bench/cqadupstack/cqadupstack \
./docs/product/BENCHMARKS.md:126:15. Phase 23 — **Unified memory bench** (`wellinformed bench memory`): 8 suites scoring 9 dimensions, composite **0.8597** on real public corpora (Phase 23.7 — Hetzner, 2026-05-20). Synthetic-fallback composite is 0.9107.
./docs/product/BENCHMARKS.md:136:that's stricter than any single public suite. Phase 23 ships `wellinformed
./docs/product/BENCHMARKS.md:142:wellinformed bench memory --json
...
./docs/product/BENCHMARKS.md:245:Three real-corpus suites are now wired into the bench CLI. They share the env contract `WELLINFORMED_BENCH_PUBLIC_REAL=1` (master gate; off by default to keep CI fast) and each takes a dataset-directory env var. Without the gate or the dataset they `t.skip()` cleanly and the composite falls back to the synth/proxy value (registration order in `src/cli/commands/bench.ts` is `synth → real` so real overwrites synth iff real ran).
./docs/product/BENCHMARKS.md:258:WELLINFORMED_BENCH_PUBLIC_REAL=1 \
./docs/product/BENCHMARKS.md:263:  wellinformed bench memory --json
...
./docs/research/octopus-discover/round-5-2026-05-26/probes/claude-sonnet-2.md:178:1. **Nix/NixOS community** — discourse.nixos.org (~40K members), GitHub nix-community (~600 active contributors). Value proposition: reproducible local tooling, no cloud dependency, Nix flake install is a natural fit for wellinformed.
./docs/research/octopus-discover/round-5-2026-05-26/probes/claude-sonnet-2.md:180:3. **Elixir/Phoenix community** — elixirforum.com (~50K members). Highly documentation-oriented, love functional programming, many run local dev environments. wellinformed's functional DDD architecture resonates.
...
./docs/p2p/p2p-threat-model.md:3:Scope: the attack surface exposed once a wellinformed node accepts data from **untrusted peers** via `share sync` (Y.js CRDT) or `touch` (one-shot pull). This document enumerates concrete paths from a hostile peer to code execution or secret exfiltration, and the mitigations that are (a) shipped, (b) planned, or (c) deferred.
./docs/p2p/p2p-threat-model.md:5:**Threat actor.** A peer on the P2P network who has completed the libp2p handshake and can send arbitrary payloads on `/wellinformed/share/1.0.0` and `/wellinformed/touch/1.0.0`. Peers are not authenticated beyond their ed25519 peer identity — there is no external PKI and no "this peer is trusted" claim.
./docs/p2p/p2p-threat-model.md:11:4. `~/.wellinformed/*.yaml`, `peer-identity.json` — private key for peer identity.
...
./docs/research/octopus-discover/round-4-2026-05-26/synthesis.md:9:The project pivoted from "wellinformed: agent-memory product" to
...
./docs/research/octopus-discover/round-4-2026-05-26/synthesis.md:20:called "wellinformed" internally.
...
./docs/research/octopus-discover/round-4-2026-05-26/probes/claude-sonnet-5.md:11:The project pivoted from "wellinformed: agent-memory product" to
...
./docs/research/octopus-discover/round-4-2026-05-26/probes/claude-sonnet-5.md:22:called "wellinformed" internally.
...
./docs/research/octopus-discover/round-4-2026-05-26/probes/gemini-1.md:12:The project pivoted from "wellinformed: agent-memory product" to
...
./docs/research/octopus-discover/round-4-2026-05-26/probes/gemini-1.md:23:called "wellinformed" internally.
...
./docs/research/octopus-discover/round-4-2026-05-26/probes/gemini-4.md:12:The project pivoted from "wellinformed: agent-memory product" to
...
./docs/research/octopus-discover/round-4-2026-05-26/probes/gemini-4.md:23:called "wellinformed" internally.
...
./docs/research/octopus-discover/round-5-2026-05-26/probes/codex-3.md:138:workdir: /Users/saharbarak/personal/wellinformed
...
./docs/research/octopus-discover/round-5-2026-05-26/probes/codex-3.md:302:/bin/zsh -lc 'rg --files . | rg "(akashik|round|locomo|lme|agentmemory|federat|benchmark|retrieval|memory)"' in /Users/saharbarak/personal/wellinformed
./docs/research/octopus-discover/round-5-2026-05-26/probes/codex-3.md:334:/bin/zsh -lc 'rg -n "Round 4|Akashik|ambitioned-curator|LoCoMo|LME-S|agentmemory|federation-level|compounding" .' in /Users/saharbarak/personal/wellinformed
./docs/research/octopus-discover/round-5-2026-05-26/probes/codex-3.md:377:./docs/research/beat-the-competitors-retrieval-plan.md:89:If those land, **wellinformed becomes the published-leaderboard leader on LME-S** with a fully open-source, CPU-only, retrieval-only pipeline. No LLM judge, no GPT-4 reasoning loop, no closed-weight model — just a clean hybrid sparse+dense+rerank+graph pipeline that beats the proprietary stacks.
./docs/research/octopus-discover/round-5-2026-05-26/probes/codex-3.md:379:./docs/research/beat-the-competitors-retrieval-plan.md:124:Once leaderboard-leading on retrieval, the next frontier is **hybrid retrieval + LLM extraction with SQuAD-F1 + LLM-judge as competing axes in the composite**. That makes wellinformed comparable to mem0's 92.5 LoCoMo composite — different scoring philosophy, head-to-head. Phase 23.8 already laid the SQuAD-F1 foundation; Phase 24 adds the judge axis.
./docs/research/octopus-discover/round-5-2026-05-26/probes/codex-3.md:395:./docs/research/octopus-discover/round-4-2026-05-26/README.md:5:- The pivot from `wellinformed` (agent-memory product) to **Akashik**
...
./docs/research/octopus-discover/round-5-2026-05-26/probes/codex-3.md:435:./docs/marketing/storybrand-messaging-draft.md:8:**Brand name:** Akashik (formerly project codename `wellinformed`).
...
./docs/research/octopus-discover/round-5-2026-05-26/probes/codex-3.md:489:./docs/research/octopus-discover/round-4-2026-05-26/probes/claude-sonnet-5.md:266:# FOURTH-ROUND ANALYSIS — Akashik / wellinformed
...
./docs/research/octopus-discover/round-5-2026-05-26/probes/codex-3.md:681:./docs/README.md:25:- [`GRAPHRAG-AUDIT.md`](product/GRAPHRAG-AUDIT.md) — wellinformed audited against 2025/2026 GraphRAG state of the art (Microsoft GraphRAG, HippoRAG 2, LightRAG, MultiHop-RAG, LoCoMo).
...
./docs/research/octopus-discover/round-5-2026-05-26/probes/codex-3.md:925:./docs/product/BENCHMARKS.md:239:| **wellinformed** (Hetzner, Phase 23.7 real public corpora) | unified composite | **0.8597** | real BEIR SciFact + LongMemEval-S oracle + LoCoMo factual; synthetic in 5 of 9 dimensions |
./docs/research/octopus-discover/round-5-2026-05-26/probes/codex-3.md:935:/bin/zsh -lc "sed -n '1,240p' docs/research/octopus-discover/round-4-2026-05-26/synthesis.md" in /Users/saharbarak/personal/wellinformed
./docs/research/octopus-discover/round-5-2026-05-26/probes/codex-3.md:945:The project pivoted from "wellinformed: agent-memory product" to
...
./docs/research/octopus-discover/round-5-2026-05-26/probes/codex-3.md:956:called "wellinformed" internally.
...
./docs/research/octopus-discover/round-5-2026-05-26/probes/codex-3.md:1179:/bin/zsh -lc "sed -n '1,240p' docs/research/octopus-discover/round-4-2026-05-26/README.md" in /Users/saharbarak/personal/wellinformed
./docs/research/octopus-discover/round-5-2026-05-26/probes/codex-3.md:1185:- The pivot from `wellinformed` (agent-memory product) to **Akashik**
./docs/research/octopus-discover/round-5-2026-05-26/probes/codex-3.md:1241:/bin/zsh -lc "sed -n '1,260p' docs/research/beat-the-competitors-retrieval-plan.md" in /Users/saharbarak/personal/wellinformed
./docs/research/octopus-discover/round-5-2026-05-26/probes/codex-3.md:1243:# How wellinformed beats the retrieval leaderboard (forward plan)
./docs/research/octopus-discover/round-5-2026-05-26/probes/codex-3.md:1248:| Benchmark | wellinformed | Competitor best | Gap |
./docs/research/octopus-discover/round-5-2026-05-26/probes/codex-3.md:1331:If those land, **wellinformed becomes the published-leaderboard leader on LME-S** with a fully open-source, CPU-only, retrieval-only pipeline. No LLM judge, no GPT-4 reasoning loop, no closed-weight model — just a clean hybrid sparse+dense+rerank+graph pipeline that beats the proprietary stacks.
./docs/research/octopus-discover/round-5-2026-05-26/probes/codex-3.md:1366:Once leaderboard-leading on retrieval, the next frontier is **hybrid retrieval + LLM extraction with SQuAD-F1 + LLM-judge as competing axes in the composite**. That makes wellinformed comparable to mem0's 92.5 LoCoMo composite — different scoring philosophy, head-to-head. Phase 23.8 already laid the SQuAD-F1 foundation; Phase 24 adds the judge axis.
./docs/research/octopus-discover/round-5-2026-05-26/probes/codex-3.md:1368:Beyond that: **federated retrieval** (the unique wellinformed bet) — measure how P2P-shared rooms across multiple peers lift recall on out-of-distribution questions, vs single-peer baselines. No public benchmark covers this today; we'd publish one.
./docs/research/octopus-discover/round-5-2026-05-26/probes/codex-3.md:1429:- **Bus factor on `wellinformed-rs`** — Rust ARM cross-compile is specialized knowledge; consider TypeScript-only fallback path.
...
./docs/research/octopus-discover/round-5-2026-05-26/probes/codex-3.md:1435:/bin/zsh -lc "sed -n '1,240p' docs/marketing/how-akashik-works.md" in /Users/saharbarak/personal/wellinformed
...
./docs/research/octopus-discover/round-5-2026-05-26/probes/codex-3.md:1688:/bin/zsh -lc "sed -n '620,760p' docs/research/octopus-discover/round-4-2026-05-26/probes/claude-sonnet-2.md" in /Users/saharbarak/personal/wellinformed
...
./docs/research/octopus-discover/round-5-2026-05-26/probes/codex-3.md:1711:/bin/zsh -lc 'rg -n "Q1|Q2|Q3|Q4|Q5|Q6|Q7|Q8|closest prior art|benchmark|launch plan|competitor|counter-argument|counter argument|novel|federation|compounding" docs/research/octopus-discover/round-4-2026-05-26/probes/*.md' in /Users/saharbarak/personal/wellinformed
...
./docs/research/octopus-discover/round-5-2026-05-26/probes/codex-3.md:2259:docs/research/octopus-discover/round-4-2026-05-26/probes/claude-sonnet-5.md:375:**The real risk:** publishing "wellinformed 0.9202 vs mem0 0.925" when the evaluation protocols differ is a credibility trap. The correct framing for Akashik launch is NOT R@5 comparison against single-user products. It's `web_fallback_rate` vs baseline (before federation) — a metric only Akashik can report.
...
./docs/research/octopus-discover/round-5-2026-05-26/probes/codex-3.md:2279:/bin/zsh -lc "sed -n '1,260p' findings.md" in /Users/saharbarak/personal/wellinformed
...
./docs/research/octopus-discover/round-4-2026-05-26/probes/codex-0.md:12:The project pivoted from "wellinformed: agent-memory product" to
...
./docs/research/octopus-discover/round-4-2026-05-26/probes/codex-0.md:23:called "wellinformed" internally.
...
./docs/research/octopus-discover/round-4-2026-05-26/probes/codex-0.md:399:workdir: /Users/saharbarak/personal/wellinformed
./docs/research/octopus-discover/round-4-2026-05-26/probes/codex-0.md:450:The project pivoted from "wellinformed: agent-memory product" to
...
./docs/research/octopus-discover/round-4-2026-05-26/probes/codex-0.md:461:called "wellinformed" internally.
...
./docs/research/octopus-discover/round-4-2026-05-26/probes/codex-3.md:12:The project pivoted from "wellinformed: agent-memory product" to
...
./docs/research/octopus-discover/round-4-2026-05-26/probes/codex-3.md:23:called "wellinformed" internally.
...
./docs/research/octopus-discover/round-4-2026-05-26/probes/codex-3.md:532:workdir: /Users/saharbarak/personal/wellinformed
./docs/research/octopus-discover/round-4-2026-05-26/probes/codex-3.md:583:The project pivoted from "wellinformed: agent-memory product" to
...
./docs/research/octopus-discover/round-4-2026-05-26/probes/codex-3.md:594:called "wellinformed" internally.
...
./docs/research/octopus-discover/round-4-2026-05-26/probes/claude-sonnet-2.md:11:The project pivoted from "wellinformed: agent-memory product" to
...
./docs/research/octopus-discover/round-4-2026-05-26/probes/claude-sonnet-2.md:22:called "wellinformed" internally.
...
./docs/research/octopus-discover/round-4-2026-05-26/probes/claude-sonnet-2.md:418:*What they commit to*: Index one active project (`wellinformed index <repo>`), join `oss-commons` room, run their peer for at least 4 hours/day for 30 days.
./docs/research/octopus-discover/round-4-2026-05-26/probes/claude-sonnet-2.md:436:1. `wellinformed peer add` via a lightweight coordinator server (not just mDNS — mDNS doesn't cross routers)
./docs/research/octopus-discover/round-4-2026-05-26/probes/claude-sonnet-2.md:437:2. `wellinformed ask` shows "[answered by: @username, 4 days ago]" in the output
./docs/research/octopus-discover/round-4-2026-05-26/probes/claude-sonnet-2.md:438:3. A `wellinformed network status` command showing R(T,t) for the top 10 topics in the room
./docs/research/octopus-discover/round-4-2026-05-26/probes/claude-sonnet-2.md:440:**What will kill the launch:** If the coordinator server requires Docker or a domain name to self-host, only 10 people will run peers. The "default open" mode must be `wellinformed peer join oss-commons.akashik.dev` — one command, no setup.
...
exec
/bin/zsh -lc "sed -n '1,160p' NEXT_STEPS.md" in /Users/saharbarak/personal/wellinformed
 succeeded in 0ms:
...
exec
/bin/zsh -lc "sed -n '1,120p' README.md.bak.830lines" in /Users/saharbarak/personal/wellinformed
 succeeded in 0ms:
<p align="center">
...
exec
/bin/zsh -lc "find .claude-octopus -maxdepth 3 -type f | sort | sed -n '1,120p'" in /Users/saharbarak/personal/wellinformed
exec
/bin/zsh -lc "find .agents -maxdepth 3 -type f | sort | sed -n '1,120p'" in /Users/saharbarak/personal/wellinformed
 succeeded in 0ms:
...
exec
/bin/zsh -lc "find wellinformed-rs -maxdepth 3 -type f | sort | sed -n '1,120p'" in /Users/saharbarak/personal/wellinformed
 succeeded in 0ms:
wellinformed-rs/.gitignore
wellinformed-rs/Cargo.lock
wellinformed-rs/Cargo.toml
wellinformed-rs/src/application/mod.rs
wellinformed-rs/src/application/pipeline.rs
wellinformed-rs/src/bin/embed_server.rs
wellinformed-rs/src/bin/wellinformed_cli.rs
wellinformed-rs/src/domain/beir.rs
wellinformed-rs/src/domain/encoder_spec.rs
wellinformed-rs/src/domain/metrics.rs
wellinformed-rs/src/domain/mod.rs
wellinformed-rs/src/domain/room_routing.rs
wellinformed-rs/src/domain/tunnel_graph.rs
wellinformed-rs/src/domain/vector_ops.rs
wellinformed-rs/src/infrastructure/encoder_port.rs
wellinformed-rs/src/infrastructure/fastembed_encoder.rs
wellinformed-rs/src/infrastructure/json_report.rs
wellinformed-rs/src/infrastructure/mod.rs
wellinformed-rs/src/infrastructure/progress.rs
wellinformed-rs/src/lib.rs
wellinformed-rs/src/main.rs

exec
/bin/zsh -lc "sed -n '1,200p' src/cli/commands/onboard.ts" in /Users/saharbarak/personal/wellinformed
 succeeded in 0ms:
/**
...
exec
/bin/zsh -lc "sed -n '1,200p' src/telegram/bot.ts" in /Users/saharbarak/personal/wellinformed
 succeeded in 0ms:
/**
...
exec
/bin/zsh -lc "sed -n '1,200p' src/infrastructure/rust-retrieval.ts" in /Users/saharbarak/personal/wellinformed
 succeeded in 0ms:
/**
