...
# Role: researcher
# Phase: probe
# Prompt: Analyze the LOCAL CODEBASE in the current directory for: -P CLEANUP AUDIT — what's no longer relevant in the Folklore project given the full arc:
...
# Folklore Cleanup Audit — 2026-05-26

## Context Synopsis (what I verified from the code)

- **Active product:** Folklore (OSS federated knowledge commons). Codebase still named `folklore` — intentional two-name period per `docs/PROJECT-PLAN-FOLKLORE.md`.
- **Current state:** Phase 23.7 (Hetzner real-corpus benches + FolkloreBench-F validated). Phase 24 is next (federation sweep, niche-evaporation mitigation, IPC auth hardening).
- **Explicitly de-prioritized:** All further R@5 / LongMemEval / bge-reranker tuning (PROJECT-PLAN-FOLKLORE.md §"What we're explicitly NOT doing").
- **Architecture:** DDD, functional TypeScript, neverthrow Results, libp2p P2P, SQLite + ONNX vectors, folklore-rs Rust sidecar (BM25 + dense fusion).
...
| File | Action | Reason |
|---|---|---|
| `README.md` | NEEDS-REWRITE | Still says "folklore" product framing. Must be updated to Folklore identity before Phase 25 pilot launch. |
...
| `NEXT_STEPS.md` | DELETE | Same — root-level planning scratchpad. Project plan is `docs/PROJECT-PLAN-FOLKLORE.md` now. |
...
| `.planning/PROJECT.md` | NEEDS-REWRITE | Superseded by `docs/PROJECT-PLAN-FOLKLORE.md`. Keep the file but redirect or update to current Folklore framing. |
...
| `.planning/ROADMAP.md` | ARCHIVE | Pre-pivot roadmap. `docs/product/ROADMAP.md` + `docs/PROJECT-PLAN-FOLKLORE.md` are the live artifacts. |
...
| `.planning/BENCH-COMPETITORS.md` | ARCHIVE | Pre-pivot competitor benchmarking. The explicit "not competing with agentmemory" decision in PROJECT-PLAN-FOLKLORE.md makes this moot. |
...
| `.planning/phases/phase-23/23-CONTEXT.md` | KEEP | Active — Phase 23 is the benchmark acceptance contract. FolkloreBench-F lives here. |
...
| File | Action | Reason |
|---|---|---|
| `docs/PROJECT-PLAN-FOLKLORE.md` | KEEP | Canonical 30-60 day engineering + launch plan. The authoritative "what's next." |
| `docs/marketing/storybrand-messaging-draft.md` | KEEP | Final Folklore brand positioning (2026-05-26). This is the live marketing artifact. See Q4 analysis below. |
| `docs/marketing/how-folklore-works.md` | KEEP | Architectural credibility anchor. Referenced by PROJECT-PLAN-FOLKLORE.md as load-bearing for pilot. |
| `docs/research/octopus-discover/round-5-2026-05-26/` | KEEP | The canonical recommendation that drives Phase 24 and Phase 25. |
| `docs/research/octopus-discover/round-4-2026-05-26/` | KEEP | Referenced directly in PROJECT-PLAN-FOLKLORE.md §Phase 24.5 (8 launch blockers). |
...
| `docs/research/beat-the-competitors-retrieval-plan.md` | ARCHIVE | Strong empirical content (loss analysis by question type, E1–E9 experiment matrix) but the mission is explicitly superseded. PROJECT-PLAN-FOLKLORE.md says "not in this 60-day window." Archive with an annotation, don't delete — the headroom analysis will be needed if retrieval work ever resumes. |
| `docs/research/performance-prediction-matrix.md` | ARCHIVE | Hardware × rerank tier matrix. Well-researched, but the retrieval-tuning sprint it supported is parked. Archive with note: "superseded by PROJECT-PLAN-FOLKLORE.md §Not doing." |
...
| `docs/product/ROADMAP.md` | NEEDS-REWRITE | Pre-pivot roadmap. Superseded by PROJECT-PLAN-FOLKLORE.md but the file still exists in docs/product/ and will confuse newcomers. |
...
| `docs/marketing/SITE-REDESIGN-SPEC.md` | NEEDS-REWRITE | Site redesign spec for Folklore. The spec may still be valid structurally but the brand is Folklore now. |
...
| `docs/marketing/BRAND-KIT.md` | NEEDS-REWRITE | Brand kit exists but assets (logos, wordmarks) all say `folklore`. Must be updated for Folklore before launch. |
| `docs/assets/logo-*.svg`, `og-card.*`, `og-portrait.*` | NEEDS-REWRITE | All existing brand assets are `folklore` identity. Folklore rebrand requires new assets. These are stale until the rebrand PR. |
...
| File | Action | Reason |
|---|---|---|
| `tests/bench-folklore-federation.test.ts` | KEEP | FolkloreBench-F — the core validation instrument for the compounding thesis. |
...
| `tests/bench-auto-forget.test.ts`, `bench-beta-calibration.test.ts`, `bench-retention-band.test.ts`, `bench-tier-promotion.test.ts`, `bench-write-gate.test.ts` | KEEP | Phase 21/23 folklore-specific synthetic benches. All are part of the composite score formula. |
| `tests/llm-listwise-rerank.test.ts` | ARCHIVE/DISABLE | LLM listwise rerank is explicitly de-prioritized (PROJECT-PLAN-FOLKLORE.md §"Not doing"). The code still exists in `src/domain/llm-listwise-rerank.ts` but this test is now "aspirational not tested." Either add a `.skip` with a note, or archive. Don't delete — the code may be revived post-pilot. |
...
| Path | Action | Reason |
|---|---|---|
| `src/domain/llm-listwise-rerank.ts` | KEEP (annotate) | Code is correct and well-tested. But the retrieval sprint that would activate it is explicitly parked. Add a `// Phase 24+ — not wired into the bench or ask path; activation deferred per PROJECT-PLAN-FOLKLORE.md` comment at the top. |
| `src/infrastructure/llm-listwise-rerank.ts` | KEEP (annotate) | Same rationale. Infrastructure adapter for the parked feature. |
| `src/domain/federation-sim.ts` | KEEP | FolkloreBench-F core. The `compoundingSlope`, `webFallbackRateOverTime`, `propagationHalfLife` functions are the primary validation instruments for Phase 24. |
...
| `src/cli/commands/bench.ts` | KEEP | `folklore bench memory` driver. Phase 23. |
...
| `src/cli/commands/swarm.ts` | INVESTIGATE | `swarm` CLI command — is this the claude-flow swarm integration or a folklore-native feature? If it's unused glue from the octopus harness, it could be a delete candidate. |
...
| `.claude/skills/folklore/SKILL.md` | NEEDS-REWRITE | The folklore skill needs updating to Folklore brand. |
| `folklore-rs/` | KEEP | Rust sidecar (BM25 + HNSW). Active — referenced by `rust-retrieval.ts` and has a regression test (`phase29`). |
...
The current `storybrand-messaging-draft.md` (2026-05-26) already represents the final OSS-community positioning. The context says there were three revisions (personal → team → OSS community) but the file itself contains only the final Folklore OSS-commons framing — the earlier drafts were in the now-archived `positioning-draft.md` and `positioning-v2.1.md`. The "evolution table" would be in those archived files, not in the current one.
...
Fourteen completed phase directories plus `SOTA-UPGRADE-PLAN.md`, `BENCH-COMPETITORS.md`, `performance-prediction-matrix.md`, `beat-the-competitors-retrieval-plan.md`, and the domain-irrelevant SOTA attack files (CFD, physics, particle physics, math) are all historical artifacts that are cluttering the active planning context. Moving them to `.planning/archive/` costs nothing (git history preserves them), and the search-and-read cognitive load goes down immediately. The "explicitly not doing retrieval tuning" decision in PROJECT-PLAN-FOLKLORE.md means these files currently *actively mislead* anyone who reads them — a new contributor seeing `beat-the-competitors-retrieval-plan.md` would reasonably assume that's the current mission. Archiving corrects that signal.

**3. Rewrite `docs/product/MANIFESTO.md`, `docs/product/VISION.md`, and `docs/product/ROADMAP.md` to Folklore OSS-commons framing.**
These three files are the first things a potential pilot contributor will read to understand what the project *is*. They currently describe an agent-memory product. The `storybrand-messaging-draft.md` + `how-folklore-works.md` + `PROJECT-PLAN-FOLKLORE.md` together contain everything needed to rewrite all three in under an hour. Until this is done, the project's own docs will undermine the Phase 25 pilot recruitment. The `README.md` is in the same bucket, but it's also gated by the domain-rename question — rewriting MANIFESTO/VISION/ROADMAP can happen now without touching the binary package name.
```
