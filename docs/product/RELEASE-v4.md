...
> Folklore v4 is the first OSS P2P agent memory framework with cryptographic identity, brain-shaped caching, and episodic-to-semantic background consolidation. CPU-local, Apache 2.0 / MIT, zero SaaS dependencies.
...
Every number is reproducible via `bench/bench-*.mjs` on a commodity CPU (M-series or equivalent) within 30 minutes. Claims not backed by a specific benchmark have been removed.

### 1. Query latency — 9× on cached, 3.2× on cold (daemon IPC)

**Claim**: A running `folklore daemon` with its L1 query cache warm serves repeat `ask` queries in **~100 ms**. A cold CLI (no daemon) takes **~900 ms**.

**Measured** (live Folklore home, 10,607 nodes, warm ONNX cache on disk):
...
**Measured** (`bench/bench-embed-throughput.mjs`, bge-base via Rust fastembed, N=32):
| Pattern | docs/sec |
|---------|----------|
...
The coalescing decorator wraps any `Embedder` and queues individual `.embed()` calls into batches, flushing at 32 items OR after 20 ms. On by default; opt-out via `FOLKLORE_EMBEDDER_BATCH=off`.

**Reproduction**:
```bash
FOLKLORE_RUST_BIN=./folklore-rs/target/release/embed_server \
  node bench/bench-embed-throughput.mjs --model bge-base --n 32
```
...
Toggle via `FOLKLORE_VECTOR_QUANTIZATION=binary-512`. Schema migration is idempotent: existing vectors.db files get a new `raw_bin` column on open; old rows stay NULL (invisible to binary search until re-indexed), new upserts populate both fp32 and binary.

**Reproduction**:
```bash
node bench/bench-lab.mjs                        # 192-config sweep
```
...
folklore consolidate run <room> --dry-run --threshold 0.8 --min-size 5
# to commit: drop --dry-run
node bench/bench-consolidation.mjs <room>       # storage + quality gate
```
...
**Coordination with the daemon**: Both `folklore daemon` and `folklore consolidate run` acquire the cross-process write lock at `<home>/folklore.lock` (file-based exclusive create with stale-PID recovery). The daemon holds it for its lifetime + refreshes every 20s; consolidate waits up to 30s for it. **No "stop the daemon first" caveat — run anytime.** Stale-recovery handles the case where a prior holder crashed without releasing.

**Atomic prune** — pass `--prune` to `folklore consolidate run` and the source raw entries get deleted from BOTH the graph and the vector index after successful consolidation. Closes the §2j quality regression by removing BM25 competition from the still-indexed raw text. Mutually exclusive with `--dry-run`.
...
**Claim**: Every memory entry Folklore emits can be cryptographically attributed to a **user DID** (not a device, not a provider). Cross-device memory portability is proven end-to-end — a signed envelope on device A verifies on a freshly-recovered device B with zero prior contact.
...
- Domain-separated signatures (`folklore-auth:v1:` vs `folklore-sig:v1:`)
...
09223d1  feat(runtime): FOLKLORE_VECTOR_QUANTIZATION env var + search dispatch
...
```bash
npm install -g folklore@4
folklore identity init         # no-op if you already have a DID
folklore daemon start          # enables the IPC hot path
```
...
export FOLKLORE_VECTOR_QUANTIZATION=binary-512

# Rust fastembed backend (higher-quality ONNX ports)
export FOLKLORE_EMBEDDER_BACKEND=rust
export FOLKLORE_EMBEDDER_MODEL=bge-base

# Coalescing batch decorator is ON by default. Opt out:
# export FOLKLORE_EMBEDDER_BATCH=off
```
...
```bash
ollama pull qwen2.5:1.5b
folklore consolidate run sessions --threshold 0.8 --min-size 5
# add --prune to atomically delete source entries (closes §2j quality regression):
folklore consolidate run sessions --threshold 0.8 --min-size 5 --prune
...
- ✅ **v4.0 — Native Rust client** (`folklore-rs/src/bin/folklore_cli.rs`, commit `a462f86`). Cold-starts ~5 ms, IPC round trip ~22 ms, total **27 ms** end-to-end for cached repeats. **33× faster than v3 cold CLI**. Falls back to Node shim when daemon socket absent or command not delegatable.
...
- ✅ **v4.0 — Cross-process write lock** (`src/infrastructure/process-lock.ts`, commit `5591757`). Daemon and mutating CLI commands coordinate via `<home>/folklore.lock`. Stale-PID recovery + 20s heartbeat refresh. No "stop daemon" caveat anymore.
...
- ✅ **v4.0 — Backup-before-prune** (commits `3c16238`, `91840da`). `--prune` writes NDJSON backup of source nodes by default; `--no-backup` opt-out. `folklore sessions reingest` recovers from full sessions-state.json wipe + re-trigger.
- ✅ **v4.0 — Cache observability** (`folklore cache-stats`, commit `b2ab5ed`). Daemon-side L1 hit/miss/eviction counters exposed via IPC.
