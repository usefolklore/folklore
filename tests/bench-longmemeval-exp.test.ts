...
test('EXP: LongMemEval-S chunking experiment', { timeout: 24 * 60 * 60 * 1000 }, async (t) => {
  if (process.env.FOLKLORE_BENCH_PUBLIC_REAL !== '1') {
    t.skip('FOLKLORE_BENCH_PUBLIC_REAL not set — skipping real-corpus suite');
    return;
  }
  ...
  // Set `FOLKLORE_RERANK=1` to activate; `FOLKLORE_RERANK_MODEL`
  ...
  // `FOLKLORE_LLM_RERANK=1`. When both are set, LISTWISE WINS
  ...
  const RERANK_HEAD = Number(process.env.FOLKLORE_RERANK_HEAD ?? (listwiseScorer ? 30 : 20));
  ...
  const KMAX = Number(process.env.FOLKLORE_BENCH_LME_KMAX ?? 50);
  // EXPERIMENT: sub-session chunking. Index overlapping turn-windows instead
  // of one node per session, then dedup chunk hits back to parent sessions
  // before scoring. Matches the relevant TURNS in a long session rather than
  // diluting them across the whole-session embedding. Identity (off) = the
  // committed real adapter's behaviour exactly.
  const CHUNK_ON = process.env.FOLKLORE_EXP_CHUNK === '1';
  const CHUNK_TURNS = Number(process.env.FOLKLORE_EXP_CHUNK_TURNS ?? 6);
  const CHUNK_STRIDE = Number(process.env.FOLKLORE_EXP_CHUNK_STRIDE ?? 3);
  if (CHUNK_ON) console.log(`  EXPERIMENT sub-session chunking ON · window=${CHUNK_TURNS} turns stride=${CHUNK_STRIDE}`);
  // When chunking, one session yields many chunk-nodes, so over-retrieve far
  // deeper to still surface KMAX *distinct sessions* after dedup.
  const baseOver = Math.max(KMAX, (reranker || listwiseScorer) ? RERANK_HEAD : 5);
  const overRetrieveK = CHUNK_ON ? Math.max(baseOver * 6, 200) : baseOver;
  if (listwiseScorer) {
    ...
    console.log(`  cross-encoder rerank ON · model=${process.env.FOLKLORE_RERANK_MODEL ?? 'Xenova/ms-marco-MiniLM-L-6-v2'} · over-retrieve k=${overRetrieveK} → rerank top-${RERANK_HEAD} → final K=${K}`);
  }

  // E11 (Phase 23.9): rule-based contextual enrichment — write-path.
  // When `FOLKLORE_BENCH_CONTEXTUAL_ENRICH=1`, prepend
  ...
  for (let i = 0; i < dataset.length; i++) {
    ...
    try {
      ...
      // EXPERIMENT: maps each indexed node id back to its parent session id.
      // Off (whole-session) this is the identity sid→sid.
      const chunkParent = new Map<string, string>();

      // Index every haystack session — as one node, or (experiment) as
      // overlapping turn-window chunks tagged with the parent session id.
      for (let s = 0; s < q.haystack_session_ids.length; s++) {
        ...
        // Build the list of (nodeId, turnSlice) units to index.
        const units: { nodeId: string; slice: readonly LmeTurn[] }[] = [];
        if (CHUNK_ON && turns.length > CHUNK_TURNS) {
          let w = 0;
          for (let start = 0; start < turns.length; start += CHUNK_STRIDE) {
            units.push({ nodeId: `${sid}#${w}`, slice: turns.slice(start, start + CHUNK_TURNS) });
            w += 1;
            if (start + CHUNK_TURNS >= turns.length) break;
          }
        } else {
          units.push({ nodeId: sid, slice: turns });
        }

        for (const { nodeId, slice } of units) {
          const rawText = sessionToText(slice);
          if (rawText.length === 0) continue;
          const indexedText = enrichOn
            ? enrichText(rawText, { date, sessionId: sid, participants })
            : rawText;
          sessionTextByNode.set(nodeId, indexedText);
          chunkParent.set(nodeId, sid);
          const r = await indexNode({ graphs, vectors, embedder })({
            node: {
              ...
              file_type: 'document',
              source_file: sid,
              source_uri: `session://${nodeId}`,
              summary: rawText.slice(0, 400),
              fetched_at: date ?? '2026-05-19T00:00:00Z',
            },
            text: indexedText,
          });
          if (r.isErr()) throw new Error(`index ${nodeId}: ${JSON.stringify(r.error)}`);
        }
      }
      ...
      // 21-50.
      // Map chunk hits → parent session ids, dedup preserving rank order, so
      // recall is scored at session granularity (gold is answer_session_ids).
      // Identity when chunking is off.
      const seenParent = new Set<string>();
      const retrievedFull: string[] = [];
      for (const m of head) {
        const parent = chunkParent.get(m.node_id as string) ?? (m.node_id as string);
        if (seenParent.has(parent)) continue;
        seenParent.add(parent);
        retrievedFull.push(parent);
      }
    ...
    // `FOLKLORE_BENCH_PROGRESS_EVERY_N` (default 25). Live tails
    ...
    // were hiding).
    const PROGRESS_EVERY_N = Number(process.env.FOLKLORE_BENCH_PROGRESS_EVERY_N ?? 25);
  ...
  // their full pipeline + reranker. folklore retrieves with
