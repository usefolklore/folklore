 ...
 *
 * This test is opt-in via `FOLKLORE_RUST_BIN` env var or a
...
const rustBinaryAvailable = (): string | null => {
  const candidate = process.env.FOLKLORE_RUST_BIN ?? repoBinaryPath();
...
test('phase-29: Rust embedder end-to-end retrieval quality gate on fixture', async (t) => {
  ...
  if (!binary) {
    t.skip(
      'folklore-rs embed_server binary not built — build with `cargo build --release --manifest-path folklore-rs/Cargo.toml` or set FOLKLORE_RUST_BIN',
    );
    return;
  }
