...
test('pickRerankTier: WELLINFORMED_RERANK=0 forces none on any hardware', () => {
...
test('pickRerankTier: invalid override is ignored, falls back to hw default', () => {
  ...
  const env = rerankEnvFromProcess({ WELLINFORMED_RERANK_TIER: 'bogus' });
...
test('rerankEnvFromProcess: parses all env vars', () => {
  const env = rerankEnvFromProcess({
    WELLINFORMED_RERANK_TIER: 'llm-listwise-small',
    WELLINFORMED_RERANK_MODEL: 'qwen2.5:3b',
    WELLINFORMED_RERANK_LATENCY_MS: '2500',
    WELLINFORMED_RERANK_HEAD: '40',
    WELLINFORMED_RERANK: '1',
  });
