...
test('bench: real LoCoMo factual harmonic-mean F1', { timeout: 24 * 60 * 60 * 1000 }, async (t) => {
  ...
  const useLlmExtractor = process.env.FOLKLORE_BENCH_LLM_EXTRACTOR === '1';
  const extractor = useLlmExtractor ? llmExtractorFromEnv() : null;
  if (useLlmExtractor && extractor === null) {
    t.skip('FOLKLORE_BENCH_LLM_EXTRACTOR=1 but no extractor resolvable from env (set FOLKLORE_OLLAMA_URL or FOLKLORE_BENCH_LLM_EXTRACTOR_FIXTURE=1)');
    return;
  }
  if (extractor) {
  ...
  const KMAX = Number(process.env.FOLKLORE_BENCH_LOCOMO_KMAX ?? 50);
  ...
  for (let sIdx = 0; sIdx < dataset.length; sIdx++) {
    ...
    try {
      ...
      for (let qIdx = 0; qIdx < factualQa.length; qIdx++) {
        ...
        // Tunable via `FOLKLORE_BENCH_PROGRESS_EVERY_N` (default 25).
  ...
  // when the extractor was wired in (`FOLKLORE_BENCH_LLM_EXTRACTOR=1`).
  ...
  // FOLKLORE_BENCH_LLM_EXTRACTOR=1 path (squadF1 metric, reported
