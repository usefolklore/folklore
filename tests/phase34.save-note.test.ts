...
test("phase-34: nodeFromSave maps 'source' → document, others → rationale", () => {
  ...
  const src = nodeFromSave({ type: 'source', label: 'BAAI bge-base', date: d });
  const syn = nodeFromSave({ type: 'synthesis', label: 'Hybrid lift', date: d });
  const con = nodeFromSave({ type: 'concept', label: 'RNG tunnels', date: d });
  const dec = nodeFromSave({ type: 'decision', label: 'Dense-only for stance', date: d });
...
test('phase-34: nodeFromSave clamps body to 8000 chars, stamps note_type + id as embedding_id', () => {
  ...
  const n = nodeFromSave({ type: 'synthesis', label: 'L', body: big, date: new Date('2026-04-17T00:00:00Z') });
...
test('phase-34: save is idempotent within a UTC day (same id → merged)', () => {
  const d = new Date('2026-04-17T00:00:00Z');
  const first = nodeFromSave({ type: 'concept', label: 'Touch primitive', body: 'v1', date: d });
  const later = nodeFromSave({ type: 'concept', label: 'Touch primitive', body: 'v2 revised', date: d });
