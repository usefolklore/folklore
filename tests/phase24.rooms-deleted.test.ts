 ...
 *
 *   ROOMS-DEL-01  `wellinformed room` CLI is removed
 *   ROOMS-DEL-02  ~/.wellinformed/rooms.json no longer read/written
 ...
 *   ROOMS-DEL-06  `wellinformed migrate v5` exists + idempotent + lossless
 *   ROOMS-DEL-07  Read-side workspace pre-filter; `--workspace all` opts out
 *   ROOMS-DEL-08  All wellinformed hooks pass without `room` field
 *
...
describe('Phase 24 — CLI surface (ROOMS-DEL-01)', () => {
  test('`wellinformed room` is an unknown subcommand (non-zero + "unknown" in stderr)', () => {
    const r = runCli(['room']);
    assert.notEqual(r.code, 0, '`wellinformed room` must exit non-zero');
    assert.match(r.stderr, /unknown/i, 'stderr must indicate unknown command');
  });

  test('`wellinformed save --room x` is rejected with a V5 error', () => {
...
describe('Phase 24 — migrate v5 (ROOMS-DEL-06)', () => {
  const writeV4Fixture = (home: string): void => {
    ...
    const v4Graph = {
      ...
      nodes: [
        ...
        { id: 'n3', label: 'C', file_type: 'document', source_file: '/c', room: 'wellinformed-dev' },
      ],
      links: [],
    };
  ...
  test('migrate v5 strips room, defaults private:false, deletes registries', () => {
    ...
    try {
      writeV4Fixture(home);
      const r = runCli(['migrate', 'v5'], { WELLINFORMED_HOME: home });
  ...
  test('migrate v5 is idempotent: second run exits 0 with "Already on V5"', () => {
    ...
    try {
      writeV4Fixture(home);
      const r1 = runCli(['migrate', 'v5'], { WELLINFORMED_HOME: home });
      assert.equal(r1.code, 0, 'first migration must succeed');

      const r2 = runCli(['migrate', 'v5'], { WELLINFORMED_HOME: home });
  ...
  test('migrate v5 --rollback restores graph.json from backup', () => {
    ...
    try {
      ...
      const rFwd = runCli(['migrate', 'v5'], { WELLINFORMED_HOME: home });
      assert.equal(rFwd.code, 0);

      const rRollback = runCli(['migrate', 'v5', '--rollback'], { WELLINFORMED_HOME: home });
...
describe('Phase 24 — wellinformed hooks (ROOMS-DEL-08)', () => {
  ...
  test('session-start statusline source does not reference rooms count', () => {
    const candidates = [
      ...
      '.claude/helpers/wi-statusline.cjs',
    ];
    for (const c of candidates) {
      const full = join(ROOT, c);
      if (!existsSync(full)) continue;
