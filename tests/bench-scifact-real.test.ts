...
test('bench: real BEIR SciFact NDCG@10', { timeout: 24 * 60 * 60 * 1000 }, async (t) => {
  ...
  try {
    ...
    // Activated via FOLKLORE_RERANK=1; model selectable via
    // FOLKLORE_RERANK_MODEL (default Xenova/ms-marco-MiniLM-L-6-v2,
    ...
    if (reranker) {
      console.log(`  cross-encoder rerank ON · model=${process.env.FOLKLORE_RERANK_MODEL ?? 'Xenova/ms-marco-MiniLM-L-6-v2'} · over-retrieve k=${overRetrieveK} → rerank top-${RERANK_HEAD} → NDCG@${K}`);
    }
    ...
    const report: BenchSuiteReport = {
      ...
      perQuery,
      elapsedMs,
      notes: `Real BEIR SciFact — ${corpus.length} docs × ${queries.length} test queries via Xenova all-MiniLM-L6-v2 (fp32, mean-pooled, 512 max_len). Rerank=${reranker ? (process.env.FOLKLORE_RERANK_MODEL ?? 'Xenova/ms-marco-MiniLM-L-6-v2') : 'off'} (over-retrieve k=${overRetrieveK}, head=${RERANK_HEAD}). Replaces the 30-doc labeled proxy.`,
    };
