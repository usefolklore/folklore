/**
 * `folklore jobs <sub>` — inspect the daemon's background queue.
 *
...
const USAGE = `usage: folklore jobs <sub>
...
const checkDaemon = (): number | null => {
  ...
  if (!existsSync(paths.home) || !isRunning(paths.home)) {
    console.error('jobs: daemon is not running. start it with `folklore daemon start`.');
    return 1;
  }
  return null;
};
...
const watch = async (): Promise<number> => {
  ...
  const draw = async (): Promise<void> => {
    ...
    process.stdout.write(`folklore jobs — ${new Date().toISOString()}\n\n`);
