/**
 * `folklore consolidate <sub>` — Phase 4c CLI surface for the
...
const parseRunArgs = (args: readonly string[]): RunArgs | string => {
  ...
  let minSize = 5;
  let maxSize = 100;
  let model = process.env.FOLKLORE_OLLAMA_MODEL ?? 'qwen2.5:1.5b';
  let prune = false;
  ...
  // entries go to an NDJSON file that `folklore sessions reingest`
...
const runCmd = async (args: readonly string[]): Promise<number> => {
  ...
  const lockRes = await acquireLock(folkloreHome(), {
    owner: 'consolidate',
    waitMs: 30_000,
    pollIntervalMs: 250,
  });
  if (lockRes.isErr()) {
    ...
    console.error(`  retry, or run 'folklore daemon stop' to free the lock.`);
    return 1;
  }
  ...
  try {
    ...
    if (ping.isErr()) {
      ...
      console.error(`  start it with: ollama serve  (or configure FOLKLORE_OLLAMA_URL)`);
      return 1;
    }
    ...
    if (parsed.prune && res.value.source_ids_marked.length > 0) {
      ...
      // graph nodes to an NDJSON file so `folklore sessions reingest`
...
const help = (): number => {
  ...
  console.log('Set FOLKLORE_OLLAMA_URL / FOLKLORE_OLLAMA_MODEL to override.');
  console.log('');
  console.log('Run consolidation while `folklore daemon` is stopped — v4.0 has no');
...
const pruneMarkedCmd = async (args: readonly string[]): Promise<number> => {
  ...
  const lockRes = await acquireLock(folkloreHome(), { owner: 'consolidate-prune', waitMs: 30_000, pollIntervalMs: 250 });
