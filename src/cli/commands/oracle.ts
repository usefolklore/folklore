...
const liveBroadcast = async (
  ...
  try {
    ...
    pubsub.subscribe('/folklore/oracle/1.0.0');
    ...
    console.log(`  live:   published to /folklore/oracle/1.0.0 (${dialed} peer(s) dialed)`);
    return 0;
  } finally {
...
const ask = async (rest: readonly string[]): Promise<number> => {
  ...
  if (!text) {
    console.error('oracle ask: missing question — usage: folklore oracle ask "your question" [--live]');
    return 1;
  }
  ...
  try {
    ...
    console.log(live
      ? '  peers subscribed to /folklore/oracle/1.0.0 get it now; others on next touch.'
...
const answer = async (rest: readonly string[]): Promise<number> => {
  ...
  const text = textTokens.join(' ').trim();
  if (!qid || !text) {
    console.error('oracle answer: usage: folklore oracle answer <question-id> "your answer" [--confidence 0.7] [--live]');
    return 1;
  }
  if (!isQuestionId(qid)) {
...
const show = async (rest: readonly string[]): Promise<number> => {
  const [qid] = rest;
  if (!qid) {
    console.error('oracle show: missing question id — usage: folklore oracle show <qid>');
    return 1;
  }
  const rt = await defaultRuntime();
  if (rt.isErr()) {
...
const USAGE = `usage: folklore oracle <ask|answer|list|show|answerable>
...
surface. Run \`folklore daemon start\` (or ensure peers are connected)
