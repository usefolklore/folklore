/**
 * `folklore entity <sub>` — manage the entity registry.
 *
 *   add <label> [--alias A] [--alias B] [--type T] [--note ...]
 *   list [--json]
 *   remove <id>
 *
 * The registry lives at $FOLKLORE_HOME/entities.json. It's the
...
const USAGE = `usage: folklore entity <sub>
...
const add = async (rest: readonly string[]): Promise<number> => {
  ...
  console.log(`run \`folklore recall ${label}\` after ingest to see hits.`);
  return 0;
};
...
const list = async (rest: readonly string[]): Promise<number> => {
  ...
  if (allEnts.length === 0) {
    ...
    console.log('add one with: folklore entity add <label> [--alias A] [--type T]');
    return 0;
  }
  if (ents.length === 0) {
