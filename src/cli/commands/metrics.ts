/**
 * `folklore metrics` — emit the daemon's live metrics snapshot.
 *
 ...
 * shim in `bin/folklore.js` intercepts this command BEFORE we
 * reach this fallback and routes it over the unix socket.
 *
 * This file only fires when:
 *   - the user runs `folklore metrics` with no daemon running, OR
...
/**
 * `folklore metrics bypass [--json] [--since <iso>]`
 *
 ...
 *   - FOLKLORE_DENY_ON_TERMINAL=0 (soft persuasion only): some
 *     bypass is expected; the rate measures how often the contract
 *     block is ignored.
 *   - FOLKLORE_DENY_ON_TERMINAL=1 (hard deny): bypass should
...
const bypassSummary = (args: readonly string[]): number => {
  ...
  console.log(`folklore metrics bypass (window: ${out.window_since})`);
  ...
  if (bypassPassed > 0) {
    ...
    console.log(`  off (FOLKLORE_DENY_ON_TERMINAL=0 is the default) or the harness`);
...
export const metricsCmd = async (args: string[]): Promise<number> => {
  ...
  const note = existsSync(sock)
    ...
    : 'metrics live in the daemon process. start `folklore daemon` to populate.';
  console.log(
    JSON.stringify({
      counters: {},
      gauges: {},
