/**
 * `folklore peers rep [<peer-id>] [--subject <key>] [--json]`
 *
 ...
 * consumers (CI tooling, the future `folklore audit export`).
 */
...
const renderAllHuman = (rows: readonly PeerRow[]): void => {
  if (rows.length === 0) {
    console.log('no peer reputation data yet — needs at least one federated ask with `folklore ask --peers`.');
    return;
  }
...
const USAGE = `usage: folklore peers rep [<peer-id>] [--subject <key>] [--json]
