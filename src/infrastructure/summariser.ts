 ...
 * via `WELLINFORMED_OLLAMA_MODEL` or the OllamaClientOptions.model.
 */
 ...
 *
 * Order of precedence:
 *   1. `WELLINFORMED_SUMMARISER=fixture` → fixtureSummariser (tests)
 *   2. `WELLINFORMED_SUMMARISER=ollama` OR ollama unset OR default →
...
export const summariserFromEnv = (
  ...
  const choice = (process.env.WELLINFORMED_SUMMARISER ?? '').toLowerCase();
  if (choice === 'fixture') {
    return fixtureSummariser({
      fallback: process.env.WELLINFORMED_SUMMARISER_FIXTURE ?? 'fixture-summary',
    });
  }
