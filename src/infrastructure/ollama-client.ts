 ...
 * override via FOLKLORE_OLLAMA_URL at the runtime layer.
 */
export const ollamaClient = (opts: OllamaClientOptions = {}): OllamaClient => {
  const baseUrl = (opts.baseUrl ?? process.env.FOLKLORE_OLLAMA_URL ?? DEFAULTS.baseUrl).replace(/\/$/, '');
  const defaultModel = opts.model ?? process.env.FOLKLORE_OLLAMA_MODEL ?? DEFAULTS.model;
