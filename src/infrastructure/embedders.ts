 ...
 *
 * Spawns the `folklore-rs/target/release/embed_server` binary on
...
export const rustSubprocessEmbedder = (opts: RustSubprocessOptions): Embedder => {
  const binaryPath =
    opts.binaryPath ??
    process.env.FOLKLORE_RUST_BIN ??
    // Default path assumes the repo layout: folklore-rs is a sibling
 ...
 * encoder. Measured gain on the live folklore stack (bench-embed-
