  ...
  while (true) {
    try {
      ...
      if (Date.now() >= deadline) {
        throw new Error(`timed out waiting for sessions-state.json lock after ${LOCK_MAX_WAIT_MS}ms`, { cause: e });
      }
 ...
 * folklore processes (e.g., daemon + CLI) from racing and clobbering
