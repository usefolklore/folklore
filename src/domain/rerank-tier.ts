...
export interface RerankEnv {
  ...
  /**
   * Master kill-switch — `WELLINFORMED_RERANK=0` forces `none` tier
...
export const rerankEnvFromProcess = (env: NodeJS.ProcessEnv = process.env): RerankEnv => {
  const override = env.WELLINFORMED_RERANK_TIER as RerankTier | undefined;
  ...
  return {
    ...
    modelOverride: env.WELLINFORMED_RERANK_MODEL || undefined,
    latencyBudgetMs: env.WELLINFORMED_RERANK_LATENCY_MS
      ? Number(env.WELLINFORMED_RERANK_LATENCY_MS)
      : undefined,
    headSizeOverride: env.WELLINFORMED_RERANK_HEAD
      ? Number(env.WELLINFORMED_RERANK_HEAD)
      : undefined,
    disabled: env.WELLINFORMED_RERANK === '0',
  };
};
 ...
 *
 * Order of precedence:
 *   1. Explicit env override (`WELLINFORMED_RERANK_TIER`)
 *   2. Master kill-switch (`WELLINFORMED_RERANK=0`)
...
export const pickRerankTier = (
  ...
  if (env.disabled) {
    return {
      tier: 'none',
      headSize: 0,
      reason: 'WELLINFORMED_RERANK=0 (master kill-switch)',
      latencyBudgetMs: 0,
    };
  }
