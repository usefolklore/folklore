#!/bin/bash
# folklore v2.0 comprehensive benchmark
...
**folklore:** $($WI version 2>/dev/null || echo "unknown")
...
bench_capture "reindex folklore (116 files)"       $WI codebase reindex 19a0c7525684eded
...
graph_size=$(wc -c < ~/.folklore/graph.json 2>/dev/null || echo 0)
vectors_size=$(wc -c < ~/.folklore/vectors.db 2>/dev/null || echo 0)
code_size=$(wc -c < ~/.folklore/code-graph.db 2>/dev/null || echo 0)
...
research_nodes=$(sqlite3 ~/.folklore/vectors.db "SELECT COUNT(*) FROM vec_meta" 2>/dev/null || echo 0)
code_nodes=$(sqlite3 ~/.folklore/code-graph.db "SELECT COUNT(*) FROM code_nodes")
code_edges=$(sqlite3 ~/.folklore/code-graph.db "SELECT COUNT(*) FROM code_edges")
codebases=$(sqlite3 ~/.folklore/code-graph.db "SELECT COUNT(*) FROM codebases")
...
| File | Size |
|------|------|
| ~/.folklore/graph.json (research graph) | $(echo "scale=2; $graph_size / 1024 / 1024" | bc) MB |
| ~/.folklore/vectors.db (ONNX vectors + meta) | $(echo "scale=2; $vectors_size / 1024 / 1024" | bc) MB |
| ~/.folklore/code-graph.db (Phase 19 code graph) | $(echo "scale=2; $code_size / 1024 / 1024" | bc) MB |
...
Generated by: \`bench/bench-v2.sh\`
EOF
