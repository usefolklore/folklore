...
const RESULTS_DIR = join(CACHE_DIR, 'qrel-rejudge-v3-gptoss20b');
...
const judgeOne = async (claim, passage) => {
  ...
  const res = await fetch(`${OLLAMA_URL}/api/generate`, {
    ...
    body: JSON.stringify({
      ...
      prompt,
      stream: false,
      options: { temperature: 0, num_predict: 250 }, // CoT needs more tokens
    }),
  });
...
const result = {
  ...
  cohens_kappa_v1: 0.4181, cohens_kappa_v2: 0.4577,
  delta_kappa: v2VsV1,
