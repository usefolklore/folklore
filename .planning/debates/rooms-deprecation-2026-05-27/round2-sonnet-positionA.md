...
The correct answer is a **per-node `private: bool` flag** — a single boolean field on every graph node, defaulting to `false`. The sharing rule becomes: "sync all nodes where `private = false` to trusted peers." The user runs `wellinformed save --private` for anything they don't want federated. Done.
...
2. Add `private: bool` (default `false`) to the graph node schema. Wire `wellinformed save --private` to set it. Update sharing to filter on this field instead of `shared-rooms.json`. This is ~30 lines and makes `shared-rooms.json` obsolete without a protocol break.
