...
Tag creation is not a user action; it's a *consequence* of save-context. `wellinformed save --tag "p2p-llm"` doesn't create a tag — it stamps the string. There is no `addTag(tag) → Result<void, RoomError>`. There is no `TagRegistry`. There is no "tag doesn't exist" error path.
...
wellinformed share room p2p-llm
