...
**Source curation.** Today `init` uses hand-curated `RoomMeta.keywords` (rooms.ts:26-28). Under tags, a "room" is just a tag — the user (or LLM) stamps `akashik-dev` on relevant sources. Keywords were always a poor man's embedding; drop them.
...
- Migration: on landing day, every existing node gets `tags: [existing_room]`. The 5 user rooms (`akashik-dev`, `p2p-llm`, `tlvtech`, `forge`, `auto-tlv`) become 5 tags. `shared-rooms.json` becomes `share-patterns.json` with the same policy (e.g., `share_patterns: ["p2p-llm", "oss:*"]`). No data loss, one-shot transform.
...
One-time migration. Each node in the graph gets `tags: [room]` stamped (the room field becomes a tag). The user (or a `akashik migrate` command) can re-tag as needed. `default_room` becomes a concept that doesn't exist — queries match against any tag. The "wrong default room" bug (`SYNTHESIS.md:8`) ends permanently because there is no default.

**Axis 4: Sharing UX.**
Today `share.ts:234-305` is a full TUI picker over rooms. Under tags, the user gets:
- A one-time `--share-pattern "oss:*"` policy.
- Or per-node sharing via `akashik tag <node> --share`.
...
2. **Migration**: `akashik migrate rooms-to-tags` — stamps `tags: [room]` on every node, writes `share-patterns.json` from `shared-rooms.json`.
