...
3. Add `private: bool` to graph node schema; wire `wellinformed save --private`
...
3. **Add `node.private: boolean`** (defaults `false`) to the graph node schema. Wire `wellinformed save --private` to set it.
...
- `wellinformed save "something"` from `~/personal/akashik` → routed to room `akashik` (auto-created), tagged `workspace: akashik`. User never sees the room machinery.
- `wellinformed save "lead info" --private` from anywhere → stored locally, never federates, regardless of room.
- `wellinformed ask "..."` from `~/personal/akashik` → returns nodes from this workspace first, no `--room` flag needed.
- `wellinformed room` (the CRUD CLI) → command not found. The 171 lines that produced this pain are gone.
