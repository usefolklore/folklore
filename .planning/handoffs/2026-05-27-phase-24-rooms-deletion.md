...
The work is on `feat/delete-rooms` and **not yet merged**. The user has not run `wellinformed migrate v5` on their live `~/.wellinformed/` data yet (recommended before merge).

---

## How we got here (decision chronology)

1. **Initial complaint:** User saw "wellinformed • tlvtech" in their Claude Code statusline while working in the `akashik` repo. The brand was supposed to have been swept to "Akashik" already (prior conservative rebrand at commit `5d7fa16` did 20 docs). Two separate problems: (a) "wellinformed" label leaking through, (b) "tlvtech" as default room was their stale cold-email research room from `~/.wellinformed/rooms.json`.
2. **Quick fixes:**
   - Statusline label `.claude/helpers/wi-statusline.cjs:205` changed from "wellinformed" to "Akashik". (User had to explicitly authorize — auto-mode classifier blocked self-modification first attempt.)
...
- Rebrand sweep (docs/) — many user-facing markdown files now say "Akashik" instead of "wellinformed" in prose; technical identifiers preserved per conservative rules
...
**24-05:** 6 Claude Code hooks updated to drop `room` field from hit formatters: `wellinformed-{smart-hook, prompt-submit, mcp-pre, post-fetch}.cjs` + `wellinformed-{session-start, session-capture}.sh`. Format now shows `[workspace, age, peer]` triple with `-` fallback. All cjs pass `node --check`, both shells pass `bash -n`. 7 commits.
...
**24-11:** Built `src/cli/commands/migrate.ts` (309 lines new). Registered in `cli/index.ts`. **`wellinformed migrate v5`** is:
...
- Atomic backup → `~/.wellinformed/graph.v4-backup.json` before write
- Includes `--rollback` flag

`wellinformed doctor` extended to warn on V4-dirty home (rooms.json or shared-rooms.json present, or any node with `room` field). Once migrated, prints `[ ok ] N/N sampled nodes V5-clean`.
...
1. ✓ `wellinformed save "x"` routes to workspace-derived slice, `private: false` default, never sees room concept
2. ✓ `wellinformed ask "x"` from git repo filters by workspace; `--workspace all` opt-out works
3. ✓ `wellinformed share <peer>` shares all `private === false` nodes; no `--room` flag, no `shared-rooms.json` to maintain
...
1. **End-to-end private-node share** between two real peers. Confirm: `wellinformed save --private "secret"` → not federated; peer B doesn't receive it. Then `wellinformed save "public"` → peer B does receive it.
2. **Real-data migrate idempotency** on the user's live `~/.wellinformed/` (which has 5 rooms with accumulated data). Run `wellinformed migrate v5` once; verify backup exists; verify graph.json is V5; re-run and verify "Already on V5." exit. Then optionally `wellinformed migrate v5 --rollback` and confirm V4 data restored. Then re-migrate for the final state.
...
- Did **not** run `wellinformed migrate v5` on the user's live `~/.wellinformed/` data. Migration command exists and is verified on synthetic V4 fixtures, but the live data is still V4.
- Did **not** push to remote.
- Did **not** merge to `main`.
- Did **not** delete `~/.wellinformed/rooms.json` or `~/.wellinformed/shared-rooms.json` on disk (those will be deleted by `wellinformed migrate v5` when the user runs it).
