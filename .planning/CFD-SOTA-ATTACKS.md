...
KG probe (`mcp__folklore__search`) on multigrid retrieval, Krylov + IR, Petrov-Galerkin retrieval, and per-dim preconditioning returned only generic RAG indexes (vearch, RAG_Techniques, AutoRAG) and Matryoshka/transformer-family pages. **None of these CFD operators have been applied to dense+BM25 retrieval in this codebase or in indexed prior art.** Net-new attack surface.
...
**e. Effort.** **2 hours.** Compute `s` once offline from cached corpus vectors in `~/.folklore/bench/scifact__rust-via-ts__bge-base/vectors.db` (~50 ms), persist as a 3KB Float32Array, apply at query/index time via `src/domain/vectors.ts` ~15 LOC. Re-rank cached top-100 — no re-embed, no re-index.
