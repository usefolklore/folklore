...
### Current path
```
prompt → hook → wellinformed ask --peers --json
  ↓
...
1. **Replace per-peer touch dial with gossipsub broadcast** — single
   `publish(/wellinformed/ask/1.0.0, q)` reaches all peers in one
...
### Acceptance
- `wellinformed ask --peers` p50 ≤ 50ms over 10 peers
...
sometimes still calls `wellinformed search`, `wellinformed ask`,
`wellinformed get_node` 1–3 more times before responding.
...
2. **Add a terminal contract hint.** The hook's contract block
   ...
   Do not call wellinformed/Grep/Read/WebSearch — those are
   redundant when terminal:true."
   - File: `.claude/hooks/wellinformed-prompt-submit.cjs`
...
3. **Pre-fan to MCP tools.** When the hook fetches federated
   ...
   `mcp__wellinformed__ask` calls in the same session return the
...
## 3. 10k simulated peers (`wellinformed swarm sim`)

### Plan

1. **New mode: `wellinformed swarm sim --count 10000 --corpus ...`**
...
2. **Corpus generator: `wellinformed swarm gen --count 10000
...
### Acceptance
- `wellinformed swarm sim --count 10000` boots in ≤ 10s.
- `wellinformed ask --peers "..."` against a 10k swarm:
