...
- Runtime budget: 30-45 seconds. Tagged as "slow"; skippable via `FOLKLORE_SKIP_SLOW=1`
...
if (cfg.relays) {
  for (const relayAddr of cfg.relays) {
    await node.dial(multiaddr(relayAddr)).catch((e) => {
      process.stderr.write(`folklore: relay dial failed for ${relayAddr}: ${(e as Error).message}\n`);
    });
  }
}
```
...
const SKIP = process.env.FOLKLORE_SKIP_SLOW === '1';
...
**How to avoid:** Use a two-step heuristic: (1) check if `node.getPeers()` still contains the peer ID immediately after the event (within the same microtask); if NOT present, the peer fully disconnected. (2) Only count as "unexpected" if the peer had previously been `keep-alive`-tagged. Peers tagged via `dialAndTag` have the `keep-alive-folklore` tag — their disconnects are unexpected. Peers without the tag are ephemeral connections that can close normally.
...
| NET-04 | 10-peer mesh: all connected to 3+ peers | integration (slow, real libp2p nodes) | `FOLKLORE_SKIP_SLOW=0 node --import tsx --test tests/phase18.production-net.test.ts` | ❌ Wave 0 |
...
- **Phase gate:** Full suite green before `/gsd:verify-work`. Slow test must pass with `FOLKLORE_SKIP_SLOW=0`.
...
- All existing project source files read directly from `/Users/saharbarak/workspace/folklore/src/`
