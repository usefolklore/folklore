...
| PreToolUse + SessionStart hooks | Idempotent install via `folklore claude install` |
...
| **SESS-05** | `folklore recent-sessions list [--hours --project --limit --json]` CLI | `src/cli/commands/recent-sessions.ts` + `rollupSessions` helper | phase20 describe F | ✓ |
...
|---|-----------|--------|----------|
| 1 | Session JSONL files indexed automatically via daemon tick into `sessions` room | ✓ | Daemon log: `tick: room=folklore-dev new=34`; sources.json shows `claude-sessions-default` enabled; rooms.json has `sessions` room created at daemon bootstrap |
| 2 | `folklore recent-sessions` CLI shows recent sessions with duration + tool-call summary | ✓ | CLI emits structured output or "no sessions" message — command wired + dispatcher registered |
...
1. **Initial historical backlog ingest is slow.** The `~/.claude/projects/` directory contains thousands of historical JSONL entries. On first run, ONNX embedding at ~20 docs/sec means the full backlog takes tens of minutes to index. This is not a bug — it's the expected cost of batch ingestion. Subsequent ticks are fast (incremental via byteOffset). For users who want faster initial bootstrap, they can set `interval_seconds: 60` in config or manually run `folklore trigger --room sessions` and let it complete in the background.
