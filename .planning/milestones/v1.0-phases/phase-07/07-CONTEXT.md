...
Telegram bot that serves as the mobile interface to folklore: forward links for ingest, receive daily digests, query the graph from your phone. Single-user, long-polling, runs inside the daemon process.
...
- BotFather setup documented in CLI `folklore telegram setup`
...
- Config loaded from ~/.folklore/config.yaml via config-loader.ts
...
- BotFather is the setup mechanism — `folklore telegram setup` should guide token creation
