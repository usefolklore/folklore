# Enterprise Architect Audit — akashik v2.0
...
2. **No composition root.** The only place the P2P stack is wired together is inside `startLoop`. There is no `bootstrap.ts` / `composition-root.ts` / `wire.ts`. If you wanted to run the P2P stack without the daemon (e.g. for an on-demand `akashik peer connect-now` command), you'd need to copy the 120 lines.
...
- A user running `akashik daemon start` AND `akashik peer add /ip4/.../tcp/...` opens **two nodes with the same ed25519 identity** on different ports.
...
- This is a classic "daemon vs CLI one-shot" split-brain. Syncthing solves this by making the CLI a thin HTTP client against the running daemon. akashik should do the same: CLI commands that touch the network should dispatch to the daemon via a Unix domain socket, not open parallel nodes.
...
**What akashik's share layer actually guarantees:**
...
1. **14 patterns cover ~5-15% of real-world secrets by count.** Serious secret scanners (TruffleHog v3, GitHub's secret scanning, gitleaks) ship **500-900 patterns** and use entropy-based detection to catch the long tail. The akashik scanner catches the top 14 by memorability, not by real-world distribution. A Postgres connection string `postgres://user:password@host` is not caught. An arbitrary 32-byte hex API key is not caught. A `.pem` file content is caught only by the header, not by the body.
...
**Context.** Phase 15-18 shipped ed25519 identity + the full libp2p stack (TCP, Noise, Yamux, mDNS, kad-dht, circuit-relay-v2, dcutr, upnp-nat, identify, crypto, peer-id) + Y.js room sharing over a custom `/akashik/share/1.0.0` protocol + federated search over `/akashik/search/1.0.0`. ~2500 LOC total, 121 tests, 10-peer in-process integration test.
...
**Drivers.** The real driver is positioning: "the only MCP memory tool with peer sharing." This is a real moat in the BENCH-COMPETITORS table — mem0, Graphiti/Zep, Letta, Engram, mcp-memory-service all have zero P2P. akashik's moat is defensible.
...
- **Bad:** 11 package dependencies, all of which need to be upgrade-tracked. libp2p 4.x is already in beta — akashik is pinned at 3.2.0 and will need a migration within 6 months. circuit-relay-v2 hop-relay server is explicitly out of scope ("deferred to v3") — so the feature only works if someone else runs a hop-relay, which nobody does. dcutr hole-punching requires both peers to be libp2p dcutr-capable — akashik is its only user, so it only works akashik-to-akashik.
...
| **Custom share protocol `/akashik/share/1.0.0`** | **D** | Pure Cool Tech. y-libp2p exists. Writing a custom length-prefixed framing + SubscribeRequest JSON envelope with 7 numbered pitfalls is the exact thing a small team shouldn't own. |
...
What IS v2.1? The README is positioning akashik as **both** (a) a research-lab SOTA retrieval system (BENCH-v2 4-wave writeup) and (b) a production MCP tool users adopt in 3 commands (Install → Try it → Wire into Claude Code). These two products have **opposite** engineering priorities:
...
**akashik's moat is the honest benchmarks, the multi-source ingestion breadth, and the MCP-native integration** — everything outside that (P2P protocol internals, tree-sitter parsing, pattern detection, DHT, relay) is cost, not moat, and v2.1 should shrink the cost surface and double down on the moat surface.
