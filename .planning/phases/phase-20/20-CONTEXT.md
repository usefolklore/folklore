...
- State file: `~/.akashik/sessions-state.json` with `{ version: 1, files: { [filePath]: { mtime, byteOffset, lastLineNum } } }` — mirrors the `peers.json` / `shared-rooms.json` schema pattern
...
- Dedicated `sessions` room auto-provisioned on first ingest. Distinct from `akashik-dev` and all research rooms. Never the default room
- PreToolUse hook: extend the EXISTING `.claude/hooks/akashik-hook.sh` that `akashik claude install` drops. Add a SessionStart branch that runs `akashik recent-sessions --hours 24 --json` and emits the first session summary as additionalContext. Hook stays idempotent — doesn't change what it emits for other events
...
- Extended files:
  ...
  - `.claude/hooks/akashik-hook.sh` (generated by `akashik claude install`) — extend with SessionStart branch. Also update the install command to emit the new hook body
...
- Files NOT touched:
  ...
  - `~/.akashik/graph.json` and `vectors.db` — sessions room is additive, does not touch existing nodes
...
- The `.claude/projects/-Users-saharbarak-workspace-akashik/` directory on disk already has multiple `<session-id>.jsonl` files (confirmed via ls) — Phase 20 can run retroactively on the historical data to validate the adapter
...
- **Session replay / timeline UI** — Phase 20 ships data, not visualization. A later `akashik viz sessions` command could render a timeline
- **Cross-session pattern mining** — e.g. "you tend to fix Make.com scenarios on Mondays". Out of scope
- **Automatic task resumption** — the hook surfaces context but doesn't auto-continue tasks. That's a Claude Code feature not a akashik feature
