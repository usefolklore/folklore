...
dependency_graph:
  ...
  provides:
    - "wellinformed share <peer> [--audit-only] [--json] — peer-only V5 share command"
    - "wellinformed unshare <peer> — peer-removal-only V5 unshare command"
...
- **share.ts collapsed from 331 -> 157 lines.** Three subcommands (`audit`, `room`, `ui`) replaced by a single direct shape: `wellinformed share <peer-id>`. The audit subroutine survives as the default pre-flight output (printed before persisting); `--audit-only` exits early.
...
- **Hooks (24-04, 24-12):** none of the 5 wellinformed hooks reference share/unshare directly (verified clean).
