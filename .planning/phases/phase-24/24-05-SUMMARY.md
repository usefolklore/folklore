...
key_files:
  created: []
  modified:
    - path: ".claude/hooks/folklore-smart-hook.cjs"
      change: "renderHits formatter: [h.room ?? '?'] → [h.workspace ?? '-']; docstring updated"
    - path: ".claude/hooks/folklore-prompt-submit.cjs"
      change: "Drop h.room guard from auto-pull condition; drop room from (peer, room) target key (peer-only); drop --room from `folklore touch`; hit formatter renders workspace; topic-fallback uses workspaces"
    - path: ".claude/hooks/folklore-mcp-pre.cjs"
      change: "Banner `domains` slot: drop rooms fallback (topics.join only)"
    - path: ".claude/hooks/folklore-post-fetch.cjs"
      change: "Drop `const ROOM = 'research'`; drop --room from save argv; docstring updated to 'V5: global graph, public (not --private)'"
    - path: ".claude/hooks/folklore-session-start.sh"
      change: "Drop ROOMS env var probe; drop ROOM_COUNT segment from statusline"
    - path: ".claude/hooks/folklore-session-capture.sh"
...
requirements_delivered:
  - "ROOMS-DEL-08 (claude-hooks subsystem) — all 6 folklore-* hook scripts format hits without `room` field; none pass `--room` to any sub-CLI; none read `~/.folklore/rooms.json`"
---

# Phase 24 Plan 05: Folklore Claude Code Hooks — Room Removal Summary

**One-liner:** Excised every `h.room` reference, `--room` flag, and `rooms.json` probe from the 6 folklore Claude Code hook scripts; hit formatters now render `workspace` (fallback `-`), boot-time auto-capture writes raw nodes with `private: false`, and post-fetch auto-save lands web results in the global graph without a room flag — 20 net LOC removed, all 6 hooks pass syntax check + smoke test.

## What Was Built

### 1. `folklore-smart-hook.cjs` (PreToolUse: Grep / Glob / Read / WebSearch / WebFetch)
...
### 2. `folklore-prompt-submit.cjs` (UserPromptSubmit)
...
- **Auto-pull target dedup (lines ~169-184):** key collapsed from `${peer}::${room}` to plain `peer`. `targets` array now carries `{ peer }` only. `folklore touch <peer> --max 10` (no `--room` arg). Comment updated: "V5: auto-touch-by-room is removed along with the room primitive."
...
### 3. `folklore-mcp-pre.cjs` (PreToolUse: `mcp__folklore__*`)
...
### 4. `folklore-post-fetch.cjs` (PostToolUse: WebSearch / WebFetch)

- Dropped `const ROOM = 'research';` constant + the surrounding paragraph about system rooms.
- `folklore save` argv: dropped `--room ROOM`. Now `['save', '--type', 'source', '--label', label]`.
...
### 5. `folklore-session-start.sh` (SessionStart MOTD)

- Dropped the `ROOMS=` env-var assignment that pointed at `~/.folklore/rooms.json`.
- Dropped `ROOM_COUNT=$(grep -c ... "$ROOMS")` probe.
- Statusline string: `━━ folklore ━━ N nodes │ E edges │ R rooms │ S sources` → `━━ folklore ━━ N nodes │ E edges │ S sources`. (No workspace segment substituted — the existing script has no workspace variable available, and adding cwd detection here would change boot timing.)

### 6. `folklore-session-capture.sh` (Stop hook)
...
```
folklore: 3 indexed node(s) match "test query about graphrag" (queried 3/4 peer(s))
  1. GraphRAG paper [folklore, 3d, local] d=0.92 — Microsoft GraphRAG hierarchical clustering
  2. V5 protocol design [-, 2w, peer:12D3KooWAbCd] d=1.01
  3. rerank study [folklore-dev, 3mo, local] d=0.98
```

Verified: workspace renders cleanly (`folklore`, `folklore-dev`), absent-workspace fallback shows as `-` (hit #2), age and peer slots unchanged. No crashes, no undefined slots.
...
| File | +ins | -del | Net |
|------|-----:|-----:|-----:|
| `folklore-smart-hook.cjs` | 3 | 3 | 0 |
| `folklore-prompt-submit.cjs` | 13 | 13 | 0 |
| `folklore-mcp-pre.cjs` | 4 | 5 | -1 |
| `folklore-post-fetch.cjs` | 6 | 12 | -6 |
| `folklore-session-start.sh` | 1 | 3 | -2 |
| `folklore-session-capture.sh` | 6 | 17 | -11 |
| **Total** | **33** | **53** | **-20** |
...
The plan's note "DELETE the python block at line ~15 that probes ~/.folklore/rooms.json for DEFAULT_ROOM" and "DELETE lines ~22-23 that set DEFAULT_ROOM" were collapsed into a single contiguous block removal because, in the actual file, the probe and the assignment are part of the same python heredoc (not two separate stages). The behaviour is identical.

`folklore-hook.sh` is NOT in the plan's file list and was NOT modified by this plan. It carries a pre-existing in-conversation edit (visible in `git status`) that's outside this plan's scope — left for a separate commit.

## Acceptance Criteria — All Pass

```
grep -rnE "h\.room|--room\b|rooms\.json|DEFAULT_ROOM|ROOM_COUNT|const ROOM\s*=|rooms\.join" \
  .claude/hooks/folklore-*.cjs .claude/hooks/folklore-*.sh
→ (no hits) PASS

node --check .claude/hooks/folklore-smart-hook.cjs        → OK PASS
node --check .claude/hooks/folklore-prompt-submit.cjs     → OK PASS
node --check .claude/hooks/folklore-mcp-pre.cjs           → OK PASS
node --check .claude/hooks/folklore-post-fetch.cjs        → OK PASS
bash -n .claude/hooks/folklore-session-start.sh           → OK PASS
bash -n .claude/hooks/folklore-session-capture.sh         → OK PASS
...
The hook scripts are now in lockstep with the V5 wire protocol break — they won't pass `--room` to a `folklore save` / `folklore touch` CLI that no longer accepts it, and they won't read `rooms.json` once it's deleted from disk by the `migrate v5` command (Plan 24-09 territory).

Downstream consumers:

- **Plan 24-04** (`runtime.ts` + `detectWorkspace`): the save-CLI side of the contract. When the post-fetch hook fires `folklore save --type source --label X`, the CLI now reaches into cwd, runs `git rev-parse --show-toplevel`, slugifies the basename, and writes `workspace: <slug>` onto the node. Already committed (`a775896`).
...
- **`folklore-hook.sh`** (the legacy PreToolUse + SessionStart shim): pre-existing in-conversation modification. Not touched by this plan. The legacy branch doesn't reference rooms at all (just a node count), so it's already V5-clean.
