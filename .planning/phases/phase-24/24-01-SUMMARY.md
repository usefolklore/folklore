...
dependency_graph:
  requires: []
  provides:
    - "FolkloreNodeFields with workspace + private, no room"
...
**Final shape of `FolkloreNodeFields`:**
...
- **1d — Hook scripts:** 5 `.claude/hooks/folklore-*` files (drop `room` from hit formatter; show `workspace` when present).
...
Wave 4: `folklore migrate v5` command + new `tests/phase24.rooms-deleted.test.ts` + edits to ~13 existing tests.
