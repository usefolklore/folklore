...
- One-shot command: `folklore migrate v5`. Idempotent. Lossless except the `room` field is dropped onto an optional `workspace` field where possible (heuristic: room name slugified matches a known repo basename → workspace; else null).
- Existing 5 rooms (`folklore-dev`, `p2p-llm`, `tlvtech`, `forge`, `auto-tlv`) → flat graph. `private: false` on all (user marks sensitive nodes private after).
...
- FolkloreBench-F + niche-evaporation work. Moved to separate phase (TBD).
</user_constraints>
...
|----|-------------|-----------------|
| ROOMS-DEL-01 | `folklore room` CLI removed; no subcommand routes to room CRUD | `src/cli/commands/room.ts` (172 lines) deleted; `src/cli/index.ts` dispatcher line removed; `src/cli/runtime.ts:208` (`fileRoomsConfig` wiring) removed |
| ROOMS-DEL-02 | `~/.folklore/rooms.json` no longer read or written | `rooms-config.ts` deleted; `runtime.ts:135` (`rooms: join(home, 'rooms.json')`) removed; daemon `RoomsConfig` import gone; migration command deletes the file |
| ROOMS-DEL-03 | `~/.folklore/shared-rooms.json` removed; sharing gates on `node.private === false` | `share-store.ts` deleted (322 lines); `share-sync.ts` line 60 import gone; `share-sync.ts:557` `sharedRoomsPath` field gone; sharing path rewrites to `graph.nodes.filter(n => !n.private)` |
| ROOMS-DEL-04 | `GraphNode` schema: `room` removed, `workspace?: string` + `private: boolean` added | `src/domain/graph.ts:45` (`FolkloreNodeFields`): drop `room?: Room`, add `workspace?: string` + `private: boolean`; drop `nodesInRoom()` line 213; drop `roomFilter()` line 430; drop `TraversalOptions.room` line 132 |
...
| ROOMS-DEL-06 | `folklore migrate v5` exists, idempotent, lossless except `room → workspace` heuristic | New file `src/cli/commands/migrate.ts`; reads v4 graph via legacy types; writes v5 graph via new types; reads/deletes rooms.json + shared-rooms.json; flattens peer-reputation.json |
...
| ROOMS-DEL-08 | All `.claude/hooks/folklore-*` scripts drop `room` field, pass test suite | 5 hooks affected: `smart-hook.cjs:178`, `prompt-submit.cjs:164/175/233`, `mcp-pre.cjs:201/207`, `session-start.sh:6/38`, `session-capture.sh:15/42`, `post-fetch.cjs:7/72` |
</phase_requirements>
...
2. **The graph schema must be migrated BEFORE any consuming code can compile.** `GraphNode.room` is a typed field. Removing it from the `FolkloreNodeFields` interface (graph.ts:45) breaks every importing file simultaneously. This forces a "Wave 0: schema first" — the type change drives the rest of the diff like a wedge through the compiler.
...
**Files:**
- `src/domain/graph.ts` — drop `Room`/`Wing` type aliases, drop `room?: Room` from `FolkloreNodeFields`, add `workspace?: string` + `private: boolean`, drop `nodesInRoom`, drop `roomFilter`, drop `TraversalOptions.room`.
...
- Update 5 `.claude/hooks/folklore-*` files to drop `room` from hit-formatter, optionally display `workspace` when present, remove `--room` flag passing to `folklore save`, drop `rooms.json` probes from session-start/capture shell scripts.
...
- Drop per-room Y.Doc map; use single global `~/.folklore/graph.ydoc`.
...
- **4d:** Wire `migrate v5` into `src/cli/index.ts` dispatcher. Update `folklore doctor` to check for v5-readiness (no `room` field on any sampled node).
...
- Run `folklore migrate v5` against a snapshot of the user's real ~/.folklore → verify all 21,128 nodes migrate, heuristic workspace assignment count is reasonable (~3,481 in CONTEXT.md sample), reputation file shrinks.
...
Target shape:
- One global `Y.Doc` at `~/.folklore/graph.ydoc`.
...
The on-disk schema (`~/.folklore/peer-reputation.json`) v1:
```json
{
  "version": 1,
  "peers": {
...
### `.claude/hooks/folklore-*` (Wave 1d, ROOMS-DEL-08)

Specific edits:

**`folklore-smart-hook.cjs:178`** — hit formatter:
```js
// BEFORE:
...
**`folklore-prompt-submit.cjs`** — multiple touchpoints:
...
**`folklore-mcp-pre.cjs:201-208`** — domain extraction:
- Drop the `rooms` fallback (`rooms.join(', ') || 'local'`). Use `topics.join(', ') || 'local'` directly.

**`folklore-session-start.sh:6, 38`** — drops the `ROOMS` env probe and the `${ROOM_COUNT} rooms` segment of the statusline string. Replace with `${WORKSPACE} workspace` or just drop.

**`folklore-session-capture.sh:15, 22-23, 42`** — entire `rooms.json` probe is dead. Drop the python `default_room` read; drop the `'room': '$DEFAULT_ROOM'` field in the save invocation.

**`folklore-post-fetch.cjs:34, 72`** — drops `ROOM = 'research'` constant; drops `--room`, `ROOM` from save args. The post-fetch save no longer has a room target — it lands as a global node. Privacy: NOT marked `--private` (the whole point is that web fetches are shareable).
...
### `folklore migrate v5` Flow

```
1. Detect — read first node from ~/.folklore/graph.json:
...
2. Backup
   - Copy ~/.folklore/graph.json → ~/.folklore/graph.v4-backup.json (atomic copy)
3. Load v4 data
...
7. Delete v4 artifacts
   - rm ~/.folklore/rooms.json
   - rm ~/.folklore/shared-rooms.json
   - rm ~/.folklore/shared-rooms.json.lock (if present)
...
```
$ folklore migrate v5 --rollback
Reading ~/.folklore/graph.v4-backup.json...
...
Or run `folklore migrate v5` again to re-apply.
```
...
- Running `folklore migrate v5` twice in a row: second call exits 0 with `"Already on V5"` (detected via no `room` field on sampled nodes).
- Running `folklore migrate v5` after a manual partial undo: best-effort idempotent. The v4-detection at step 1 governs whether full migration runs.
...
**Mitigation:** Add a `folklore doctor` check at boot — sample 10 random nodes; if any has a `room` field, print a yellow warning: `"⚠ V4 data detected; run \`folklore migrate v5\` to upgrade."`. Non-fatal — the system keeps working, just with vestigial room fields ignored.
...
|--------|----------|-----------|-------------------|--------------|
| ROOMS-DEL-01 | `folklore room` returns "command not found" | unit | `node --import tsx --test tests/phase24.rooms-deleted.test.ts` (group: "CLI dispatch") | ❌ Wave 4 |
| ROOMS-DEL-02 | No code path opens `~/.folklore/rooms.json` | structural | `grep -rn "rooms.json" src/ \| grep -v test \| wc -l` returns 0 | structural assertion in test |
...
| ROOMS-DEL-06 | `migrate v5` idempotent + lossless | integration | spin up tmp `~/.folklore`, write v4 fixture, run migrate twice, assert end state | ❌ Wave 4 |
| ROOMS-DEL-07 | Workspace pre-filter active in git repo | integration | spawn `folklore ask` in a tmp git repo, assert workspace tag on returned hits | ❌ Wave 4 |
...
2. **`fromJson` silently accepts v4 data.** `GraphNode` extends `Readonly<Record<string, unknown>>`, so `room: 'tlvtech'` round-trips without error in v5. A user who skips `migrate v5` would have a corrupted-but-working graph. **Mitigation:** `folklore doctor` boot-check + clear migration prompt on first v5 run.
...
12. **`folklore doctor` is an existing CLI (5,712 bytes).** Should grow a V5-readiness check (no `room` fields on sampled nodes). The doctor is the natural surface for the post-migration check + the boot-time warning.
...
17. **`federation-sim.ts` (15,209 bytes) is the FolkloreBench-F simulator.** It almost certainly models room-based propagation. CONTEXT.md says benchmark work is deferred — but the simulator code still compiles against the live types. Either drop room from sim (surgical edit) or stub the sim until Phase 25. **Planner decision.**
...
describe('Phase 24 — CLI surface', () => {
  test('`folklore room` is unknown subcommand', () => {
    // exec via tsx, assert exit code 1 + "unknown" in stderr
  });

  test('`folklore save` rejects --room flag', () => { /* ... */ });

  test('`folklore save --private` sets node.private = true', () => { /* ... */ });

  test('`folklore ask --workspace all` does not apply pre-filter', () => { /* ... */ });
});
...
describe('Phase 24 — storage', () => {
  test('No code path opens ~/.folklore/rooms.json', () => {
  ...
  test('No code path opens ~/.folklore/shared-rooms.json (except migrate)', () => { /* same pattern */ });
...
// ============================================================
describe('Phase 24 — folklore hooks', () => {
...
3. **`federation-sim.ts`** — surgical edit or stub-until-Phase-25? CONTEXT.md defers FolkloreBench-F to a future phase, but the simulator code lives in `src/domain/` and will not compile after schema change.
...
7. **Should `folklore migrate v5` be auto-triggered on first v5 boot, or always-manual?** Recommend manual — explicit user opt-in to a one-way data transform. Boot-time `doctor` check nags until run.
