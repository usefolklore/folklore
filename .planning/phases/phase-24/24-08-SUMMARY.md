...
requirements_delivered:
  - "ROOMS-DEL-01 (no room CRUD CLI/tool reachable) — partially: MCP-side tools gone; CLI-side `folklore room` is Wave 1/3"
...
External consumers (Claude Code, Cursor, Cline, Gemini CLI, any other MCP client wired to `folklore mcp start`) MUST update tool-call envelopes:
