...
dependency_graph:
  requires:
    - phase: phase-24-01
      provides: "WellinformedNodeFields with workspace + private, no room (schema wedge)"
    - phase: phase-24-02
...
tech-stack:
  added: []
  patterns:
    - "Envelope-layer V5 enforcement — libp2p protocol-path string (/wellinformed/{search,touch}/1.0.0) is unchanged; protocol version lives in the JSON envelope as `protocol_version: 5`. Pre-V5 peers receive a clear envelope-parse refusal payload, not 'protocol not handled'."
...
key-decisions:
  - "Libp2p protocol-path stays `/wellinformed/{search,touch}/1.0.0` — V5 is enforced at the envelope layer, not via a new protocol id. Pre-V5 peers get clean refusal payloads rather than 'protocol not handled' transport errors. Documented in commit messages."
...
### Libp2p protocol-path stays at `/wellinformed/{search,touch}/1.0.0`
