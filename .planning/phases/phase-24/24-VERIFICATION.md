...
gaps: []
human_verification:
  - test: "Smoke `folklore save --private` from a fresh git repo; then `folklore share <peer>` — confirm the private node is excluded from outbound Y.Map upsert."
  ...
  - test: "From a clone of an unmigrated V4 install (with rooms.json + shared-rooms.json + room-tagged nodes), run `folklore migrate v5` twice."
...
| 2 | `folklore ask "x"` from a git repo returns nodes filtered by workspace pre-filter; cross-workspace results available with `--workspace all`. | VERIFIED | `src/cli/commands/ask.ts`: parses `--workspace` (lines 52-53); `--workspace all` → `workspace = undefined` (no filter); absent → `detectWorkspace()` (line 68); `applyWorkspaceFilter` (line 127) filters `search_hits` and `recall_result.hits` by `h.workspace === workspace`. |
| 3 | `folklore share <peer>` shares all `private === false` nodes; no `--room` flag, no `shared-rooms.json` to maintain. `folklore save --private` sets the flag. | VERIFIED | `src/cli/commands/share.ts`: no `--room` argument anywhere; filters `graph.json.nodes.filter((n) => n.private !== true)` (line 102); blocks on secret-bearing nodes with `folklore save --private` guidance (line 134). `src/infrastructure/share-store.ts` deleted; sharing pipeline operates on the candidate-node list directly. |
...
| V4 data on disk | V5 detection + migration | `folklore migrate v5` | WIRED | migrate.ts `detectV4` checks for rooms.json, shared-rooms.json, or `n.room` field; transformNodes strips room, defaults private, maps room → workspace heuristically; flattenReputation removes `room:*` subject keys. |
| `.claude` hooks | folklore CLI | hit-format JSON output | WIRED | folklore-mcp-pre.cjs and folklore-prompt-submit.cjs comments document V5 contract; no `--room` flag passed; no `rooms.json` read. |

### Requirements Coverage

| Requirement | Source Plan(s) | Description | Status | Evidence |
|-------------|----------------|-------------|--------|----------|
| ROOMS-DEL-01 | 24-02, 24-08 | `folklore room` CLI command removed; no subcommand routes to room CRUD. | SATISFIED | `src/cli/commands/room.ts` deleted. `grep "room" src/cli/index.ts` returns 0 hits. No `'room':` key in the dispatch table (lines 62-123). |
| ROOMS-DEL-02 | 24-02, 24-04 | `~/.folklore/rooms.json` no longer read or written. | SATISFIED | `grep "rooms.json" src/` only matches: migrate.ts (deletion path), doctor.ts (V4 artifact detection), comments. Zero active read/write code paths. |
| ROOMS-DEL-03 | 24-02, 24-06 | `~/.folklore/shared-rooms.json` removed; sharing gates on `node.private === false`. | SATISFIED | `src/infrastructure/share-store.ts` deleted; `share.ts:102` gates on `n.private !== true`; `recall-sync.ts:300` gates on `built.node.private !== false`; `share-sync.ts:210` `collectShareable` filters on `n.private === false`. |
...
| ROOMS-DEL-08 | 24-05 | All `.claude/hooks/folklore-*` scripts format hits without `room` field; pass test suite. | SATISFIED | grep across all six folklore-* hook scripts finds only V5-narrative comments — no `--room` flag passing, no `~/.folklore/rooms.json` reads, no `room` field in formatted hit output. Phase24 hooks test suite at tests/phase24.rooms-deleted.test.ts:538 passes. |
...
| `src/cli/commands/sessions.ts` | 37, 45, 72 | Help text refers to `folklore trigger --room sessions`. | Warning | Stale guidance pointing to a no-longer-valid flag. Should update to V5 syntax. |
