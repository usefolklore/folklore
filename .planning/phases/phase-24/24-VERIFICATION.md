...
gaps: []
human_verification:
  - test: "Smoke `akashik save --private` from a fresh git repo; then `akashik share <peer>` — confirm the private node is excluded from outbound Y.Map upsert."
  ...
  - test: "From a clone of an unmigrated V4 install (with rooms.json + shared-rooms.json + room-tagged nodes), run `akashik migrate v5` twice."
...
| V4 data on disk | V5 detection + migration | `akashik migrate v5` | WIRED | migrate.ts `detectV4` checks for rooms.json, shared-rooms.json, or `n.room` field; transformNodes strips room, defaults private, maps room → workspace heuristically; flattenReputation removes `room:*` subject keys. |
| `.claude` hooks | akashik CLI | hit-format JSON output | WIRED | akashik-mcp-pre.cjs and akashik-prompt-submit.cjs comments document V5 contract; no `--room` flag passed; no `rooms.json` read. |

### Requirements Coverage

| Requirement | Source Plan(s) | Description | Status | Evidence |
|-------------|----------------|-------------|--------|----------|
| ROOMS-DEL-01 | 24-02, 24-08 | `akashik room` CLI command removed; no subcommand routes to room CRUD. | SATISFIED | `src/cli/commands/room.ts` deleted. `grep "room" src/cli/index.ts` returns 0 hits. No `'room':` key in the dispatch table (lines 62-123). |
| ROOMS-DEL-02 | 24-02, 24-04 | `~/.akashik/rooms.json` no longer read or written. | SATISFIED | `grep "rooms.json" src/` only matches: migrate.ts (deletion path), doctor.ts (V4 artifact detection), comments. Zero active read/write code paths. |
| ROOMS-DEL-03 | 24-02, 24-06 | `~/.akashik/shared-rooms.json` removed; sharing gates on `node.private === false`. | SATISFIED | `src/infrastructure/share-store.ts` deleted; `share.ts:102` gates on `n.private !== true`; `recall-sync.ts:300` gates on `built.node.private !== false`; `share-sync.ts:210` `collectShareable` filters on `n.private === false`. |
...
| ROOMS-DEL-08 | 24-05 | All `.claude/hooks/akashik-*` scripts format hits without `room` field; pass test suite. | SATISFIED | grep across all six akashik-* hook scripts finds only V5-narrative comments — no `--room` flag passing, no `~/.akashik/rooms.json` reads, no `room` field in formatted hit output. Phase24 hooks test suite at tests/phase24.rooms-deleted.test.ts:538 passes. |
...
| `src/cli/commands/sessions.ts` | 37, 45, 72 | Help text refers to `akashik trigger --room sessions`. | Warning | Stale guidance pointing to a no-longer-valid flag. Should update to V5 syntax. |
