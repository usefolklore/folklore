...
dependency_graph:
  requires:
    - phase: phase-24-01
      provides: "FolkloreNodeFields with workspace + private (schema wedge); Room kept as deprecated string alias"
    - phase: phase-24-02
...
key-decisions:
  ...
  - "Telegram capture sets private:false unconditionally. Forwarded URLs to a public-ish chat are de-facto sharable; users who need privacy should run `folklore save --private` after the fact. Multi-tier privacy (per-recipient sharing) is deferred per the phase synthesis."
...
Wave 4 (migration + tests) is unblocked. Plan 11 (`folklore migrate v5`) can lean on the peer-reputation-store filter pattern this plan established — the runtime is now lossless across the V4→V5 boundary even before migration runs.
