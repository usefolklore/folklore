...
**Success criteria:**
1. First run generates ed25519 keypair at ~/.akashik/peer-identity.json
...
- [x] 16-02-PLAN.md — Sync engine: /akashik/share/1.0.0 libp2p protocol, REMOTE_ORIGIN echo prevention, secrets-scanned in/out updates, debounced graph flush — DONE 2026-04-12
...
- [x] 17-02-PLAN.md — Protocol + discovery infra: mDNS/DHT/peer:discovery, /akashik/search/1.0.0, token bucket, runFederatedSearch — DONE 2026-04-12
...
**Success criteria:**
1. `akashik codebase index <path>` parses a TypeScript/JavaScript codebase into `~/.akashik/code-graph.db` with classes, functions, methods, imports, exports
2. `akashik codebase attach <codebase-id> --room <room-id>` attaches a codebase to a research room (M:N)
3. `akashik codebase search <query>` returns code nodes by semantic match across attached codebases
...
2. `akashik recent-sessions` CLI command shows last N sessions with duration, tool-call summary, and assistant-message highlights
...
- [x] 24-11-PLAN.md — Wave 4a: Build `akashik migrate v5` command (idempotent, lossless, --rollback); grow doctor to nag on v4 data; write V5-PROTOCOL.md, deprecate V4-PROTOCOL.md, update peer-reputation-design.md
...
2. `akashik ask "x"` from a git repo returns nodes filtered by workspace pre-filter; cross-workspace results available with `--workspace all`.
3. `akashik share <peer>` shares all `private === false` nodes; no `--room` flag, no `shared-rooms.json` to maintain. `akashik save --private` sets the flag.
4. Federation wire protocol bumped to V5: `SearchRequest.room` and `SearchResponse.room` fields removed; pre-V5 peers receive a clear protocol-version error. The 5 existing rooms migrate to a flat namespace via `akashik migrate v5` (one-shot, idempotent, lossless except the room field is dropped onto an optional `workspace` derived heuristically or null).
