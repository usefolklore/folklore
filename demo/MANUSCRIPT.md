...
1. **akashik turns local research into agent context faster than the agent can search the web.**
...
```bash
# 1. Clean akashik home so the demo starts from a known state.
mv ~/.akashik ~/.akashik.backup-$(date +%s) 2>/dev/null || true
...
export AKASHIK_GITHUB_CLIENT_ID="<your_dev_oauth_app_client_id>"   # optional
akashik onboard --yes

# 3. Verify the daemon is up + IPC reachable.
akashik daemon status
akashik metrics | jq .gauges     # expect queue.queued=0, queue.running=0
...
akashik ask "boil-off detection in cryogenic hydrogen tanks" --k 3
...
"point akashik at a folder of notes" — that's what we demonstrate.
...
> and the ML used in that field. `akashik this me` chunks them,
...
```bash
$ time akashik ask "what ML methods are used for liquid hydrogen leak detection"
```

**What viewers see (annotated):**

```
# akashik agent contract (hook_version: 2)         ◄── decision FIRST
...
graph — no `akashik` invocation visible.

**Setup:** open a Claude Code session in a fresh directory. The `.claude/settings.json`
already has the akashik PreToolUse hook wired (from `akashik onboard` earlier).
...
- RIGHT (terminal tail of `~/.akashik/daemon.log`): the hook fires, `ask` runs,
  satisfaction=0.86, `action: use_memory`, ~80 ms.

**Voiceover:**

> Claude Code session. I ask a complex question. Behind the scenes, the
> akashik hook fires before Claude's tool call, runs `ask`, and
...
> `akashik`.** The agent is just smarter.

---

## Scene 4 — Side-by-side: Claude alone vs Claude + akashik (90 sec)

**Goal:** the headline timing comparison. Same question, two sessions, visible time delta.

**Setup:** two Claude Code panels side-by-side. **LEFT** has the akashik hook
...
- **RIGHT** (Claude + akashik): Claude responds in ~1.5 sec with specifics from the
...
> Same question. Same model. The only difference is whether akashik
...
**Goal:** show that akashik has TWO retrieval channels — semantic search AND
...
```bash
# Peer B (separate machine or another Akashik home dir):
AKASHIK_HOME=~/.akashik-peerB akashik onboard --yes
AKASHIK_HOME=~/.akashik-peerB akashik save \
...
AKASHIK_HOME=~/.akashik-peerB akashik share room research

# Peer A (your main demo machine):
akashik peer add /ip4/127.0.0.1/tcp/<peerB-port>/p2p/<peerB-id>
akashik share room research
```
...
$ akashik touch --peer peer-B --room research --label "open-source LH2 spectrometers"
touch: peer-B/research → 1 chunk pulled (0.4 KB) in 380 ms
graph updated: 1 new chunk · 0 entities · _source_peer=peer-B

$ akashik ask "open-source spectrometer projects for hydrogen sensing"
…
...
> `akashik touch` pulls just that note, attributes it to the peer
...
question answered with citations from `akashik`'s own source tree.

**Setup:** in the akashik repo:

```bash
$ akashik this me
indexing /Users/.../akashik … 312 chunks / 5.3 s
```

**Recording prompt (in Claude Code):**

```
How does the akashik daemon do bounded backpressure on the job queue?
```
...
> Three things make akashik different. It's local-first — your
...
> without a middleman. Try it: github.com/SaharBarak/akashik.
