...
1. **folklore turns local research into agent context faster than the agent can search the web.**
...
```bash
# 1. Clean folklore home so the demo starts from a known state.
mv ~/.folklore ~/.folklore.backup-$(date +%s) 2>/dev/null || true
...
export FOLKLORE_GITHUB_CLIENT_ID="<your_dev_oauth_app_client_id>"   # optional
folklore onboard --yes

# 3. Verify the daemon is up + IPC reachable.
folklore daemon status
folklore metrics | jq .gauges     # expect queue.queued=0, queue.running=0
...
folklore ask "boil-off detection in cryogenic hydrogen tanks" --k 3
...
"point folklore at a folder of notes" — that's what we demonstrate.
...
> and the ML used in that field. `folklore this me` chunks them,
...
```bash
$ time folklore ask "what ML methods are used for liquid hydrogen leak detection"
```

**What viewers see (annotated):**

```
# folklore agent contract (hook_version: 2)         ◄── decision FIRST
...
graph — no `folklore` invocation visible.

**Setup:** open a Claude Code session in a fresh directory. The `.claude/settings.json`
already has the folklore PreToolUse hook wired (from `folklore onboard` earlier).
...
- RIGHT (terminal tail of `~/.folklore/daemon.log`): the hook fires, `ask` runs,
  satisfaction=0.86, `action: use_memory`, ~80 ms.

**Voiceover:**

> Claude Code session. I ask a complex question. Behind the scenes, the
> folklore hook fires before Claude's tool call, runs `ask`, and
...
> `folklore`.** The agent is just smarter.

---

## Scene 4 — Side-by-side: Claude alone vs Claude + folklore (90 sec)

**Goal:** the headline timing comparison. Same question, two sessions, visible time delta.

**Setup:** two Claude Code panels side-by-side. **LEFT** has the folklore hook
...
- **RIGHT** (Claude + folklore): Claude responds in ~1.5 sec with specifics from the
...
> Same question. Same model. The only difference is whether folklore
...
**Goal:** show that folklore has TWO retrieval channels — semantic search AND
...
```bash
# Peer B (separate machine or another Folklore home dir):
FOLKLORE_HOME=~/.folklore-peerB folklore onboard --yes
FOLKLORE_HOME=~/.folklore-peerB folklore save \
...
FOLKLORE_HOME=~/.folklore-peerB folklore share room research

# Peer A (your main demo machine):
folklore peer add /ip4/127.0.0.1/tcp/<peerB-port>/p2p/<peerB-id>
folklore share room research
```
...
$ folklore touch --peer peer-B --room research --label "open-source LH2 spectrometers"
touch: peer-B/research → 1 chunk pulled (0.4 KB) in 380 ms
graph updated: 1 new chunk · 0 entities · _source_peer=peer-B

$ folklore ask "open-source spectrometer projects for hydrogen sensing"
…
...
> `folklore touch` pulls just that note, attributes it to the peer
...
question answered with citations from `folklore`'s own source tree.

**Setup:** in the folklore repo:

```bash
$ folklore this me
indexing /Users/.../folklore … 312 chunks / 5.3 s
```

**Recording prompt (in Claude Code):**

```
How does the folklore daemon do bounded backpressure on the job queue?
```
...
> Three things make folklore different. It's local-first — your
...
> without a middleman. Try it: github.com/SaharBarak/folklore.
