#!/usr/bin/env bash
#
# folklore demo — peer reputation validation harness.
#
...
# via `folklore peers rep` to verify the wire-up actually
...
#
# Set FOLKLORE_DEMO_KEEP_DAEMONS=1 to inspect the running mesh
...
# `folklore ask --peers` boots its OWN ephemeral libp2p node and
...
FOLKLORE_HOME="$A_HOME" folklore daemon stop || true
# Remove the stale daemon.sock — the bin/folklore.js shim's
...
for q in \
  ...
  echo
  echo "→ ask: \"$q\""
  FOLKLORE_HOME="$A_HOME" folklore ask "$q" --peers --k 5 \
    | head -10 \
    || true
...
echo
echo "── \`folklore peers rep\` (default — top-3 per peer) ──"
FOLKLORE_HOME="$A_HOME" folklore peers rep || true

echo
echo "── \`folklore peers rep --subject room:research\` ──"
FOLKLORE_HOME="$A_HOME" folklore peers rep --subject room:research || true

echo
echo "── raw rep file (~/.folklore.demo/peer-reputation.json) ──"
