#!/usr/bin/env bash
#
# wellinformed demo — peer reputation validation harness.
#
...
# via `wellinformed peers rep` to verify the wire-up actually
...
#
# Set WELLINFORMED_DEMO_KEEP_DAEMONS=1 to inspect the running mesh
...
# `wellinformed ask --peers` boots its OWN ephemeral libp2p node and
...
WELLINFORMED_HOME="$A_HOME" wellinformed daemon stop || true
# Remove the stale daemon.sock — the bin/wellinformed.js shim's
...
for q in \
  ...
  echo
  echo "→ ask: \"$q\""
  WELLINFORMED_HOME="$A_HOME" wellinformed ask "$q" --peers --k 5 \
    | head -10 \
    || true
...
echo
echo "── \`wellinformed peers rep\` (default — top-3 per peer) ──"
WELLINFORMED_HOME="$A_HOME" wellinformed peers rep || true

echo
echo "── \`wellinformed peers rep --subject room:research\` ──"
WELLINFORMED_HOME="$A_HOME" wellinformed peers rep --subject room:research || true

echo
echo "── raw rep file (~/.wellinformed.demo/peer-reputation.json) ──"
