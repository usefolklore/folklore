#!/usr/bin/env bash
#
# wellinformed demo — P2P touch teardown.
#
...
# `~/.wellinformed.demo*.archived-<ts>` so the next setup starts
...
HOMES=(
  "${WELLINFORMED_DEMO_HOME:-$HOME/.wellinformed.demo}"
...
echo "── wellinformed P2P teardown ────────────────────────────"
for h in "${HOMES[@]}"; do
  if [[ -d "$h" ]]; then
    name=$(basename "$h")
    echo -n "→ $name: "
    if WELLINFORMED_HOME="$h" wellinformed daemon stop 2>/dev/null; then
      :
    else
      echo "  (not running)"
    fi
...
# Belt-and-braces — kill any wellinformed daemon process still alive
# (e.g. crashed before writing PID file). Restricted to processes
# whose argv contains 'daemon _run' so we never touch the user's
# main wellinformed daemon if it shares the parent terminal session.
if pgrep -fl "wellinformed.*daemon.*_run" >/dev/null 2>&1; then
  echo "→ residual daemons found — sending SIGTERM"
  pkill -TERM -f "wellinformed.*daemon.*_run" 2>/dev/null || true
  sleep 0.5
fi
